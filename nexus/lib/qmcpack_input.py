...
class QIxml(Names):
    ...
    def get(self,names,namedict=None,host=False,root=True):
        ...
        for name in names:
            ...
            #end if
            name_absent = True if name not in namedict else False
            not_element = False
            if not name_absent:
        ...
        for name,value in self.items():
            if isinstance(value,QIxml):
                ...
                for n,v in value.items():
                    name_absent = True if n not in namedict else False
                    not_element = False
                    if not name_absent:
...
'''
    ...
    def generate_jastrows(self,size=None,j1func='bspline',j1size=8,j2func='bspline',j2size=8):
        ...
        no_lattice = True if lattice is None else False
        no_particleset = True if particlesets is None else False
        no_wavefunction = True if wavefunction is None else False
        if no_lattice:
    ...
    def cusp_correction(self):
        ...
        if not self.is_afqmc_input():
            ...
            cc_var = True if ds is not None and 'cuspcorrection' in ds and ds.cuspcorrection else False
...
def generate_simulationcell(bconds='ppp',lr_dim_cutoff=15,lr_tol=None,lr_handler=None,system=None):
    ...
    axes_valid = True if system is not None and len(system.structure.axes)>0 else False
    if periodic:
