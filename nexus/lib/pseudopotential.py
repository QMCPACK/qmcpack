...
class GaussianPP(SemilocalPP):
    ...
    def read_text(self,text,format=None,filepath=None):
        ...
        if format=='gamess':
            i=0
            name,type,Zcore,lmax = lines[i].split()
            i+=1
            Zcore = int(Zcore)
            lmax  = int(lmax)
            ...
            i=0
            element,token = lines[i].split()
            i+=1
            label,lmax,Zcore = lines[i].split()
            i+=1
            lmax = int(lmax)
            Zcore = int(Zcore)
            while i<len(lines):
                ...
                terms = []
                for j in range(n):
                    rpow,expon,coeff = lines[i].split()
                    i+=1
                    terms.append((float(coeff),int(rpow),float(expon)))
                #end for
                channels.append(terms)
            ...
            i = 0
            conv_atomic_number,nshells = lines[i].split()
            i+=1
            ...
            for nt in nterms:
                ...
                terms = []
                for n in range(nt):
                    expon,coeff,rpow = lines[i].split()
                    i+=1
            ...
            name = None
            i=0
            Zval,lmax = lines[i].split()
            i+=1
            Zval = int(Zval)
            lmax = int(lmax)-1
            element = self.element
            Zcore = int(pt[element].atomic_number)-Zval
            ns =  [int(n) for n in lines[i].split()]
            i+= 1
            while i<len(lines):
                for n in ns:
                    terms = []
