...
In the "qmc" object given below, pay attention to keyword "excitation   = ['up', '0 3 4 4']". Value for the excitation keyword can be given in list or tuple format. Here, first element of the list ('up') denotes the spin channel used to perform the excitation. Whereas '0 3 4 4' denotes the band-index (bi) and tiling-index (ti) of annihilation and creation (an/cr) operations in $bi_{an} ti_{an} bi_{cr} ti_{cr}$ format. Another option is to use an indexing of the orbitals depending on their energetic ordering. In this example this would correspond to "excitation   = ['up', '-47 +48']". Band/twist index and energy indexes of the orbitals can be found in "einspline" files or they can be determined after parsing the "nscf.out" file using PwscfAnalyzer. Examples on how to do are provided in Lab 5 of the QMCPACK manual.\\
%\begin{lstlisting}[caption={Nexus input script for the usage example (\texttt{diamond.py}).  DFT is performed in diamond primitive cell followed by VMC in a 16 atom supercell.},label={ex:d16}]
\HRule
\begin{minted}{python}
#! /usr/bin/env python
...
#Returns standardized Primitive cell -- run rest of the calculations on this cell
...
tiling = get_band_tiling(structure   = dia2_structure, 
                         kpoints_rel = [[0.000, 0.000, 0.000], 
                                        [0.377, 0.000, 0.377], 
                                        [0.500, 0.000, 0.500]])
#K-points can also be defined for high symmetry k-points, using their labels
#tiling = get_band_tiling(structure   = dia2_structure, 
#                         kpoints_label = ['GAMMA', 'X'])

# get_primitive_cell and get_band_tiling requires "SeeK-path" python libraries
...
nscf = generate_pwscf(
    ...
    conv_thr     = 1e-8,
    nosym        = True,
    nbnd         = 8,     #Must be set to a value larger than Nelect/2
    verbosity    = 'high',#Must be set to high to parse nscf.out using PwscfAnalyzer
    wf_collect   = True,
    system       = dia2,
    pseudos      = ['C.BFD.upf'],
    dependencies = (scf, 'charge_density'),
...
qmc = generate_qmcpack(
    ...
    identifier   = 'vmc',
    path         = 'diamond/vmc',
    job          = job(cores=32,threads=16,app='qmcpack', hours = 1),
    input_type   = 'basic',
    spin_polarized = True,
    system       = dia2,
