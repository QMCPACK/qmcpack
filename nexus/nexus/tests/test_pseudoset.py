...
def test_generate_pseudoset(tmp_path):
    pseudo_dir, _, _ = setup_psps(test_dir=tmp_path, code="qmcpack")

    single = generate_pseudoset(pseudo_dir=pseudo_dir)
    assert set(single) == {"qmcpack", "rmg"}
    assert single["qmcpack"] is single["rmg"]

    mixed = generate_pseudoset(
        pseudo_dir  = pseudo_dir,
        search_type = "mixed",
        codes       = "qmcpack",
        )
    assert set(mixed) == {"qmcpack"}

    with pytest.raises(ValueError, match="mixed-search arguments"):
        generate_pseudoset(pseudo_dir=pseudo_dir, codes="qmcpack")
    with pytest.raises(ValueError, match="single-search arguments"):
        generate_pseudoset(
            pseudo_dir=pseudo_dir, search_type="mixed", code="qmcpack"
            )
    with pytest.raises(ValueError, match="pseudo_dir must be provided"):
        generate_pseudoset()


def test_generate_pseudoset_label(tmp_path):
    pseudo_dir, pseudos, _ = setup_psps(test_dir=tmp_path, code="qmcpack")
    pseudo_files = deepcopy(PseudoSet.pseudo_files)
    labeled_pseudosets = deepcopy(PseudoSet.labeled_pseudosets)
    try:
        PseudoSet.pseudo_files = {pseudo.name: str(pseudo) for pseudo in pseudos}
        PseudoSet.labeled_pseudosets = {}
        generated = generate_pseudoset(
            label="generated", qmcpack=[pseudo.name for pseudo in pseudos]
            )
        assert generated is PseudoSet.labeled_pseudosets["generated"]
        assert set(generated) == {"qmcpack"}

        with pytest.raises(ValueError, match="registration and search"):
            generate_pseudoset(
                label="invalid",
                pseudo_dir=pseudo_dir,
                qmcpack=[pseudo.name for pseudo in pseudos],
                )
    finally:
        PseudoSet.pseudo_files = pseudo_files
        PseudoSet.labeled_pseudosets = labeled_pseudosets
