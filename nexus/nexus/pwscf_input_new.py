"""Module for generating input files for Quantum ESPRESSO.

A majority of the code in this module was auto-generated by a Python script that
uses the intermediate XML representation of QE's ``pw.x`` input
description files.

A description of how to turn QE's ``.def`` documentation files into ``.xml``
is given in QE's repository, at ``q-e/dev-tools/README.helpdoc``.

Notes
-----
If you wish to add a new variable to a namelist enum, you
...
PwscfArrayInput: TypeAlias = npt.NDArray[np.floating] | Sequence[Sequence[float]] | Sequence[float]
...
#end class HubbardOrbitalResolved
...
#end class HubbardInterSite

HubbardEntry: TypeAlias = HubbardOnSite | HubbardInterSite | HubbardOrbitalResolved
