...
"""Pseudopotential-set discovery, validation, and registration utilities."""
...
def generate_pseudoset(
        # ppset inputs
        label         = unset,
        # from_dir/from_mixed_dir inputs
        search_type   = unset,
        pseudo_dir    = unset,
        skip_invalid  = unset,
        # from_dir inputs
    ...
    """Generate or register pseudopotential sets.

    This provides one entry point for :func:`ppset`,
    :meth:`PseudoSet.from_dir`, and :meth:`PseudoSet.from_mixed_dir`.
    ``search_type`` defaults to ``"single"`` when omitted.
    """
    search_type_provided = search_type is not unset
    if not search_type_provided:
        search_type = 'single'
    ...
    if label is not unset:
        ...
        for name, value in (
                ...
                ('patterns'     , patterns     ),
                ('code_Zeff_map', code_Zeff_map),
                ('skip_invalid' , skip_invalid ),
                ):
            if value is not unset:
                invalid.append(name)
        if search_type_provided:
            invalid.append('search_type')
        if invalid:
            raise ValueError(
                'Cannot provide registration and search arguments together:'
                '  {}'.format(invalid)
                )
        ppset(label, **codes_pps)
        return PseudoSet.labeled_pseudosets[label]

    if len(codes_pps)>0:
        raise ValueError(
            'Pseudopotential code keywords can only be provided with label'
            )
    if search_type not in {'single', 'mixed'}:
        raise ValueError(
            'search_type must be either "single" or "mixed", '
           f'but is {search_type}'
            )
    if pseudo_dir is unset:
        raise ValueError('pseudo_dir must be provided')
    ...
    if search_type == 'single':
        ...
        if len(invalid)>0:
            raise ValueError(
                'Cannot provide mixed-search arguments with search_type="single": '
                '  {}'.format(invalid)
                )
        ps = PseudoSet.from_dir(
            pseudo_dir   = pseudo_dir,
            code         = 'detect' if code is unset else code,
            Zeff_map     = None if Zeff_map is unset else Zeff_map,
            ext_filter   = None if ext_filter is unset else ext_filter,
            pattern      = None if pattern is unset else pattern,
            skip_invalid = False if skip_invalid is unset else skip_invalid,
            )
        pseudosets = {code:ps for code in ps.codes}
    else:
        invalid = []
        for name,value in  (
                ('code'      , code      ), 
        ...
        if len(invalid)>0:
            raise ValueError(
                'Cannot provide single-search arguments with search_type="mixed": '
                '  {}'.format(invalid)
                )
        pseudosets = PseudoSet.from_mixed_dir(
            pseudo_dir    = pseudo_dir,
            codes         = None if codes is unset else codes,
            extensions    = None if extensions is unset else extensions,
            patterns      = None if patterns is unset else patterns,
            code_Zeff_map = None if code_Zeff_map is unset else code_Zeff_map,
            skip_invalid  = False if skip_invalid is unset else skip_invalid,
            )
    return pseudosets
