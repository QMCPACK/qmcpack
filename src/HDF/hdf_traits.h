...
namespace qmcplusplus {
  ...
  template<typename T>
    struct atomic_type_traits
    {
      ...
      hid_t h5_data_type;
      T& ref_;
      atomic_type_traits(T& a):ref_(a) {dims[0]=1;}
      inline hsize_t size() const { return 1;}
      inline const_pointer_type data() const { return &ref_;}
      inline pointer_type data() { return &ref_;}
    };
  ...
  struct h5_data_type_traits<vector<T> >
  {
    ...
    inline hsize_t size() const { return ref_.size();}
    inline const T* data() const { return &ref_[0];}
    inline T* data() { return &ref_[0];}
  };
  ...
  struct h5_data_type_traits<TinyVector<T,D> >
  {
    ...
    inline hsize_t size() const { return D;}
    inline const T* data() const { return &ref_[0];}
    inline T* data() { return &ref_[0];}
  };
  ...
  struct h5_data_type_traits<vector<TinyVector<T,D> > >
  {
    ...
    inline hsize_t size() const { return dims[2];}
    inline const T* data() const { return &ref_[0];}
    inline T* data() { return &ref_[0];}
  };
  ...
  struct h5_data_type_traits<Array<T,D> >
  {
    ...
    inline hsize_t size() const { 
      hsize_t tot=dims[1]; for(int i=1; i<D;++i) tot*=dims[i];
      return tot;
    }
    inline const T* data() const { return ref_->data();}
    inline T* data() { return ref_->data();}
  };
