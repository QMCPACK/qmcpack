...
void* cuda_memory_manager_type::allocate(size_t bytes, std::string name)
{
...
void* cuda_memory_manager_type::allocate_managed(size_t bytes, std::string name, unsigned int flags)
{
  // Make sure size is a multiple of 16
  bytes = (((bytes + 15) / 16) * 16);
  void* p;
...
void cuda_memory_manager_type::deallocate(void* p)
{
  ...
  {
    fprintf(stderr,
            "Error:  CUDA memory object %s not found "
            "in the memory map\n",
            name.c_str());
    abort();
  }
...
void cuda_memory_manager_type::report()
{
  fprintf(stderr, "\n\nGPU memory map contents:\n");
  fprintf(stderr,
          "Object name                                                    Num objects            Total bytes\n");
