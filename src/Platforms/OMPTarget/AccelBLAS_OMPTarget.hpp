...
template<>
inline void gemm_batched<PlatformKind::OMPTARGET, float>(BLASHandle<PlatformKind::OMPTARGET>& handle,
                                                         const char transa,
                                                         const char transb,
                                                         int m,
                                                         int n,
                                                         ...
                                                         int batchCount)
{
  if (ompBLAS::gemm_batched(handle.h_ompblas, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc,
                            batchCount) != 0)
    throw std::runtime_error("ompBLAS::gemm_batched<float> failed!");
}

template<>
inline void gemm_batched<PlatformKind::OMPTARGET, double>(BLASHandle<PlatformKind::OMPTARGET>& handle,
                                                          const char transa,
                                                          const char transb,
                                                          int m,
                                                          int n,
                                                          ...
                                                          int batchCount)
{
  if (ompBLAS::gemm_batched(handle.h_ompblas, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc,
                            batchCount) != 0)
    throw std::runtime_error("ompBLAS::gemm_batched<double> failed!");
}

template<>
inline void gemm_batched<PlatformKind::OMPTARGET, std::complex<float>>(BLASHandle<PlatformKind::OMPTARGET>& handle,
                                                                       const char transa,
                                                                       const char transb,
                                                                       int m,
                                                                       int n,
                                                                       ...
                                                                       int batchCount)
{
  if (ompBLAS::gemm_batched(handle.h_ompblas, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc,
                            batchCount) != 0)
    throw std::runtime_error("ompBLAS::gemm_batched<std::complex<float>> failed!");
}

template<>
inline void gemm_batched<PlatformKind::OMPTARGET, std::complex<double>>(BLASHandle<PlatformKind::OMPTARGET>& handle,
                                                                        const char transa,
                                                                        const char transb,
                                                                        int m,
                                                                        int n,
                                                                        ...
                                                                        int batchCount)
{
  if (ompBLAS::gemm_batched(handle.h_ompblas, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc,
                            batchCount) != 0)
    throw std::runtime_error("ompBLAS::gemm_batched<std::complex<double>> failed!");
}
