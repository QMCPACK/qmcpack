...
bool QMCFiniteSize::execute()
{
  ...
  for(int ks=0; ks<Klist.kshell.size()-1; ks++)
    {
      app_log() << setw(12) << setprecision(8) << std::sqrt(Klist.ksq[Klist.kshell[ks]])
                << setw(12) << setprecision(8) << vsk_1d[ks]
                << setw(12) << setprecision(8) << AA->Fk_symm[ks]
                << "\n";
    }
  ...
  for(int k=0; k<nk; k++)
    {
      ...
      app_log() << setw(12) << setprecision(8) << kval
                << setw(12) << setprecision(8) << sphericalAvgSk(sk3d_spline,kval)
                << "\n";
    }
