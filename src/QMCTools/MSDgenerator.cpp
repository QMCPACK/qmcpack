...
// dummy for now
   ...
   for(int i=0; i<nea; i++) alpha0[i]='1'; 
   for(int i=0; i<neb; i++) beta0[i]='1'; 
   for(int i=0; i<neb; i++) occ0[i]='2'; 
   for(int i=neb; i<nea; i++) occ0[i]='1'; 
  ...
  if(useCSF) parser->ci_size = CSFocc.size();
  else parser->ci_size = CIcoeff.size();
