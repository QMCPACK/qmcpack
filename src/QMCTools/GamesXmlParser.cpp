...
GamesXmlParser::GamesXmlParser(int argc, char** argv) : QMCGaussianParserBase(argc, argv)
{
  basisName  = "GamesXml";
  Normalized = "no";
}
    ...
    else if (cname == "OUT")
    {
      ...
      while (cur1 != NULL)
      {
        ...
        if (cname1 == "SYSTEM_STATE")
        {
          ...
          XMLAttrString unitL(cur1, "UNITS");
          if (unitL == "ANGS")
            BohrUnit = false;
...
void GamesXmlParser::getGeometry(std::vector<xmlNodePtr>& aPtrList)
{
  ...
  copy(IonSystem.GroupID.begin(), IonSystem.GroupID.end(), std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;
}
