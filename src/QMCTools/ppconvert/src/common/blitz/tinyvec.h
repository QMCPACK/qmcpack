...
template<typename T_numtype1, typename T_numtype2, int N_rows, int N_columns, int N_vecStride>
...
public:
  ...
  inline TinyVector(const T_numtype x[]) { memcpy(data_, x, N_length * sizeof(T_numtype)); }
  ...
  unsigned length() const { return N_length; }
  ...
  unsigned _bz_suggestLength() const { return N_length; }

  bool _bz_hasFastAccess() const { return true; }

  T_numtype& restrict _bz_fastAccess(unsigned i) { return data_[i]; }

  T_numtype _bz_fastAccess(unsigned i) const { return data_[i]; }
  ...
  bool lengthCheck(unsigned i) const
  {
    BZPRECHECK(i < N_length,
               "TinyVector<" << BZ_DEBUG_TEMPLATE_AS_STRING_LITERAL(T_numtype) << "," << N_length
