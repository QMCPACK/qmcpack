...
public:
  ...
  static const int numArrayOperands = T_expr::numArrayOperands, numIndexPlaceholders = T_expr::numIndexPlaceholders + 1,
                   rank = T_expr::rank - 1;

  _bz_ArrayExprReduce(const _bz_ArrayExprReduce<T_expr, N_index, T_reduction>& reduce)
      : reduce_(reduce.reduce_), iter_(reduce.iter_), ordering_(reduce.ordering_)
  {}

  _bz_ArrayExprReduce(T_expr expr) : iter_(expr) { computeOrdering(); }

  _bz_ArrayExprReduce(T_expr expr, T_reduction reduce) : iter_(expr), reduce_(reduce) { computeOrdering(); }

  int ascending(int r) { return iter_.ascending(r); }

  int ordering(int r) { return ordering_[r]; }

  int lbound(int r) { return iter_.lbound(r); }

  int ubound(int r) { return iter_.ubound(r); }

  template<int N_destRank>
  T_numtype operator()(const TinyVector<int, N_destRank>& destIndex)
  {
    BZPRECHECK(N_destRank == N_index,
               "Array reduction performed over rank " << N_index << " to produce a rank " << N_destRank
                                                      << " expression." << std::endl
    ...
    _bz_meta_vecAssign<N_index, 0>::assign(index, destIndex, _bz_update<int, int>());
    ...
    BZPRECHECK((lbound != INT_MIN) && (ubound != INT_MAX),
               "Array reduction performed over rank " << N_index << " is unbounded." << std::endl
                                                      << "There must be an array object in the expression being reduced"
                                                      << std::endl
                                                      << "which provides a bound in rank " << N_index << ".");

    reduce_.reset();

    for (index[N_index] = iter_.lbound(N_index); index[N_index] <= ubound; ++index[N_index])
    {
...
private:
  ...
  void computeOrdering()
  {
    ...
    for (int i = 0; i < rank; ++i)
    {
      ...
      if (orderingj != INT_MIN && orderingj < rank && !in_ordering(orderingj))
...
#define BZ_DECL_ARRAY_PARTIAL_REDUCE(fn, reduction)                                                                    \
  ...
  inline _bz_ArrayExpr<_bz_ArrayExprReduce<_bz_ArrayExpr<T_expr>, N_index, reduction<_bz_typename T_expr::T_numtype>>> \
      fn(_bz_ArrayExpr<T_expr> expr, const IndexPlaceholder<N_index>&)                                                 \
  {                                                                                                                    \
    return _bz_ArrayExprReduce<_bz_ArrayExpr<T_expr>, N_index, reduction<_bz_typename T_expr::T_numtype>>(expr);       \
  }                                                                                                                    \
                                                                                                                       \
  template<typename T_numtype, int N_rank, int N_index>                                                                \
  inline _bz_ArrayExpr<_bz_ArrayExprReduce<FastArrayIterator<T_numtype, N_rank>, N_index, reduction<T_numtype>>>       \
      fn(const Array<T_numtype, N_rank>& array, const IndexPlaceholder<N_index>&)                                      \
  {                                                                                                                    \
    return _bz_ArrayExprReduce<FastArrayIterator<T_numtype, N_rank>, N_index, reduction<T_numtype>>(                   \
        array.beginFast());                                                                                            \
  }
...
_bz_typename T_reduction::T_resulttype _bz_ArrayExprFullReduce(T_expr expr, T_reduction reduction);

#define BZ_DECL_ARRAY_FULL_REDUCE(fn, reduction)                                                             \
  template<typename T_expr>                                                                                  \
  inline _bz_typename reduction<_bz_typename T_expr::T_numtype>::T_resulttype fn(_bz_ArrayExpr<T_expr> expr) \
  {                                                                                                          \
    return _bz_ArrayExprFullReduce(expr, reduction<_bz_typename T_expr::T_numtype>());                       \
  }                                                                                                          \
                                                                                                             \
  template<typename T_numtype, int N_rank>                                                                   \
  inline _bz_typename reduction<T_numtype>::T_resulttype fn(const Array<T_numtype, N_rank>& array)           \
  {                                                                                                          \
    return _bz_ArrayExprFullReduce(array.beginFast(), reduction<T_numtype>());                               \
  }
...
#define BZ_DECL_ARRAY_FULL_REDUCE_INDEXVECTOR(fn, reduction)                                                    \
  template<typename T_expr>                                                                                     \
  inline _bz_typename reduction<_bz_typename T_expr::T_numtype, T_expr::rank>::T_resulttype fn(                 \
      _bz_ArrayExpr<T_expr> expr)                                                                               \
  {                                                                                                             \
    return _bz_reduceWithIndexVectorTraversal(expr, reduction<_bz_typename T_expr::T_numtype, T_expr::rank>()); \
  }                                                                                                             \
                                                                                                                \
  template<typename T_numtype, int N_rank>                                                                      \
  inline _bz_typename reduction<T_numtype, N_rank>::T_resulttype fn(const Array<T_numtype, N_rank>& array)      \
  {                                                                                                             \
    return _bz_reduceWithIndexVectorTraversal(array.beginFast(), reduction<T_numtype, N_rank>());               \
  }
