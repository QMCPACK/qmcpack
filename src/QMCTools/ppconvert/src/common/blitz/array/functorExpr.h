...
public:
  ...
  static const int numArrayOperands = T_expr::numArrayOperands, numIndexPlaceholders = T_expr::numIndexPlaceholders,
                   rank = T_expr::rank;

  _bz_FunctorExpr(const _bz_FunctorExpr<P_functor, P_expr, P_result>& a) : f_(a.f_), iter_(a.iter_) {}

  _bz_FunctorExpr(BZ_ETPARM(T_functor) f, BZ_ETPARM(T_expr) a) : f_(f), iter_(a) {}

  _bz_FunctorExpr(BZ_ETPARM(T_functor) f, _bz_typename T_expr::T_ctorArg1 a) : f_(f), iter_(a) {}

#if BZ_TEMPLATE_CTOR_DOESNT_CAUSE_HAVOC
  template<typename T1>
  explicit _bz_FunctorExpr(BZ_ETPARM(T_functor) f, BZ_ETPARM(T1) a) : f_(f), iter_(a)
  {}
#endif

  T_numtype operator*() { return f_(*iter_); }
  ...
  int ascending(int rank) { return iter_.ascending(rank); }

  int ordering(int rank) { return iter_.ordering(rank); }

  int lbound(int rank) { return iter_.lbound(rank); }

  int ubound(int rank) { return iter_.ubound(rank); }

  void push(int position) { iter_.push(position); }

  void pop(int position) { iter_.pop(position); }

  void advance() { iter_.advance(); }

  void advance(int n) { iter_.advance(n); }

  void loadStride(int rank) { iter_.loadStride(rank); }

  bool isUnitStride(int rank) const { return iter_.isUnitStride(rank); }
  ...
  T_numtype operator[](int i) { return f_(iter_[i]); }

  T_numtype fastRead(int i) { return f_(iter_.fastRead(i)); }

  int suggestStride(int rank) const { return iter_.suggestStride(rank); }

  bool isStride(int rank, int stride) const { return iter_.isStride(rank, stride); }
...
public:
  ...
  static const int numArrayOperands     = T_expr1::numArrayOperands + T_expr2::numArrayOperands,
                   numIndexPlaceholders = T_expr1::numIndexPlaceholders + T_expr2::numIndexPlaceholders,
                   rank                 = T_expr1::rank > T_expr2::rank ? T_expr1::rank : T_expr2::rank;

  _bz_FunctorExpr2(const _bz_FunctorExpr2<P_functor, P_expr1, P_expr2, P_result>& a)
      : f_(a.f_), iter1_(a.iter1_), iter2_(a.iter2_)
  {}

  _bz_FunctorExpr2(BZ_ETPARM(T_functor) f, BZ_ETPARM(T_expr1) a, BZ_ETPARM(T_expr2) b) : f_(f), iter1_(a), iter2_(b) {}

  template<typename T1, typename T2>
  _bz_FunctorExpr2(BZ_ETPARM(T_functor) f, BZ_ETPARM(T1) a, BZ_ETPARM(T2) b) : f_(f), iter1_(a), iter2_(b)
  {}

  T_numtype operator*() { return f_(*iter1_, *iter2_); }
  ...
  int ascending(int rank) { return bounds::compute_ascending(rank, iter1_.ascending(rank), iter2_.ascending(rank)); }

  int ordering(int rank) { return bounds::compute_ordering(rank, iter1_.ordering(rank), iter2_.ordering(rank)); }

  int lbound(int rank) { return bounds::compute_lbound(rank, iter1_.lbound(rank), iter2_.lbound(rank)); }

  int ubound(int rank) { return bounds::compute_ubound(rank, iter1_.ubound(rank), iter2_.ubound(rank)); }
  ...
  bool isUnitStride(int rank) const { return iter1_.isUnitStride(rank) && iter2_.isUnitStride(rank); }
  ...
  bool canCollapse(int outerLoopRank, int innerLoopRank) const
  {
    return iter1_.canCollapse(outerLoopRank, innerLoopRank) && iter2_.canCollapse(outerLoopRank, innerLoopRank);
  }

  T_numtype operator[](int i) { return f_(iter1_[i], iter2_[i]); }

  T_numtype fastRead(int i) { return f_(iter1_.fastRead(i), iter2_.fastRead(i)); }
  ...
  bool isStride(int rank, int stride) const { return iter1_.isStride(rank, stride) && iter2_.isStride(rank, stride); }
...
template<typename P_functor, typename P_expr1, typename P_expr2, typename P_expr3, class P_result>
...
public:
  ...
  static const int numArrayOperands = T_expr1::numArrayOperands + T_expr2::numArrayOperands + T_expr3::numArrayOperands,
                   numIndexPlaceholders =
                       T_expr1::numIndexPlaceholders + T_expr2::numIndexPlaceholders + T_expr3::numIndexPlaceholders,
                   rank12 = T_expr1::rank > T_expr2::rank ? T_expr1::rank : T_expr2::rank,
                   rank   = rank12 > T_expr3::rank ? rank12 : T_expr3::rank;

  _bz_FunctorExpr3(const _bz_FunctorExpr3<P_functor, P_expr1, P_expr2, P_expr3, P_result>& a)
      : f_(a.f_), iter1_(a.iter1_), iter2_(a.iter2_), iter3_(a.iter3_)
  {}

  _bz_FunctorExpr3(BZ_ETPARM(T_functor) f, BZ_ETPARM(T_expr1) a, BZ_ETPARM(T_expr2) b, BZ_ETPARM(T_expr3) c)
      : f_(f), iter1_(a), iter2_(b), iter3_(c)
  {}

  template<typename T1, typename T2, typename T3>
  _bz_FunctorExpr3(BZ_ETPARM(T_functor) f, BZ_ETPARM(T1) a, BZ_ETPARM(T2) b, BZ_ETPARM(T3) c)
      : f_(f), iter1_(a), iter2_(b), iter3_(c)
  {}

  T_numtype operator*() { return f_(*iter1_, *iter2_, *iter3_); }
  ...
  int ascending(int rank)
  {
    return bounds::compute_ascending(rank, iter1_.ascending(rank),
                                     bounds::compute_ascending(rank, iter2_.ascending(rank), iter3_.ascending(rank)));
  }

  int ordering(int rank)
  {
    return bounds::compute_ordering(rank, iter1_.ordering(rank),
                                    bounds::compute_ordering(rank, iter2_.ordering(rank), iter3_.ordering(rank)));
  }

  int lbound(int rank)
  {
    return bounds::compute_lbound(rank, iter1_.lbound(rank),
                                  bounds::compute_lbound(rank, iter2_.lbound(rank), iter3_.lbound(rank)));
  }

  int ubound(int rank)
  {
    return bounds::compute_ubound(rank, iter1_.ubound(rank),
                                  bounds::compute_ubound(rank, iter2_.ubound(rank), iter3_.ubound(rank)));
  }
  ...
  {
    return iter1_.isUnitStride(rank) && iter2_.isUnitStride(rank) && iter3_.isUnitStride(rank);
  }
  ...
  T_numtype operator[](int i) { return f_(iter1_[i], iter2_[i], iter3_[i]); }

  T_numtype fastRead(int i) { return f_(iter1_.fastRead(i), iter2_.fastRead(i), iter3_.fastRead(i)); }
  ...
  {
    return iter1_.isStride(rank, stride) && iter2_.isStride(rank, stride) && iter3_.isStride(rank, stride);
  }
  ...
  bool shapeCheck(const T_shape& shape)
  {
    return iter1_.shapeCheck(shape) && iter2_.shapeCheck(shape) && iter3_.shapeCheck(shape);
  }
...
_bz_inline_et _bz_ArrayExpr<
    _bz_FunctorExpr<P_functor, _bz_typename asExpr<P_expr>::T_expr, _bz_typename asExpr<P_expr>::T_expr::T_numtype>>
    applyFunctor(const P_functor& f, const ETBase<P_expr>& a)
{
  typedef _bz_FunctorExpr<P_functor, _bz_typename asExpr<P_expr>::T_expr,
...
_bz_inline_et _bz_ArrayExpr<_bz_FunctorExpr2<P_functor,
    ...
    applyFunctor(const P_functor& f, const ETBase<P_expr1>& a, const ETBase<P_expr2>& b)
{
  typedef _bz_FunctorExpr2<P_functor, _bz_typename asExpr<P_expr1>::T_expr, _bz_typename asExpr<P_expr2>::T_expr,
                           BZ_PROMOTE(_bz_typename asExpr<P_expr1>::T_expr::T_numtype,
                                      _bz_typename asExpr<P_expr2>::T_expr::T_numtype)>
      f2;
...
_bz_inline_et _bz_ArrayExpr<_bz_FunctorExpr3<P_functor,
                                             _bz_typename asExpr<P_expr1>::T_expr,
                                             _bz_typename asExpr<P_expr2>::T_expr,
    ...
    applyFunctor(const P_functor& f, const ETBase<P_expr1>& a, const ETBase<P_expr2>& b, const ETBase<P_expr3>& c)
{
  typedef _bz_FunctorExpr3<P_functor, _bz_typename asExpr<P_expr1>::T_expr, _bz_typename asExpr<P_expr2>::T_expr,
                           _bz_typename asExpr<P_expr3>::T_expr,
                           BZ_PROMOTE(_bz_typename asExpr<P_expr1>::T_expr::T_numtype,
                                      BZ_PROMOTE(_bz_typename asExpr<P_expr2>::T_expr::T_numtype,
                                                 _bz_typename asExpr<P_expr3>::T_expr::T_numtype))>
...
#define _BZ_MAKE_FUNCTOR(classname, funcname)                       \
  ...
  {                                                                 \
  public:                                                           \
    _bz_Functor##classname##funcname(const classname& c) : c_(c) {} \
...
#define _BZ_MAKE_FUNCTOR2(classname, funcname)                                             \
  ...
  public:                                                                                  \
    ...
    inline BZ_PROMOTE(T_numtype1, T_numtype2) operator()(T_numtype1 x, T_numtype2 y) const \
    {                                                                                      \
...
#define _BZ_MAKE_FUNCTOR3(classname, funcname)                                                       \
  ...
  public:                                                                                            \
    ...
    inline BZ_PROMOTE(BZ_PROMOTE(T_numtype1, T_numtype2), T_numtype3) operator()(T_numtype1 x,       \
                                                                                 T_numtype2 y,       \
                                                                                 T_numtype3 z) const \
    {                                                                                                \
...
#define BZ_DECLARE_FUNCTOR(classname)                                                                             \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < classname,                                      \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,                            \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr>::T_expr::T_numtype >> operator()(const BZ_BLITZ_SCOPE(ETBase) < \
                                                                                   P_expr > &a) const             \
  {                                                                                                               \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < classname,                             \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,                   \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr>::T_expr::T_numtype >> (*this, a.unwrap());                 \
  }

#define BZ_DECLARE_FUNCTOR2(classname)                                                                           \
  template<typename P_expr1, typename P_expr2>                                                                   \
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < classname,                                    \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                          \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr,                                                      \
      BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                            \
                 _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype) >>                         \
      operator()(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b) const \
  {                                                                                                              \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < classname,                           \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                 \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr,                                                 \
           BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                       \
                      _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype) >>                    \
...
#define BZ_DECLARE_FUNCTOR3(classname)                                                                                 \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < classname,                                          \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                                \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr3>::T_expr,      \
      BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                                  \
                 BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype,                       \
                            _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr3 > ::T_expr::T_numtype)) >>                   \
      operator()(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b,             \
                 const BZ_BLITZ_SCOPE(ETBase) < P_expr3 > &c) const                                                    \
  {                                                                                                                    \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < classname,                                 \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                       \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr3>::T_expr, \
           BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                             \
                      BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype,                  \
...
#define BZ_DECLARE_FUNCTOR_RET(classname, ret)                                                  \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < classname,                    \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,          \
      ret >> operator()(const BZ_BLITZ_SCOPE(ETBase) < P_expr > &a) const                       \
  {                                                                                             \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < classname,           \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr, \
           ret >> (*this, a.unwrap());                                                          \
...
#define BZ_DECLARE_FUNCTOR2_RET(classname, ret)                                                                   \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < classname,                                     \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                           \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr,                                                       \
      ret >> operator()(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b) \
                 const                                                                                            \
  {                                                                                                               \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < classname,                            \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                  \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, ret >> (*this, a.unwrap(), b.unwrap());          \
  }
...
#define BZ_DECLARE_FUNCTOR3_RET(classname, ret)                                                                        \
  ...
  {                                                                                                                    \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < classname,                                 \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                       \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr3>::T_expr, \
           ret >> (*this, a.unwrap(), b.unwrap(), c.unwrap());                                                         \
  }
...
#define BZ_DECLARE_MEMBER_FUNCTION(classname, funcname)                                                      \
  ...
  template<typename P_expr>                                                                                  \
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < _bz_Functor##classname##funcname,          \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,                       \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr>::T_expr::T_numtype >>                                      \
      funcname(const BZ_BLITZ_SCOPE(ETBase) < P_expr > &a) const                                             \
  {                                                                                                          \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < _bz_Functor##classname##funcname, \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,              \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr>::T_expr::T_numtype >> (*this, a.unwrap());            \
  }
...
#define BZ_DECLARE_MEMBER_FUNCTION2(classname, funcname)                                                       \
  ...
  template<typename P_expr1, typename P_expr2>                                                                 \
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < _bz_Functor##classname##funcname,           \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                        \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr,                                                    \
      BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                          \
                 _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype) >>                       \
      funcname(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b) const \
  {                                                                                                            \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < _bz_Functor##classname##funcname,  \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,               \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr,                                               \
           BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                     \
                      _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype) >>                  \
...
#define BZ_DECLARE_MEMBER_FUNCTION3(classname, funcname)                                                               \
  ...
  template<typename P_expr1, typename P_expr2, typename P_expr3>                                                       \
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < _bz_Functor##classname##funcname,                   \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                                \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr3>::T_expr,      \
      BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                                  \
                 BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype,                       \
                            _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr3 > ::T_expr::T_numtype)) >>                   \
      funcname(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b,               \
               const BZ_BLITZ_SCOPE(ETBase) < P_expr3 > &c) const                                                      \
  {                                                                                                                    \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < _bz_Functor##classname##funcname,          \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                       \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr3>::T_expr, \
           BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1 > ::T_expr::T_numtype,                             \
                      BZ_PROMOTE(_bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr2 > ::T_expr::T_numtype,                  \
...
#define BZ_DECLARE_MEMBER_FUNCTION_RET(classname, funcname, ret)                                             \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < _bz_Functor##classname##funcname,          \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,                       \
      ret >> funcname(const BZ_BLITZ_SCOPE(ETBase) < P_expr > &a) const                                      \
  {                                                                                                          \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr) < _bz_Functor##classname##funcname, \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr>::T_expr,              \
           ret >> (*this, a.unwrap());                                                                       \
...
#define BZ_DECLARE_MEMBER_FUNCTION2_RET(classname, funcname, ret)                                                     \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < _bz_Functor##classname##funcname,                  \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                               \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr,                                                           \
      ret >> funcname(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b) const \
  {                                                                                                                   \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr2) < _bz_Functor##classname##funcname,         \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                      \
           _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, ret >> (*this, a.unwrap(), b.unwrap());              \
...
#define BZ_DECLARE_MEMBER_FUNCTION3_RET(classname, funcname, ret)                                                      \
  ...
  BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < _bz_Functor##classname##funcname,                   \
                                _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                                \
      _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr2>::T_expr, _bz_typename BZ_BLITZ_SCOPE(asExpr)<P_expr3>::T_expr,      \
      ret >> funcname(const BZ_BLITZ_SCOPE(ETBase) < P_expr1 > &a, const BZ_BLITZ_SCOPE(ETBase) < P_expr2 > &b,        \
                      const BZ_BLITZ_SCOPE(ETBase) < P_expr3 > &c) const                                               \
  {                                                                                                                    \
    return BZ_BLITZ_SCOPE(_bz_ArrayExpr)<BZ_BLITZ_SCOPE(_bz_FunctorExpr3) < _bz_Functor##classname##funcname,          \
                                         _bz_typename BZ_BLITZ_SCOPE(asExpr) < P_expr1>::T_expr,                       \
