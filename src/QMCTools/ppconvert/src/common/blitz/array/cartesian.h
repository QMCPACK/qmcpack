...
public:
  ...
  iterator end() { return iterator(_cp_end_tag()); }

  CartesianProduct(const T_container& container0, const T_container& container1)
  {
  ...
  CartesianProduct(const T_container& container0, const T_container& container1, const T_container& container2)
  {
  ...
  const T_container& operator[](int i) { return *(containers_[i]); }
...
void CartesianProduct<T_tuple, T_container, N_containers>::debugDump()
{
  ...
  for (int i = 0; i < N_containers; ++i)
  {
    ...
    _bz_typename T_container::const_iterator iter = containers_[i]->begin(), end = containers_[i]->end();
    for (; iter != end; ++iter)
      std::cout << (*iter) << '\t';
  }
}
...
public:
  ...
  CartesianProductIterator(_cp_end_tag) { endflag_ = true; }

  bool operator==(const iterator& x) const { return (endflag_ == x.endflag_); }

  bool operator!=(const iterator& x) const { return endflag_ != x.endflag_; }

  const T_tuple& operator*() const { return tuple_; }
...
template<typename T_tuple, typename T_container, int N_containers>
void CartesianProductIterator<T_tuple, T_container, N_containers>::operator++()
{
