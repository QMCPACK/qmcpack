...
protected:
  ...
  explicit MemoryBlock(size_t items)
  {
    ...
    std::cout << "MemoryBlock: allocated " << std::setw(8) << length_ << " at " << ((void*)dataBlockAddress_)
              << std::endl;
#endif
  ...
  virtual ~MemoryBlock()
  {
    ...
    {
#ifdef BZ_DEBUG_LOG_ALLOCATIONS
      std::cout << "MemoryBlock:     freed " << std::setw(8) << length_ << " at " << ((void*)dataBlockAddress_)
                << std::endl;
#endif
  ...
  void addReference()
  {
    ...
    std::cout << "MemoryBlock:    reffed " << std::setw(8) << length_ << " at " << ((void*)dataBlockAddress_)
              << " (r=" << (int)references_ << ")" << std::endl;
#endif
    BZ_MUTEX_UNLOCK(mutex)
  }

  T_type* restrict data() { return data_; }

  const T_type* restrict data() const { return data_; }

  T_type*& dataBlockAddress() { return dataBlockAddress_; }
...
#ifdef BZ_DEBUG_LOG_REFERENCES
    std::cout << "MemoryBlock: dereffed  " << std::setw(8) << length_ << " at " << ((void*)dataBlockAddress_)
              << " (r=" << (int)references_ << ")" << std::endl;
#endif
    BZ_MUTEX_UNLOCK(mutex)
    return refcount;
...
{
public:
  UnownedMemoryBlock(size_t length, P_type* data) : MemoryBlock<P_type>(length, data)
  {
...
public:
...
#ifdef BZ_DEBUG_LOG_ALLOCATIONS
    std::cout << "MemoryBlockReference: created MemoryBlock at " << ((void*)block_) << std::endl;
#endif
  ...
  void blockRemoveReference()
  {
    ...
    {
#ifdef BZ_DEBUG_LOG_ALLOCATIONS
      std::cout << "MemoryBlock: no more refs, delete MemoryBlock object at " << ((void*)block_) << std::endl;
#endif
  ...
  ~MemoryBlockReference() { blockRemoveReference(); }
