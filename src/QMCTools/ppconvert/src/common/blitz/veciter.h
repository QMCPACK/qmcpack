...
public:
  ...
  explicit VectorIter(Vector<P_numtype>& x) : data_(x.data())
  {
    stride_ = x.stride();
    length_ = x.length();
  }

  VectorIter(P_numtype* restrict data, int stride, int length) : data_(data), stride_(stride), length_(length) {}
  ...
  P_numtype operator*() const { return *data_; }

  P_numtype& operator*() { return *data_; }
  ...
  static const int _bz_staticLengthCount = 0, _bz_dynamicLengthCount = 1, _bz_staticLength = 0;

  bool _bz_hasFastAccess() const { return isUnitStride(); }

  P_numtype _bz_fastAccess(int i) const { return data_[i]; }

  P_numtype& restrict _bz_fastAccess(int i) { return data_[i]; }
...
public:
  ...
  explicit VectorIterConst(const Vector<P_numtype>& x) : data_(x.data())
  {
    stride_ = x.stride();
    length_ = x.length();
  }
