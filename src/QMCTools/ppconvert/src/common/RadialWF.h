...
public:
  inline double operator()(double r, double u) { return WF.NormDeriv(r, u); }
...
public:
  inline Vec2 operator()(double r, Vec2& u_and_du) { return WF.PseudoDerivs(r, u_and_du); }
...
public:
  inline Vec2 operator()(double r, Vec2& u_and_du) { return WF.NormalDerivs(r, u_and_du); }
...
public:
  inline Vec2 operator()(double r, Vec2& u_and_du) { return WF.ScalarRelDerivs(r, u_and_du); }
...
inline Vec2 RadialWF::PseudoDerivs(double r, Vec2& u_and_du)
{
  ...
  derivs[1]   = 1.0 / A *
      (-dAdr * u_and_du[1] + (dAdr / r + (double)(l * (l + 1)) * B / (r * r) + 2.0 * (V - Energy)) * u_and_du[0]);
  return derivs;
}
