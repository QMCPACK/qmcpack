...
bool IOVarBase::Read(TinyVector<T, LEN>& val)
{
  if (GetFileType() == ASCII_TYPE)
  {
...
bool IOVarBase::Read(Array<T, RANK>& val,
                     ...
                     T10 s10)
{
  static const int numSlices = SliceCheck<T0>::isSlice + SliceCheck<T1>::isSlice + SliceCheck<T2>::isSlice +
      SliceCheck<T3>::isSlice + SliceCheck<T4>::isSlice + SliceCheck<T5>::isSlice + SliceCheck<T6>::isSlice +
      SliceCheck<T7>::isSlice + SliceCheck<T8>::isSlice + SliceCheck<T9>::isSlice + SliceCheck<T10>::isSlice;
...
template<typename T>
bool IOVarBase::Append(const T val)
{
  assert(GetRank() == 1);
  int n = GetExtent(0);
  Resize(n + 1);
