...
public:
  ...
  inline void Init(Grid* NewGrid,
                   const Array<double, 2>& NewYs,
  ...
  inline MultiCubicSplineCommon(Grid* NewGrid,
                                Array<double, 2> NewYs,
  ...
  inline MultiCubicSplineCommon(Grid* NewGrid, Array<double, 2> NewYs) { Init(NewGrid, NewYs); }

  inline double Params(int i, int j) const { return (y(i, j)); }
  ...
  inline double operator()(int i, int j) const { return (y(i, j)); }
...
inline void MultiCubicSplineCommon::operator()(double x, Array<double, 1>& yVec)
{
  ...
  for (int i = 0; i < NumSplines; i++)
  {
    yVec(i) = a * y(low, i) + b * y(hi, i) + (a3minusa * d2y(low, i) + b3minusb * d2y(hi, i)) * h2over6;
  }
}
...
inline double MultiCubicSplineCommon::Deriv(int i, double x)
{
  ...
  return ((y(hi, i) - y(low, i)) * hinv +
          (h * sixinv) * ((3.0 * b * b - 1.0) * d2y(hi, i) - (3.0 * a * a - 1.0) * d2y(low, i)));
}
...
inline void MultiCubicSplineCommon::Deriv(double x, Array<double, 1>& deriv)
{
  ...
  {
    deriv(i) =
        (y(hi, i) - y(low, i)) * hinv + hover6 * ((b2times3 - 1.0) * d2y(hi, i) - (a2times3 - 1.0) * d2y(low, i));
  }
}
