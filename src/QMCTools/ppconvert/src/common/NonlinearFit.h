...
public:
  ...
  inline Array<double, 2>& GetCovariance() { return AlphaInv; }

  NonlinearFitClass(ModelType model) : Model(model) { AlphaInv.resize(M, M); };
};


template<int M, typename ModelType>
double NonlinearFitClass<M, ModelType>::Chi2(const Array<double, 1>& x,
                                             const Array<double, 1>& y,
                                             const Array<double, 1>& sigma,
                                             TinyVector<double, M> params)
{
...
template<int M, typename ModelType>
void NonlinearFitClass<M, ModelType>::CalcAlphaBeta(const Array<double, 1>& x,
                                                    const Array<double, 1>& y,
                                                    const Array<double, 1>& sigma,
                                                    TinyVector<double, M> params)
{
...
template<int M, typename ModelType>
void NonlinearFitClass<M, ModelType>::Solve()
{
  for (int i = 0; i < M; i++)
  {
    dParams[i] = 0.0;
...
template<int M, typename ModelType>
void NonlinearFitClass<M, ModelType>::Fit(const Array<double, 1>& x,
                                          const Array<double, 1>& y,
                                          const Array<double, 1>& sigma,
                                          TinyVector<double, M>& params)
{
