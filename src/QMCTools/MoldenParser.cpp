...
MoldenParser::MoldenParser(int argc, char** argv) : QMCGaussianParserBase(argc, argv)
{
  basisName      = "Gaussian-G2";
  Normalized     = "no";
  usingECP       = false;
...
void MoldenParser::parse(const std::string& fname)
{
...
void MoldenParser::getGeometry(std::istream& is)
{
  //atomic numbers
...
void MoldenParser::getGaussianCenters(std::istream& is)
{
...
void MoldenParser::getMO(std::istream& is)
{
...
void MoldenParser::getMO_single_set(std::istream& is, Matrix<double>& CartMat, std::vector<value_type>& EigVal)
{
  int nq  = numMO / 4;
  int rem = numMO % 4;
  int cnt = 0;
...
void MoldenParser::getMoldenCI(std::istream& is)
{
  int Ci_indx;
  double Ci_Coeff;
