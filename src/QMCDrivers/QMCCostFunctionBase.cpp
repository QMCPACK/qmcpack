...
namespace qmcplusplus {
  ...
  QMCCostFunctionBase::QMCCostFunctionBase(MCWalkerConfiguration& w, TrialWaveFunction& psi, QMCHamiltonian& h): 
    ...
    w_en(0.0), w_var(0.0), w_abs(0.0), MinKE(-100.0),samplePsi2(true),OneOverM(1.0),
...
//  }
  ...
  bool
        ...
        Return_t&  dl, Return_t& val_proj, Return_t& lambda_max)
    {
      ...
      {
        for(int j=0; j<x0.size(); ++j) Params(j)=x0[j];
      ...
      for(int k=0; k<max_poly; ++k) A(0,k)=0.0;
      ...
      for(int i=1; i<=maxclones; ++i)
      {
        ...
        for(int k=0; k<max_poly; ++k) A(i,k)=std::pow(dxmax,k);
      ...
      if(dl<dx*0.25) lambda_max *=0.5; // narrow the bracket
      if(dl>dxmax*5.0) lambda_max *=2.0; //widen the bracket

      app_log() << "    minimum at " << dl << " estimated=" << val_proj << " LambdaMax " << lambda_max;
      for(int j=0; j<x0.size(); ++j) Params(j)=x0[j]+dl*gr[j];
      val_proj=Cost();
