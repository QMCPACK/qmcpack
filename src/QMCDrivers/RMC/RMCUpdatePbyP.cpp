...
void RMCUpdatePbyPWithDrift::advanceWalkersVMC()
{
  ...
  if (nAcceptTemp > 0)
  {
    ...
    energy_timer_.start();
    enew = H.evaluate(Psi, W);
    energy_timer_.stop();
    prophead.resetProperty(logpsi, Psi.getPhase(), enew, rr_accepted, rr_proposed, 0.0);
    prophead.Weight = 1.0;
    H.auxHevaluate(Psi, W, prophead, true, false); //evaluate properties but not collectables.
  ...
  W.update();
  H.auxHevaluate(Psi, W, centerbead); //evaluate collectables but not properties.
...
void RMCUpdatePbyPWithDrift::advanceWalkersRMC()
{
  ...
  if ((RandomGen() <= acceptProb) || (prophead.Age >= MaxAge || lastbead.Age >= MaxAge))
  {
    ...
    H.auxHevaluate(Psi, W, prophead, true, false); //evaluate properties? true.  collectables?  false.
    H.saveProperty(prophead.getPropertyBase());
    prophead.Age    = 0;
  ...
  H.auxHevaluate(Psi, W, centerbead, false, true); //evaluate properties?  false.  Collectables?  true.
}
