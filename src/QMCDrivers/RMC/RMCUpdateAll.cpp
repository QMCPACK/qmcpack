...
void RMCUpdateAllWithDrift::advanceWalkersVMC()
{
  ...
  if (RandomGen() < acceptProb)
  {
    ...
    H.auxHevaluate(W, Psi, overwriteWalker, true, false); //properties but not collectables.
...
void RMCUpdateAllWithDrift::advanceWalkersRMC()
{
  ...
  if ((RandomGen() < acceptProb) || curhead.Age >= MaxAge)
  {
    ...
    H.auxHevaluate(W, Psi, overwriteWalker, true, false); //evaluate properties but not collectables.
    H.saveProperty(overwriteWalker.getPropertyBase());
    overwriteWalker.Age = 0;
  ...
  H.auxHevaluate(W, Psi, centerbead, false, true); //collectables, but not properties
}
