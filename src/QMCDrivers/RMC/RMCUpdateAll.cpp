...
namespace qmcplusplus { 
  ...
  void RMCUpdateAllWithDrift::advanceWalkers(WalkerIter_t it, WalkerIter_t it_end, bool measure)
  {
    ...
    if (scaleDrift==true) RealType nodecorr=setScaledDriftPbyPandNodeCorr(m_tauovermass,curhead.G,drift);
	else assignDrift(m_tauovermass,curhead.G,drift);
    ...
    if (!W.makeMoveWithDrift(curhead,drift ,deltaR, m_sqrttau))
    {
	  ...
	  if (scaleDrift==true) nodecorrnew = setScaledDriftPbyPandNodeCorr(m_tauovermass,W.G,drift);
	  else assignDrift(m_tauovermass,W.G,drift);
	...
	/*	RealType dS = curhead.Properties(W.reptile->Action[2]) + W.Properties(W.reptile->Action[2])
        ...
        if ((RandomGen() < acceptProb ) || (curhead.Age>=MaxAge))
        {
          ...
          if (curhead.Age>=MaxAge) app_log()<<"\tForce Acceptance...\n";
