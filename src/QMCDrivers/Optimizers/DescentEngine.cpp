...
DescentEngine::DescentEngine(Communicate *comm, const xmlNodePtr cur)
    : my_comm_(comm), engine_target_excited_(false), num_params_(0),
      flavor_("RMSprop"), tjf_2body_eta_(.01), tjf_1body_eta_(.01),
      f_eta_(.001), gauss_eta_(.001), ci_eta_(.01), orb_eta_(.001),
      ramp_eta_(false), ramp_num_(30), store_num_(5), omega_(0.0),
      collection_step_(-1), compute_step_(-1), print_deriv_("no") {
  descent_num_ = 0;
  store_count_ = 0;
  processXML(cur);
}
...
bool DescentEngine::processXML(const xmlNodePtr cur) {
  ...
  if (collection_step_ != -1) {
    app_log() << "On descent finalization, have collect_count as true"
              << std::endl;
    collect_count_ = true;
  }
...
void DescentEngine::prepareStorage(const int num_replicas,
  ...
  if (!engine_target_excited_) {
    ...
    for (int i = 0; i < num_replicas; i++) {
      ...
      std::fill(replica_le_der_samp_[i].begin(), replica_le_der_samp_[i].end(),
                0.0);

      replica_der_rat_samp_[i].resize(num_optimizables);
      std::fill(replica_der_rat_samp_[i].begin(),
                replica_der_rat_samp_[i].end(), 0.0);
    }
  }
  // Excited state case
  else {
    ...
    for (int i = 0; i < num_replicas; i++) {
      ...
      std::fill(replica_numer_der_samp_[i].begin(),
                replica_numer_der_samp_[i].end(), 0.0);
      std::fill(replica_denom_der_samp_[i].begin(),
                replica_denom_der_samp_[i].end(), 0.0);
    }
  }
...
void DescentEngine::takeSample(
  ...
  // Excited State Case
    ...
    d = (omega_ * omega_ - static_cast<ValueType>(2) * omega_ * etmp +
         etmp * etmp) * vgs_samp;
...
void DescentEngine::sample_finish() {
  ...
  app_log() << "Energy Average: " << std::setprecision(9) << e_avg_
            << std::endl;
  app_log() << "Energy Variance: " << std::setprecision(9) << e_var_
            << std::endl;
  app_log() << "Weight Total: " << std::setprecision(9) << w_sum_ << std::endl;
  e_sd_ = std::sqrt(e_var_);
  app_log() << "Energy Standard Deviation: " << std::setprecision(9) << e_sd_
            << std::endl;
  ...
  if (!engine_target_excited_) {
    ...
    this->mpi_unbiased_ratio_of_means(numSamples, w_history_, tnv_history_,
                                      vg_history_, numer_avg_, numer_var_,
                                      numer_err_);

    this->mpi_unbiased_ratio_of_means(numSamples, w_history_, tdv_history_,
                                      vg_history_, denom_avg_, denom_var_,
                                      denom_err_);

    this->mpi_unbiased_ratio_of_means(numSamples, w_history_, tnv_history_,
                                      tdv_history_, target_avg_, target_var_,
                                      target_err_);

    app_log() << "Target Function Average: " << std::setprecision(9)
              << target_avg_ << std::endl;
    app_log() << "Target Function Variance: " << std::setprecision(9)
              << target_var_ << std::endl;
    app_log() << "Target Function Error: " << std::setprecision(9)
  ...
  for (int i = 0; i < num_optimizables; i++) {
    ...
    if (!engine_target_excited_) {
      ...
      if (print_deriv_ == "yes") {
        app_log() << "Parameter # " << i
                  ...
                  << std::endl;
        app_log() << "Parameter # " << i
                  << " Overlap term: " << avg_der_rat_samp_.at(i) << std::endl;
        app_log() << "Derivative for param # " << i << " : " << lderivs_.at(i)
                  << std::endl;
      }
    ...
    // Excited state case
      ...
      if (print_deriv_ == "yes") {
        app_log() << "Parameter # " << i
                  << " Numer Deriv: " << avg_numer_der_samp_.at(i) << std::endl;
        app_log() << "Parameter # " << i
...
void DescentEngine::mpi_unbiased_ratio_of_means(
    int numSamples, std::vector<ValueType> &weights,
    std::vector<ValueType> &numerSamples, std::vector<ValueType> &denomSamples,
    ValueType &mean, ValueType &variance, ValueType &stdErr) {
...
void DescentEngine::updateParameters() {
  ...
  app_log()
      << "Finalization Descent Num (should be zero if not on last section): "
      << final_descent_num_ << std::endl;
  if (final_descent_num_ > collection_step_ && collect_count_) {
    app_log()
        << "Should be storing history, length of history on one process is: "
        << final_lev_history_.size() << std::endl;
  }

  app_log() << "Parameter Type step sizes: "
            << " TJF_2Body_eta=" << tjf_2body_eta_
            << " TJF_1Body_eta=" << tjf_1body_eta_ << " F_eta=" << f_eta_
            << " CI_eta=" << ci_eta_ << " Orb_eta=" << orb_eta_ << std::endl;

  // Get set of derivatives for current (kth) optimization step
  std::vector<ValueType> cur_deriv_set =
      deriv_records_.at(deriv_records_.size() - 1);
  ...
  if (flavor_.compare("RMSprop") == 0) {
    ...
    ValueType cur_lambda =
        static_cast<ValueType>(.5) +
        static_cast<ValueType>(.5) *
            std::sqrt(static_cast<ValueType>(1.0) +
                      static_cast<ValueType>(4.0) * lambda_ * lambda_);
    ValueType next_lambda =
        static_cast<ValueType>(.5) +
        static_cast<ValueType>(.5) *
            std::sqrt(static_cast<ValueType>(1.0) +
                      static_cast<ValueType>(4.0) * cur_lambda * cur_lambda);
    ...
    // steepest descent
    ValueType d = 100;
    ValueType decay_factor = std::exp(-(static_cast<ValueType>(1.0) / d) *
                                      (static_cast<ValueType>(descent_num_)));
    gamma = gamma * decay_factor;

    ValueType rho = .9;

    for (int i = 0; i < num_params_; i++) {
      ValueType cur_square =
          std::pow(cur_deriv_set.at(i), static_cast<RealType>(2));
      ...
      if (derivs_squared_.size() < num_params_) {
        ...
        cur_square =
            rho * derivs_squared_.at(i) +
            (static_cast<ValueType>(1.0) - rho) *
                std::pow(cur_deriv_set.at(i), static_cast<RealType>(2));
      }
      ...
      ValueType step_decay_denom =
          static_cast<ValueType>(1.0) +
          step_lambda * static_cast<ValueType>(descent_num_);
      ...
      if (descent_num_ > 0) {
        ...
        current_params_[i] =
            (static_cast<ValueType>(1.0) - gamma) *
                (current_params_[i] - tau * cur_deriv_set[i]) +
  ...
  // random size within a range.
    ...
    for (int i = 0; i < num_params_; i++) {
      ...
      if (std::isnan(std::real(sign))) {
        app_log() << "Got a nan, choosing sign randomly with 50-50 probability"
                  << std::endl;
      ...
      app_log() << "This is random alpha: " << alpha << " with sign: " << sign
                << std::endl;
    ...
    if (flavor_.compare("ADAM") == 0) {
      ...
      for (int i = 0; i < num_params_; i++) {
        ...
        numer = beta1 * numer_records_[i] +
                (static_cast<ValueType>(1.0) - beta1) * cur_deriv_set[i];
        v = beta2 * denom_records_[i] +
            (static_cast<ValueType>(1.0) - beta2) * cur_square;

        cor_numer =
            numer / (static_cast<ValueType>(1.0) -
                     std::pow(beta1, static_cast<RealType>(descent_num_ + 1)));
        cor_v = v / (static_cast<ValueType>(1.0) -
                     std::pow(beta2, static_cast<RealType>(descent_num_ + 1)));
  ...
  if (collect_count_) {
    ...
    if (final_descent_num_ >= compute_step_) {
      ...
      ValueType final_e_sum = std::accumulate(final_le_avg_history_.begin(),
                                              final_le_avg_history_.end(),
                                              static_cast<ValueType>(0.0));
      ValueType final_e_avg = final_e_sum / collected_steps;

      ValueType final_var_sum = std::accumulate(final_var_avg_history_.begin(),
                                                final_var_avg_history_.end(),
                                                static_cast<ValueType>(0.0));
      ...
      this->computeFinalizationUncertainties(
          final_w_history_, final_lev_history_, final_vg_history_);
      ...
      if (engine_target_excited_) {
        ...
        ValueType final_tar_sum = std::accumulate(
            final_tar_avg_history_.begin(), final_tar_avg_history_.end(),
            static_cast<ValueType>(0.0));

        ValueType final_tar_avg = final_tar_sum / collected_steps;

        ValueType final_tar_var_sum = std::accumulate(
            final_tar_var_history_.begin(), final_tar_var_history_.end(),
            static_cast<ValueType>(0.0));
        ValueType final_tar_var_avg = final_tar_var_sum / collected_steps;

        app_log() << "Final average target function: " << final_tar_avg
                  << std::endl;
        app_log() << "Final target function varaince: " << final_tar_var_avg
                  << std::endl;
        this->computeFinalizationUncertainties(
            final_w_history_, final_tnv_history_, final_tdv_history_);
      }
    }
  }
}
...
DescentEngine::ValueType DescentEngine::setStepSize(int i) {
  ...
  if ((name.find("uu") != std::string::npos) ||
      (name.find("ud") != std::string::npos)) {
  ...
  // it is a 1 body Jastrow parameter.
    ...
    type_eta = f_eta_;
             ...
             name.find("CSFcoeff_") != std::string::npos) {
    type_eta = ci_eta_;
  ...
  if (ramp_eta_ && descent_num_ < ramp_num_) {
    type_eta = type_eta *
               static_cast<ValueType>((descent_num_ + 1) / (double)ramp_num_);
  }
...
// average
void DescentEngine::computeFinalizationUncertainties(
    std::vector<ValueType> &weights, std::vector<ValueType> &numerSamples,
  ...
  for (int i = 0; i < blocks; i++) {
    ...
    std::copy(wtv.begin() + i * section_len,
              wtv.begin() + (i + 1) * section_len, sub_wtv.begin());
    std::copy(nmv.begin() + i * section_len,
              nmv.begin() + (i + 1) * section_len, sub_nmv.begin());
    std::copy(dnv.begin() + i * section_len,
              dnv.begin() + (i + 1) * section_len, sub_dnv.begin());

    ValueType blockVar;
    ValueType blockMean;
    ValueType blockErr;

    this->mpi_unbiased_ratio_of_means(section_len, sub_wtv, sub_nmv, sub_dnv,
                                      blockMean, blockVar, blockErr);
  ...
  // output file.
  app_log() << "Uncertainty in variance of averaged quantity: "
            << var_uncertainty << std::endl;
  ...
  while (n > 128) {
    ...
    this->mpi_unbiased_ratio_of_means(n, wtv, nmv, dnv, currentMean, currentVar,
                                      currentErr);

    app_log() << "Blocking analysis error for per process length " << n
