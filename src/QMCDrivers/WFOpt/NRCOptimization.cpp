...
struct sign2<double>
{
  inline static double apply(double a, double b) { return (b > 0.0) ? std::abs(a) : -std::abs(a); }
};

template<>
struct sign2<float>
{
  inline static float apply(float a, float b) { return (b > 0.0) ? std::abs(a) : -std::abs(a); }
};

template<class T>
NRCOptimization<T>::NRCOptimization()
{
  ITMAX            = 100;
  ZEPS             = 1.0e-10;
  CGOLD            = 0.3819660e0;
...
template<class T>
inline int NRCOptimization<T>::CubicFormula(double a, double b, double c, double d, double& x1, double& x2, double& x3)
{
  double A = b / a;
  double B = c / a;
  double C = d / a;
...
template<class T>
inline typename NRCOptimization<T>::Return_t NRCOptimization<T>::QuarticMinimum(std::vector<Return_t>& coefs)
{
  double a, b, c, d;
  a = 4.0 * coefs[4];
  b = 3.0 * coefs[3];
  ...
  {
    double v1 = coefs[0] + coefs[1] * x1 + coefs[2] * x1 * x1 + coefs[3] * x1 * x1 * x1 + coefs[4] * x1 * x1 * x1 * x1;
    double v2 = coefs[0] + coefs[1] * x2 + coefs[2] * x2 * x2 + coefs[3] * x2 * x2 * x2 + coefs[4] * x2 * x2 * x2 * x2;
    double v3 = coefs[0] + coefs[1] * x3 + coefs[2] * x3 * x3 + coefs[3] * x3 * x3 * x3 + coefs[4] * x3 * x3 * x3 * x3;
    if (v1 < v2 && v1 < v3)
      return x1;
    if (v2 < v1 && v2 < v3)
      return x2;
...
template<class T>
bool NRCOptimization<T>::lineoptimization3(const std::function<Return_t(Return_t)>& evalCost,
                                           int points,
                                           Return_t& zeroCost)
{
...
template struct NRCOptimization<qmcplusplus::QMCTraits::RealType>;
