...
#pragma omp parallel reduction(+ : et_tot, e2_tot)
  {
    ...
    for (int iw = 0, iwg = wPerRank[ip]; iw < wRef.numSamples(); ++iw, ++iwg)
    {
      ...
      psi_ref.evaluateDeltaLogSetup(wRef, saved[LOGPSI_FIXED], saved[LOGPSI_FREE], *dLogPsi[iwg],
                                           *d2LogPsi[iwg]);
      saved[REWEIGHT] = 1.0;
      Return_rt etmp;
      if (needGrads)
      {
        ...
        etmp = hClones[ip]->evaluateValueAndDerivatives(wRef, psi_ref, OptVariablesForPsi, Dsaved, HDsaved);
        ...
        etmp = hClones[ip]->evaluate(wRef, psi_ref);
...
#pragma omp parallel reduction(+ : wgt_tot, wgt_tot2)
  {
    ...
    for (int iw = 0, iwg = wPerRank[ip]; iw < wRef.numSamples(); ++iw, ++iwg)
    {
      ...
      if (needGrad)
      {
        ...
        saved[ENERGY_NEW] =
            H_KE_Node[ip]->evaluateValueAndDerivatives(wRef, psi_ref, OptVariablesForPsi, Dsaved, HDsaved) + saved[ENERGY_FIXED];
        ...
        saved[ENERGY_NEW] = H_KE_Node[ip]->evaluate(wRef, psi_ref) + saved[ENERGY_FIXED];
