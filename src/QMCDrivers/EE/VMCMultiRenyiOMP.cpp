...
namespace qmcplusplus
  ...
  {
    resetRun();
      if (EEN>W.getActiveWalkers()-1) EEN=W.getActiveWalkers()-1;
      if(myComm->rank()==0)
      {   
        N_dat.str(""); s_dat.str(""); ee_dat.str("");
...
//         if(csEE!="no")
//         {
          for(int i(0);i<Jvalue.size();i++) csoffset += Jvalue[i];
          myComm->allreduce(csoffset);
          csoffset *= nrm;     
//         }
        for (int ip=0; ip<NumThreads; ++ip) Movers[ip]->csoffset=csoffset;
...
#pragma omp parallel
      {
        ...
        for (int i=0; i<EENR; i++) EE_indices[i].resize(nnode,W.groups()+1);
        
        ParticleSet::ParticlePos_t L(1);
        L.setUnit(PosUnit::LatticeUnit);
        for (int i=0;i<DIM;i++) L[0][i]=0;
        ...
        #pragma omp parallel
        {
          ...
          while(wit!=wit_end)
          {
            for (int iat(0); iat<(*wit)->R.size();iat++)
              for(int x(0);x<DIM;x++)
              {
                while ((*wit)->R[iat][x]<0) (*wit)->R[iat][x]+=L[0][x];
                while ((*wit)->R[iat][x]> L[0][x]) (*wit)->R[iat][x]-=L[0][x];
              }
              wit++;
          }
         }
...
//           app_log()<<endl;
  ...
  //               several volumes we can integrate out
                ...
                {
                  z=(W[i]->R[iat][0]);
                  for (int x=0;x<DIM;x++) z=std::max(z,W[i]->R[iat][x]);
                }
        ...
        for(int j(1);j<EENR;j++) for (int g=0; g<W.groups()+1;g++) for(int i(0); i<nnode; i++)
...
//         if(csEE!="no")
//         {
          for(int i(0);i<nnode;i++) csJval += Jvalue[i];
...
//         matrix is computed, now we power up and take trace
        ...
        for(int j(0);j<EENR;j++)
        {
          ...
          for(itr[ 0 ]=0;itr[ 0 ]<nnode;itr[ 0 ]++) for(itr[ 1 ]=itr[0]+1;itr[ 1 ]<nnode;itr[ 1 ]++)
          {
              ...
              if(tv[0]!=0)
              {
                ...
                estimateA[0]+=x; estimateV[ 0 ]+=abs(x); estimateN[0]+=1;
                estimateA[EEN+0]+=y; estimateV[EEN+ 0 ]+=abs(y); estimateN[EEN+0]+=1;
                estimateA[EEN2+0]+=z; estimateV[ EEN2+0 ]+=abs(z); estimateN[EEN2+0]+=1;
                estimateA[EEN3+0]+=a; estimateV[ EEN3+0 ]+=abs(a); estimateN[EEN3+0]+=1;
...
//                estimateA[EEN3+0]+=a; estimateV[ EEN3+0 ]+=abs(a); estimateS[EEN3+0]+=(a>0?1:-1); estimateN[EEN3+0]+=1;
                ...
                if(EEN > 1) godeeper(1,ratios3,estimateA,estimateV,estimateS,estimateN,tv,itr);
              }
          }
...
//combinatorial factor for each increasing power of S_N
          ...
          for(int g(0);g<W.groups()+1;g++) for(int n(0);n<10;n++) SpinN(g,n) *= 1.0/(myComm->size()*nnode);
  ...
  void VMCMultiRenyiOMP::godeeper(int lvl,vector<Matrix<RealType> >& ratios, vector<RealType>& estimateA,vector<RealType>& estimateV,vector<RealType>& estimateS,vector<RealType>& estimateN,vector<RealType>& tv,vector<int>& itr)
{
  ...
  for(itr[ itrl ]=itr[lvl]+1;itr[ itrl ]<W.getActiveWalkers();itr[ itrl ]++)
  {
      ...
      if(tv[ lvl ]!=0)
      {
        ...
        estimateA[lvl]+=x; estimateV[lvl]+=abs(x); estimateN[ lvl]+=1;
        estimateA[EEN+lvl]+=y; estimateV[EEN+lvl]+=abs(y); estimateN[EEN+ lvl]+=1;
        estimateA[2*EEN+lvl]+=z; estimateV[2*EEN+lvl]+=abs(z); estimateN[2*EEN+ lvl]+=1;
        estimateA[3*EEN+lvl]+=a; estimateV[3*EEN+lvl]+=abs(a); estimateN[3*EEN+ lvl]+=1;
        
        if(EEN > lvl) godeeper(itrl,ratios,estimateA,estimateV,estimateS,estimateN,tv,itr);
      }
  }
}
