  ...
  void QMCDriverNew::startup(xmlNodePtr cur, const QMCDriverNew::AdjustedWalkerCounts& awc)
  {
    ...
    if (qmcdriver_input_.areWalkersSerialized())
    {
      ...
      for (int i = 0; i < crowds_.size(); ++i)
        crowds_[i] = std::make_unique<Crowd>(*estimator_manager_, dispatchers_);
    }
    else
    {
      ...
      for (int i = 0; i < crowds_.size(); ++i)
        crowds_[i] =
            std::make_unique<Crowd>(*estimator_manager_, *population_.get_golden_electrons(),
 ...
 */
  ...
  QMCDriverNew::AdjustedWalkerCounts QMCDriverNew::adjustGlobalWalkerCount(int num_ranks, int rank_id,
                                                                           IndexType required_total,
                                                                           IndexType walkers_per_rank,
                                                                           RealType reserve_walkers, int num_crowds)
  {
      ...
      for (int iel = 0; iel < ref_G.size(); iel++)
      {
        ...
        if (std::abs(lap_diff) > std::abs(ref_L[iel]) * threshold)
        {
          ...
          if (std::is_same<RealType, FullPrecRealType>::value)
            success = false;
          msg << "walker[" << iw << "] lap[" << iel << "] ref = " << ref_L[iel] << " wrong = " << Ls[iw][iel]
