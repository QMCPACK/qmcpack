...
//     //set the optimization flag
  ...
  QDT::m_param.add(param_tol,"alloweddifference","double");
...
/** Clean up the vector */
template<Batching batching>
QMCLinearOptimize<batching>::~QMCLinearOptimize()
{
  delete vmcEngine;
  delete optTarget;
}
...
template<Batching batching>
void QMCLinearOptimize<batching>::add_timers(std::vector<NewTimer*>& timers)
{
...
*/
template<Batching batching>
void QMCLinearOptimize<batching>::addConfiguration(const std::string& a)
{
  if (a.size())
...
template<Batching batching>
void QMCLinearOptimize<batching>::start()
{
...
template<Batching batching>
void QMCLinearOptimize<batching>::finish()
{
  QDT::MyCounter++;
  app_log() << "  Execution time = " << std::setprecision(4) << t1.elapsed() << std::endl;
  app_log() << "  </log>" << std::endl;
...
template<Batching batching>
void QMCLinearOptimize<batching>::generateSamples()
{
  app_log() << "<optimization-report>" << std::endl;
  //if(WarmupBlocks)
  //{
...
template<Batching batching>
typename QMCDriver<batching>::RealType
QMCLinearOptimize<batching>::getLowestEigenvector(Matrix<RealType>& A, Matrix<RealType>& B, std::vector<RealType>& ev)
{
  int Nl(ev.size());
...
template<Batching batching>
typename QMCLinearOptimize<batching>::RealType QMCLinearOptimize<batching>::getLowestEigenvector(Matrix<RealType>& A, std::vector<RealType>& ev)
{
  int Nl(ev.size());
  //Tested the single eigenvalue speed and It was no faster.
  //segfault issues with single eigenvalue problem for some machines
...
template<Batching batching>
bool QMCLinearOptimize<batching>::nonLinearRescale(std::vector<RealType>& dP, Matrix<RealType>& S)
{
...
template<Batching batching>
void QMCLinearOptimize<batching>::getNonLinearRange(int& first, int& last)
{
...
template<Batching batching>
typename QMCLinearOptimize<batching>::RealType
QMCLinearOptimize<batching>::getNonLinearRescale(std::vector<RealType>& dP, Matrix<RealType>& S)
{
  int first(0),last(0);
...
template<Batching batching>
void QMCLinearOptimize<batching>::orthoScale(std::vector<RealType>& dP, Matrix<RealType>& S)
{
//     int first(0),last(0);
//     getNonLinearRange(first,last);
...
template<Batching batching>
typename QMCLinearOptimize<batching>::RealType
QMCLinearOptimize<batching>::getSplitEigenvectors(int first, int last, Matrix<RealType>& FullLeft, Matrix<RealType>& FullRight, std::vector<RealType>& FullEV, std::vector<RealType>& LocalEV, std::string CSF_Option, bool& CSF_scaled)
{
...
template<Batching batching>
bool QMCLinearOptimize<batching>::put(xmlNodePtr q)
{
...
template<Batching batching>
void QMCLinearOptimize<batching>::resetComponents(xmlNodePtr cur)
{
...
template<Batching batching>
bool QMCLinearOptimize<batching>::fitMappedStabilizers(std::vector<std::pair<RealType,RealType> >&
