    ...
    */
	...
	if(cname == "branch") {
	  ...
	  if(cname == "trial_energies") {
        ...
        for(int i=0; i<EgBufferSize; i++) c<<Eg[i]<<" ";
    ...
    void updateBranchData() {
      ...
      c.setf(std::ios::right,std::ios::adjustfield);
      c<<std::endl;
      for(int i=0; i<EgBufferSize; i++) {c<<Eg[i]<<" "; if(i%5 == 4) c << std::endl;}
