  ...
  for (int iorb = 0; iorb < Psi.getOrbitals().size(); iorb++)
  {
    ...
    if (sd)
    {
      for (int isd = 0; isd < sd->Dets.size(); isd++)
      {
        ...
        fail_log << "  Slater Determiant " << isd << " (for particles " << det->FirstIndex << " to " << det->LastIndex
...
void WaveFunctionTester::runDerivCloneTest()
{
  ...
  fout << std::endl << "   Deriv  Numeric Analytic" << std::endl;
  for (int i = 0; i < Nvars; i++)
    fout << i << "  " << PGradient[i] << "  " << Dsaved[i] << "  " << (PGradient[i] - Dsaved[i]) / PGradient[i]
         << std::endl;
  fout << std::endl << "   Hderiv  Numeric Analytic" << std::endl;
  for (int i = 0; i < Nvars; i++)
    fout << i << "  " << HGradient[i] << "  " << HDsaved[i] << "  " << (HGradient[i] - HDsaved[i]) / HGradient[i]
         << std::endl;
  for (int i = 0; i < Nvars; i++)
  {
    for (int j = 0; j < Nvars; j++)
