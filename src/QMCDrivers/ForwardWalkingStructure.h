...
namespace qmcplusplus {
  ...
  { 
    ...
    inline void copy(const Walker_t& a)
    {
      ...
      ParentID = a.ParentID; 
      Pos.resize(a.R.size()); 
      for(int i=0; i<Pos.size(); ++i) convert(a.R[i],Pos[i]);
    }
  ...
  struct ForwardWalkingHistoryObject
  {
    ...
    { 
      ...
      for(int iw=0; iw<nw; ++iw) now[iw].copy(*W[iw]);
      number_of_walkers+=nw;
    }

    inline void clearConfigsForForwardWalking()
    {
      for(int i=0; i<ForwardWalkingHistory.size();++i) delete ForwardWalkingHistory[i];
