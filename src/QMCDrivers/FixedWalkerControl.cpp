 ...
 * set SwapMode
 */
FixedWalkerControl::FixedWalkerControl() {
  SwapMode=1;
...
void FixedWalkerControl::getIndexPermutation(MCWalkerConfiguration& W) {
  ...
  RealType nwInv=1.0/static_cast<RealTyle>(nw);
  ...
  for(int iw=0; iw<nw; iw++) {
    ...
    do {
      ...
      ind++;
      ni++;
    } while(Zeta[iw]<tryp && Zeta[iw] >= wCur);
    wCur+=fabs(wConf[iw]);
    if(ni) {
      icdiff++;
    } 
...
void FixedWalkerControl::shuffleIndex(int nw) {
 ...
 */
void FixedWalkerControl::swapWalkersAsync(MCWalkerConfiguration& W) {
  NumSwaps++;
 ...
 * The communication is one-dimensional. 
 */
void FixedWalkerControl::swapWalkersBlocked(MCWalkerConfiguration& W) {
  NumSwaps++;
 ...
 * other methods.
 */
void FixedWalkerControl::swapWalkersMap(MCWalkerConfiguration& W) {
