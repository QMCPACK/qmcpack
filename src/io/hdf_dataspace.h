...
namespace qmcplusplus {
  ...
  template<typename T, unsigned D, hsize_t DS>
    struct h5_space_type<Tensor<T,D>, DS>
    {
      typedef T* pointer;
      hsize_t dims[DS+2];
      inline h5_space_type(){ dims[DS]=D; dims[DS+1]=D;}
      inline int size() const {return DS+2;}
  ...
  template<typename T, unsigned D, hsize_t DS>
    struct h5_space_type<Tensor<complex<T>,D>, DS>
    {
      typedef T* pointer;
      hsize_t dims[DS+3];
      inline h5_space_type(){ dims[DS]=D; dims[DS+1]=D;dims[DS+2]=2;}
      inline int size() const {return DS+2;}
      inline pointer get_address(Tensor<complex<T>,D>* a) { return reinterpret_cast<T*>(a->data());}
    };
}
#endif
/***************************************************************************
