...
__global__ void applyW_stageV_kernel(const int* restrict delay_list_gpu,
                        ...
                        cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  applyW_stageV_kernel<float, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(delay_list_gpu, delay_count, temp_gpu, numorbs, ndelay, V_gpu, Ainv);
}
                        ...
                        cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  applyW_stageV_kernel<cuComplex, BS><<<dimGrid, dimBlock, 0, hstream>>>(delay_list_gpu, delay_count,
                                                                         (cuComplex*)temp_gpu, numorbs, ndelay,
                                                                         (cuComplex*)V_gpu, (cuComplex*)Ainv);
}
                        ...
                        cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  applyW_stageV_kernel<double, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(delay_list_gpu, delay_count, temp_gpu, numorbs, ndelay, V_gpu, Ainv);
}
                        ...
                        cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  applyW_stageV_kernel<cuDoubleComplex, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(delay_list_gpu, delay_count, (cuDoubleComplex*)temp_gpu, numorbs, ndelay,
                                          (cuDoubleComplex*)V_gpu, (cuDoubleComplex*)Ainv);
}
...
void make_identity_matrix_cuda(const int nrows, double* mat, const int lda, cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB, NB);
  make_identity_matrix_kernel<double, BS><<<dimGrid, dimBlock, 0, hstream>>>(nrows, mat, lda);
}
...
void make_identity_matrix_cuda(const int nrows, std::complex<double>* mat, const int lda, cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB, NB);
  make_identity_matrix_kernel<cuDoubleComplex, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(nrows, (cuDoubleComplex*)mat, lda);
}
...
void extract_matrix_diagonal_cuda(const int nrows, const double* mat, const int lda, double* diag, cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  extract_matrix_diagonal_kernel<double, BS><<<dimGrid, dimBlock, 0, hstream>>>(nrows, mat, lda, diag);
}
                                  ...
                                  cudaStream_t hstream)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  extract_matrix_diagonal_kernel<cuDoubleComplex, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(nrows, (cuDoubleComplex*)mat, lda, (cuDoubleComplex*)diag);
}
...
__global__ void copy_matrix_kernel(const int nrows,
                                   const int ncols,
                                   const T_IN* restrict mat_in,
                                   const int lda,
                                   T_OUT* restrict mat_out,
                                   const int ldb)
{
  int col = threadIdx.x + blockIdx.x * BS;
  if (col < ncols)
                      ...
                      cudaStream_t hstream)
{
  ...
  copy_matrix_kernel<double, float, BS><<<dimGrid, dimBlock, 0, hstream>>>(nrows, ncols, mat_in, lda, mat_out, ldb);
}
                      ...
                      cudaStream_t hstream)
{
  ...
  copy_matrix_kernel<float, double, BS><<<dimGrid, dimBlock, 0, hstream>>>(nrows, ncols, mat_in, lda, mat_out, ldb);
}
                      ...
                      cudaStream_t hstream)
{
  ...
  copy_matrix_kernel<double, float, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(nrows, ncols * 2, (const double*)mat_in, lda * 2, (float*)mat_out, ldb * 2);
}
                      ...
                      cudaStream_t hstream)
{
  ...
  copy_matrix_kernel<float, double, BS>
      <<<dimGrid, dimBlock, 0, hstream>>>(nrows, ncols * 2, (const float*)mat_in, lda * 2, (double*)mat_out, ldb * 2);
}
