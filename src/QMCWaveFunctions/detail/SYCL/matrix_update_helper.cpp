                               ...
                               const std::vector<sycl::event>& dependencies)
{
  ...
  return aq.parallel_for(
      sycl::nd_range<1>{{static_cast<size_t>(batch_count*COLBS)}, {static_cast<size_t>(COLBS)}},
      dependencies,
      [=](sycl::nd_item<1> item) {
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int ib = 0; ib < num_blocks; ib++)
        {
          ...
          if (col_id < delay_count)
          {
            ...
            if (row_id >= 0)
              tempMat_iw[row_id * lda + col_id] += mone; //subtractOne<T>(tempMat_iw[row_id * lda + col_id]);
          }
        }
      });
}
