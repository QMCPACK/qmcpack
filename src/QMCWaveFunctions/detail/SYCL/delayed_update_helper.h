...
void applyW_stageV_kernel(const int*  delay_list_gpu, const int delay_count,
...
void applyW_stageV_sycl(const int *delay_list_gpu, const int delay_count,
                        T* temp_gpu, const int numorbs, const int ndelay,
                        T* V_gpu, const T* Ainv, sycl::queue q)
{
  const int BS = 128;
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(
        sycl::nd_range<1>(dimBlock*dimGrid, dimBlock),
        [=](sycl::nd_item<1> item_ct1) {
          applyW_stageV_kernel<T, BS>(delay_list_gpu, delay_count, temp_gpu,
                                          numorbs, ndelay, V_gpu, Ainv,
                                          item_ct1);
        });
  });
}
 ...
 */
template<typename T, int BS>
void make_identity_matrix_kernel(const int nrows, T*  mat, const int lda,
                                 sycl::nd_item<2> item_ct1)
{
  ...
  if(col<nrows)
  {
    for (int row = item_ct1.get_group(1) * BS;
         row < sycl::min((unsigned int)((item_ct1.get_group(1) + 1) * BS),
                         (unsigned int)nrows);
         row++)
      mat[row*lda+col] = T(0);
...
template<typename T>
void make_identity_matrix_sycl(const int nrows, T *mat, const int lda,
                               sycl::queue q)
{
  const int BS = 128;
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(
        ...
        [=](sycl::nd_item<2> item_ct1) {
          make_identity_matrix_kernel<T, BS>(nrows, mat, lda, item_ct1);
        });
  });
}
 ...
 */
template<typename T, int BS>
void extract_matrix_diagonal_kernel(const int nrows, const T*  mat, const int lda, T*  diag,
                                    sycl::nd_item<1> item_ct1)
{
  int col = item_ct1.get_local_id(0) + item_ct1.get_group(0) * BS;
...
template<typename T>
void extract_matrix_diagonal_sycl(const int nrows, const T *mat,
                                  const int lda, T *diag,
                                  sycl::queue q)
{
  const int BS = 128;
  const int NB = (nrows+BS-1)/BS;
...
void copy_matrix_kernel(const int nrows, const int ncols, const T_IN*  mat_in, const int lda, T_OUT*  mat_out, const int ldb,
                        sycl::nd_item<2> item_ct1)
{
  int col = item_ct1.get_local_id(0) + item_ct1.get_group(0) * BS;
  if(col<ncols)
