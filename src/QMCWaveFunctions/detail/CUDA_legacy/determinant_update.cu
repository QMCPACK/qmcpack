...
__global__ void update_inverse_cuda1(updateJob* updateList, int N, int rowstride)
{
  ...
  {
    delta[threadIdx.x] = u[block * BS + threadIdx.x] - A[k * rowstride + block * BS + threadIdx.x];
    __syncthreads();
...
template<typename T, int BS>
__global__ void update_inverse_cuda2(updateJob* updateList, int N, int rowstride)
{
  ...
  for (int block = 0; block < numblocks; block++)
  {
    Ainv_colk_shared[tid] = prefact * Ainv_colk[block * BS + threadIdx.x];
    __syncthreads();
...
void update_inverse_cuda(updateJob updateList[], float dummy, int N, int rowstride, int numWalkers)
{
  const int BS1 = 64;
  const int BS2 = 64;
  int NB1       = (N + BS1 - 1) / BS1;
  ...
  update_inverse_cuda1<float, BS1><<<dimGrid1, dimBlock1>>>(updateList, N, rowstride);
  update_inverse_cuda2<float, BS2><<<dimGrid2, dimBlock2>>>(updateList, N, rowstride);
}


void update_inverse_cuda(updateJob updateList[], double dummy, int N, int rowstride, int numWalkers)
{
  const int BS1 = 32;
  const int BS2 = 32;
  ...
  dim3 dimBlock2(BS2);
  dim3 dimGrid2(NB2, numWalkers);
  update_inverse_cuda1<double, BS1><<<dimGrid1, dimBlock1>>>(updateList, N, rowstride);
  update_inverse_cuda2<double, BS2><<<dimGrid2, dimBlock2>>>(updateList, N, rowstride);
...
*/
template<typename T, int BS>
__device__ __forceinline__ void update_inverse_core1(T* __restrict__ A,
...
*/
template<typename T, int BS>
__device__ __forceinline__ void update_inverse_core2(T* __restrict__ Ainv,
  ...
  for (int block = 0; block < numBlocks; block++)
  {
    ...
    if (col_Ainv < N)
    {
      for (int i = 0; i < min(BS, N - blockStart); i++)
      {
        ...
        Ainv[row_Ainv * rowstride + col_Ainv] += u_Ainv_shared * Ainv_colk_shared[i];
      }
    }
  }
}
...
template<typename T, int BS>
__device__ __forceinline__ void update_inverse_core2_subblock(T* __restrict__ Ainv,
                                                              const T* __restrict__ u_Ainv,
                                                              const T* __restrict__ Ainv_colk,
                                                              int k,
                                                              int N,
...
template<typename T, int BS>
__global__ void update_inverse_kernel1(T** data,
                                       int* iat,
                                       int A_off,
                                       int Ainv_off,
                                       int newRow_off,
                                       ...
                                       int rowstride)
{
  ...
  update_inverse_core1<T, BS>(A, Ainv, u, u_Ainv, Ainv_colk, k, N, rowstride);
}

template<typename T, int BS>
__global__ void update_inverse_kernel2(T** data,
                                       int* iat,
                                       int A_off,
                                       int Ainv_off,
                                       int newRow_off,
...

{
  ...
  int k              = iat[blockIdx.y];
  update_inverse_core2<T, BS>(Ainv, u_Ainv, Ainv_colk, k, N, rowstride);
}

void update_inverse_cuda(float** data,
                         int iat[],
                         int A_off,
                         int Ainv_off,
                         int newRow_off,
                         ...
                         int numWalkers)
{
  ...
  dim3 dimBlock2(BS2);
  dim3 dimGrid2(NB2, numWalkers);
  update_inverse_kernel1<float, BS1>
      <<<dimGrid1, dimBlock1>>>(data, iat, A_off, Ainv_off, newRow_off, AinvDelta_off, AinvColk_off, N, rowstride);
  update_inverse_kernel2<float, BS2>
      <<<dimGrid2, dimBlock2>>>(data, iat, A_off, Ainv_off, newRow_off, AinvDelta_off, AinvColk_off, N, rowstride);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
    fprintf(stderr, "CUDA error in update_inverse_cuda:\n  %s\n", cudaGetErrorString(err));
...
void update_inverse_cuda(double** data,
                         int iat[],
                         int A_off,
                         int Ainv_off,
                         int newRow_off,
                         ...
                         int numWalkers)
{
  ...
  dim3 dimBlock2(BS2);
  dim3 dimGrid2(NB2, numWalkers);
  update_inverse_kernel1<double, BS1>
      <<<dimGrid1, dimBlock1>>>(data, iat, A_off, Ainv_off, newRow_off, AinvDelta_off, AinvColk_off, N, rowstride);
  update_inverse_kernel2<double, BS2>
      <<<dimGrid2, dimBlock2>>>(data, iat, A_off, Ainv_off, newRow_off, AinvDelta_off, AinvColk_off, N, rowstride);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
...
void update_inverse_cuda(std::complex<float>** data,
                         int iat[],
                         int A_off,
                         int Ainv_off,
                         int newRow_off,
                         ...
                         int numWalkers)
{
  ...
  update_inverse_kernel1<thrust::complex<float>, BS1><<<dimGrid1, dimBlock1>>>((thrust::complex<float>**)data,
                                                                               iat,
                                                                               A_off,
                                                                               Ainv_off,
                                                                               newRow_off,
                                                                               ...
                                                                               N,
                                                                               rowstride);
  update_inverse_kernel2<thrust::complex<float>, BS2><<<dimGrid2, dimBlock2>>>((thrust::complex<float>**)data,
                                                                               iat,
                                                                               A_off,
                                                                               Ainv_off,
                                                                               newRow_off,
...
void update_inverse_cuda(std::complex<double>** data,
                         int iat[],
                         int A_off,
                         int Ainv_off,
                         int newRow_off,
                         ...
                         int numWalkers)
{
  ...
  update_inverse_kernel1<thrust::complex<double>, BS1><<<dimGrid1, dimBlock1>>>((thrust::complex<double>**)data,
                                                                                iat,
                                                                                A_off,
                                                                                Ainv_off,
                                                                                newRow_off,
                                                                                ...
                                                                                N,
                                                                                rowstride);
  update_inverse_kernel2<thrust::complex<double>, BS2><<<dimGrid2, dimBlock2>>>((thrust::complex<double>**)data,
                                                                                iat,
                                                                                A_off,
                                                                                Ainv_off,
                                                                                newRow_off,
...

{
  ...
  const T* u_Ainv    = sdata + AinvDelta_off; // u * Ainv
  const T* Ainv_colk = sdata + AinvColk_off;  // k-th column of orig. Ainv
  update_inverse_core2<T, BS>(A, Ainv, u, u_Ainv, Ainv_colk, k, N, rowstride);
}

template<typename T, int BS>
__global__ void update_inverse_kernel2_subblock(T** data,
                                                int k,
                                                int A_off,
                                                int Ainv_off,
                                                int newRow_off,
...

{
  ...
  const T* u_Ainv    = sdata + AinvDelta_off; // u * Ainv
  const T* Ainv_colk = sdata + AinvColk_off;  // k-th column of orig. Ainv
  update_inverse_core2_subblock<T, BS>(Ainv, u_Ainv, Ainv_colk, k, N, rowstride);
}
                         ...
                         int numWalkers)
{
  ...
  */
  ...
  update_inverse_kernel2_subblock<float, BS2>
      <<<dimGrid3, dimBlock2>>>(data, iat, A_off, Ainv_off, newRow_off, AinvDelta_off, AinvColk_off, N, rowstride);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
    fprintf(stderr, "CUDA error in update_inverse_cuda:\n  %s\n", cudaGetErrorString(err));
                         ...
                         int numWalkers)
{
  ...
  */
  dim3 dimGrid3(numWalkers, NB2, NB2);
  update_inverse_kernel2_subblock<double, BS2>
      <<<dimGrid3, dimBlock2>>>(data, iat, A_off, Ainv_off, newRow_off, AinvDelta_off, AinvColk_off, N, rowstride);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
                         ...
                         int numWalkers)
{
  ...
  */
  dim3 dimGrid3(numWalkers, NB2, NB2);
  update_inverse_kernel2_subblock<thrust::complex<float>, BS2><<<dimGrid3, dimBlock2>>>((thrust::complex<float>**)data,
                                                                                        iat,
                                                                                        A_off,
                                                                                        Ainv_off,
                                                                                        newRow_off,
                         ...
                         int numWalkers)
{
  ...
  */
  dim3 dimGrid3(numWalkers, NB2, NB2);
  update_inverse_kernel2_subblock<thrust::complex<double>, BS2>
      <<<dimGrid3, dimBlock2>>>((thrust::complex<double>**)data,
                                iat,
                                A_off,
                                Ainv_off,
                                newRow_off,
...
template<typename T, int BS>
__global__ void update_inverse_cuda1(T** A_g,
                                     T** Ainv_g,
                                     T** u_g,
                                     T** Ainv_delta_g,
                                     T** Ainv_colk_g,
...
template<typename T, int BS>
__global__ void update_inverse_cuda2(T** A_g,
                                     T** Ainv_g,
                                     T** u_g,
                                     T** Ainv_delta_g,
                                     T** Ainv_colk_g,
...
void update_inverse_cuda(float* A_g[],
                         float* Ainv_g[],
                         float* u_g[],
                         float* Ainv_delta_g[],
                         float* Ainv_colk_g[],
                         ...
                         int numWalkers)
{
  ...
  dim3 dimBlock2(BS2);
  dim3 dimGrid2(NB2, numWalkers);
  update_inverse_cuda1<float, BS1>
      <<<dimGrid1, dimBlock1>>>(A_g, Ainv_g, u_g, Ainv_delta_g, Ainv_colk_g, N, rowstride, iat);
  update_inverse_cuda2<float, BS2>
      <<<dimGrid2, dimBlock2>>>(A_g, Ainv_g, u_g, Ainv_delta_g, Ainv_colk_g, N, rowstride, iat);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
    fprintf(stderr, "CUDA error in update_inverse_cuda:\n  %s\n", cudaGetErrorString(err));
...
void update_inverse_cuda(double* A_g[],
                         double* Ainv_g[],
                         double* u_g[],
                         double* Ainv_delta_g[],
                         double* Ainv_colk_g[],
                         ...
                         int numWalkers)
{
  ...
  dim3 dimBlock2(BS2);
  dim3 dimGrid2(NB2, numWalkers);
  update_inverse_cuda1<double, BS1>
      <<<dimGrid1, dimBlock1>>>(A_g, Ainv_g, u_g, Ainv_delta_g, Ainv_colk_g, N, rowstride, iat);
  update_inverse_cuda2<double, BS2>
      <<<dimGrid2, dimBlock2>>>(A_g, Ainv_g, u_g, Ainv_delta_g, Ainv_colk_g, N, rowstride, iat);
}


template<typename T, int BS, int MAXN>
__global__ void update_inverse_transpose_cuda(T** A_g, T** AinvT_g, T** u_g, int N, int row_stride, int elec)
{
  ...
  for (int row = 0; row < N; row++)
  {
    ...
    for (int block = 0; block < numBlocks; block++)
    {
      ...
      AinvT[row * row_stride + off] = AinvT_row[off] + prefactor * sum[0] * Ainv_colk[off];
    }
    __syncthreads();
  }
}
...
template<typename T, int BS, int MAXN>
__global__ void update_inverse_transpose_cuda_2pass(T** A_g, T** AinvT_g, T** u_g, int N, int row_stride, int elec)
{
...
template<typename T, int BS>
__global__ void calc_ratios_transpose(T** AinvT_list,
                                      T** new_row_list,
                                      T* ratio_out,
                                      int N,
                                      int row_stride,
...
template<typename T, int BS>
__global__ void calc_ratios(T** Ainv_list, T** new_row_list, T* ratio, int N, int row_stride, int elec)
{
  int tid = threadIdx.x;
  ...
  if (tid == 0)
    ratio[blockIdx.x] = Ainv_new_row[0];
}


void determinant_ratios_cuda(float* Ainv_list[],
                             float* new_row_list[],
                             float* ratios,
                             int N,
                             int row_stride,
...
void determinant_ratios_cuda(double* Ainv_list[],
                             double* new_row_list[],
                             double* ratios,
                             int N,
                             int row_stride,
...
#ifdef QMC_COMPLEX
void determinant_ratios_cuda(std::complex<float>* Ainv_list[],
                             std::complex<float>* new_row_list[],
                             ...
                             int numWalkers)
{
  ...
  if (N <= 32)
    calc_ratios<thrust::complex<float>, 32><<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                                                                   (thrust::complex<float>**)new_row_list,
                                                                   (thrust::complex<float>*)ratios,
                                                                   N,
                                                                   row_stride,
                                                                   iat);
  else if (N <= 64)
    calc_ratios<thrust::complex<float>, 64><<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                                                                   (thrust::complex<float>**)new_row_list,
                                                                   (thrust::complex<float>*)ratios,
                                                                   N,
                                                                   row_stride,
    ...
    calc_ratios<thrust::complex<float>, 128><<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                                                                    (thrust::complex<float>**)new_row_list,
                                                                    (thrust::complex<float>*)ratios,
                                                                    N,
                                                                    row_stride,
    ...
    calc_ratios<thrust::complex<float>, 256><<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                                                                    (thrust::complex<float>**)new_row_list,
                                                                    (thrust::complex<float>*)ratios,
                                                                    N,
                                                                    row_stride,
    ...
    calc_ratios<thrust::complex<float>, 512><<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                                                                    (thrust::complex<float>**)new_row_list,
                                                                    (thrust::complex<float>*)ratios,
                                                                    N,
                                                                    row_stride,
    ...
    calc_ratios<thrust::complex<float>, 1024><<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                                                                     (thrust::complex<float>**)new_row_list,
                                                                     (thrust::complex<float>*)ratios,
                                                                     N,
                                                                     row_stride,
...
void determinant_ratios_cuda(std::complex<double>* Ainv_list[],
                             ...
                             int numWalkers)
{
  ...
  if (N <= 32)
    calc_ratios<thrust::complex<double>, 32><<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                                                                    (thrust::complex<double>**)new_row_list,
                                                                    (thrust::complex<double>*)ratios,
                                                                    N,
                                                                    row_stride,
                                                                    iat);
  else if (N <= 64)
    calc_ratios<thrust::complex<double>, 64><<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                                                                    (thrust::complex<double>**)new_row_list,
                                                                    (thrust::complex<double>*)ratios,
                                                                    N,
                                                                    row_stride,
    ...
    calc_ratios<thrust::complex<double>, 128><<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                                                                     (thrust::complex<double>**)new_row_list,
                                                                     (thrust::complex<double>*)ratios,
                                                                     N,
                                                                     row_stride,
    ...
    calc_ratios<thrust::complex<double>, 256><<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                                                                     (thrust::complex<double>**)new_row_list,
                                                                     (thrust::complex<double>*)ratios,
                                                                     N,
                                                                     row_stride,
    ...
    calc_ratios<thrust::complex<double>, 512><<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                                                                     (thrust::complex<double>**)new_row_list,
                                                                     (thrust::complex<double>*)ratios,
                                                                     N,
                                                                     row_stride,
...
template<typename T, int BS>
__global__ void calc_ratio_grad_lapl(T** Ainv_list,
                                     T** new_row_list,
                                     T** grad_lapl_list,
                                     T* ratio_grad_lapl,
                                     int N,
                                     ...
                                     int elec)
{
  ...
  {
    ratio_prod[BS4] -=
        (ratio_prod[BS1] * ratio_prod[BS1] + ratio_prod[BS2] * ratio_prod[BS2] + ratio_prod[BS3] * ratio_prod[BS3]);
  }
  __syncthreads();
...
void determinant_ratios_grad_lapl_cuda(float* Ainv_list[],
                                       float* new_row_list[],
                                       float* grad_lapl_list[],
                                       float ratios_grad_lapl[],
                                       int N,
                                       int row_stride,
                                       int iat,
                                       int numWalkers)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(numWalkers);
  calc_ratio_grad_lapl<float, BS><<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(Ainv_list,
                                                                               new_row_list,
                                                                               grad_lapl_list,
                                                                               ratios_grad_lapl,
                                                                               N,
...
void determinant_ratios_grad_lapl_cuda(double* Ainv_list[],
                                       double* new_row_list[],
                                       double* grad_lapl_list[],
                                       double ratios_grad_lapl[],
                                       int N,
                                       int row_stride,
                                       int iat,
                                       int numWalkers)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(numWalkers);
  calc_ratio_grad_lapl<double, BS><<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(Ainv_list,
                                                                                new_row_list,
                                                                                grad_lapl_list,
                                                                                ratios_grad_lapl,
                                                                                N,
...
#ifdef QMC_COMPLEX
void determinant_ratios_grad_lapl_cuda(std::complex<float>* Ainv_list[],
                                       std::complex<float>* new_row_list[],
                                       std::complex<float>* grad_lapl_list[],
                                       std::complex<float> ratios_grad_lapl[],
                                       int N,
                                       int row_stride,
                                       int iat,
                                       int numWalkers)
{
  ...
  calc_ratio_grad_lapl<thrust::complex<float>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>((thrust::complex<float>**)Ainv_list,
                                                    (thrust::complex<float>**)new_row_list,
                                                    (thrust::complex<float>**)grad_lapl_list,
                                                    (thrust::complex<float>*)ratios_grad_lapl,
                                                    N,
                                                    row_stride,
                                                    iat);
}
...
void determinant_ratios_grad_lapl_cuda(std::complex<double>* Ainv_list[],
                                       std::complex<double>* new_row_list[],
                                       std::complex<double>* grad_lapl_list[],
                                       std::complex<double> ratios_grad_lapl[],
                                       int N,
                                       int row_stride,
                                       int iat,
                                       int numWalkers)
{
  ...
  calc_ratio_grad_lapl<thrust::complex<double>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>((thrust::complex<double>**)Ainv_list,
                                                    (thrust::complex<double>**)new_row_list,
                                                    (thrust::complex<double>**)grad_lapl_list,
                                                    (thrust::complex<double>*)ratios_grad_lapl,
                                                    N,
                                                    row_stride,
                                                    iat);
}
...
void calc_gradient(double* Ainv_list[],
                   double* grad_lapl_list[],
                   double grad[],
                   int N,
                   int row_stride,
                   ...
                   int numWalkers)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(numWalkers);
  calc_grad_kernel<double, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(Ainv_list, grad_lapl_list, grad, N, row_stride, elec);
}

#ifdef QMC_COMPLEX
void calc_gradient(std::complex<float>* Ainv_list[],
                   std::complex<float>* grad_lapl_list[],
                   std::complex<float> grad[],
                   int N,
                   int row_stride,
                   int elec,
                   int numWalkers)
{
  ...
  calc_grad_kernel<thrust::complex<float>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>((thrust::complex<float>**)Ainv_list,
                                                    (thrust::complex<float>**)grad_lapl_list,
                                                    (thrust::complex<float>*)grad,
                                                    N,
                                                    row_stride,
                                                    elec);
}

void calc_gradient(std::complex<double>* Ainv_list[],
                   std::complex<double>* grad_lapl_list[],
                   ...
                   int numWalkers)
{
  ...
  calc_grad_kernel<thrust::complex<double>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>((thrust::complex<double>**)Ainv_list,
                                                    (thrust::complex<double>**)grad_lapl_list,
                                                    (thrust::complex<double>*)grad,
                                                    N,
                                                    row_stride,
                                                    elec);
}
...
template<typename T>
__global__ void all_ratios_kernel(T** Ainv_list, T** new_mat_list, T** ratio_list, int N, int row_stride)
{
  ...
  for (unsigned int yBlock = 0; yBlock < numBlocks; yBlock++)
  {
    ...
    for (unsigned int xBlock = 0; xBlock < numBlocks; xBlock++)
    {
      ...
      if ((xIndex < N) && (yIndex < N))
        ratio_block[threadIdx.y][threadIdx.x] += new_mat[index] * Ainv_block[threadIdx.y][threadIdx.x];
      __syncthreads();
    }
    __syncthreads();
    // Now, we have to do the reduction across the ratio_blocks
    if (threadIdx.x < 8)
      ratio_block[threadIdx.y][threadIdx.x] += ratio_block[threadIdx.y][threadIdx.x + 8];
    if (threadIdx.x < 4)
      ratio_block[threadIdx.y][threadIdx.x] += ratio_block[threadIdx.y][threadIdx.x + 4];
    if (threadIdx.x < 2)
      ratio_block[threadIdx.y][threadIdx.x] += ratio_block[threadIdx.y][threadIdx.x + 2];
    if (threadIdx.x < 1)
      ratio_block[threadIdx.y][threadIdx.x] += ratio_block[threadIdx.y][threadIdx.x + 1];
    __syncthreads();
...
void calc_all_ratios(float* Ainv_list[],
                     float* new_mat_list[],
                     float* ratio_list[],
                     int N,
                     int row_stride,
                     int num_mats)
{
  ...
  all_ratios_kernel<float><<<dimGrid, dimBlock>>>(Ainv_list, new_mat_list, ratio_list, N, row_stride);
}
...
template<typename T, int BS>
__global__ void calc_many_ratios_kernel(T** Ainv_list,
                                        T** new_row_list,
                                        T** ratio_list,
                                        int* num_ratio_list,
                                        int N,
...
// We use BS+1 to avoid bank conflicts in the writing.
  ...
  for (int block = 0; block < NB; block++)
  {
    ...
    for (int iratio = 0; iratio < num_ratios; iratio++)
      if (mask)
        ratio_sum[iratio * BS1 + tid] += Ainv_shared[tid] * new_rows[iratio * row_stride + off];
    __syncthreads();
  }
  // now, sum up ratios
  for (int iratio = 0; iratio < num_ratios; iratio++)
...
void calc_many_ratios(float* Ainv_list[],
                      float* new_row_list[],
                      float* ratio_list[],
                      int num_ratio_list[],
                      int N,
                      ...
                      int numWalkers)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(numWalkers);
  calc_many_ratios_kernel<float, BS>
      <<<dimGrid, dimBlock>>>(Ainv_list, new_row_list, ratio_list, num_ratio_list, N, row_stride, elec_list);
}

void calc_many_ratios(double* Ainv_list[],
                      double* new_row_list[],
                      double* ratio_list[],
                      int num_ratio_list[],
                      int N,
                      ...
                      int numWalkers)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(numWalkers);
  calc_many_ratios_kernel<double, BS>
      <<<dimGrid, dimBlock>>>(Ainv_list, new_row_list, ratio_list, num_ratio_list, N, row_stride, elec_list);
}


#ifdef QMC_COMPLEX
void calc_many_ratios(std::complex<float>* Ainv_list[],
                      std::complex<float>* new_row_list[],
                      std::complex<float>* ratio_list[],
                      int num_ratio_list[],
                      int N,
                      int row_stride,
                      int elec_list[],
                      int numWalkers)
{
  ...
  calc_many_ratios_kernel<thrust::complex<float>, BS><<<dimGrid, dimBlock>>>((thrust::complex<float>**)(Ainv_list),
                                                                             (thrust::complex<float>**)new_row_list,
                                                                             (thrust::complex<float>**)ratio_list,
                                                                             num_ratio_list,
                                                                             N,
                                                                             row_stride,
                                                                             elec_list);
...
void calc_many_ratios(std::complex<double>* Ainv_list[],
                      std::complex<double>* new_row_list[],
                      std::complex<double>* ratio_list[],
                      int num_ratio_list[],
                      int N,
                      int row_stride,
                      int elec_list[],
                      int numWalkers)
{
  ...
  calc_many_ratios_kernel<thrust::complex<double>, BS><<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                                                                              (thrust::complex<double>**)new_row_list,
                                                                              (thrust::complex<double>**)ratio_list,
                                                                              num_ratio_list,
                                                                              N,
                                                                              row_stride,
                                                                              elec_list);
...
template<typename T>
__global__ void scale_grad_lapl_kernel(T** grad_list, T** hess_list, T** grad_lapl_list, T* Linv, int N)
{
  ...
  if (threadIdx.x < 9)
  {
    GGt[i][j] = (G[i][0] * G[0][j] + G[i][1] * G[1][j] + G[i][2] * G[2][j]);
  }
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if (gIndex < 3 * N)
      gradBlock[i][threadIdx.x] = grad[gIndex];
  }
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    if (hIndex < 6 * N)
      hessBlock[i][threadIdx.x] = hess[hIndex];
  }
  ...
  val =
      (G[0][0] * gradBlock[0][threadIdx.x] + G[0][1] * gradBlock[1][threadIdx.x] + G[0][2] * gradBlock[2][threadIdx.x]);
  out[row + 0 * N + threadIdx.x] = val;
  // y component of gradient
  val =
      (G[1][0] * gradBlock[0][threadIdx.x] + G[1][1] * gradBlock[1][threadIdx.x] + G[1][2] * gradBlock[2][threadIdx.x]);
  out[row + 1 * N + threadIdx.x] = val;
  // z component of gradient
  val =
      (G[2][0] * gradBlock[0][threadIdx.x] + G[2][1] * gradBlock[1][threadIdx.x] + G[2][2] * gradBlock[2][threadIdx.x]);
  ...
  val                            = (GGt[0][0] * hessBlock[0][threadIdx.x] + GGt[0][1] * hessBlock[1][threadIdx.x] +
         GGt[0][2] * hessBlock[2][threadIdx.x] + GGt[1][0] * hessBlock[1][threadIdx.x] +
         GGt[1][1] * hessBlock[3][threadIdx.x] + GGt[1][2] * hessBlock[4][threadIdx.x] +
         GGt[2][0] * hessBlock[2][threadIdx.x] + GGt[2][1] * hessBlock[4][threadIdx.x] +
...
void scale_grad_lapl(float* grad_list[],
                     float* hess_list[],
                     float* grad_lapl_list[],
                     float Linv[],
                     int N,
                     int num_walkers)
{
  ...
  if (N % SCALE_BS)
    dimGrid.x++;
  scale_grad_lapl_kernel<float><<<dimGrid, dimBlock>>>(grad_list, hess_list, grad_lapl_list, Linv, N);
}


template<typename T, int BS>
__global__ void all_ratios_grad_lapl_kernel(T** Ainv_list, T** grad_lapl_list, T** out_list, int N, int row_stride)
{
  ...
  for (unsigned int yBlock = 0; yBlock < numBlocks; yBlock++)
  {
    ...
    for (unsigned int xBlock = 0; xBlock < numBlocks; xBlock++)
    {
      ...
      if ((xIndex < N) && (yIndex < N))
      {
        grad_lapl_block[0][index_local] += gl_array[index + 0 * row_stride] * Ainv_block[index_local];
        grad_lapl_block[1][index_local] += gl_array[index + 1 * row_stride] * Ainv_block[index_local];
        grad_lapl_block[2][index_local] += gl_array[index + 2 * row_stride] * Ainv_block[index_local];
        grad_lapl_block[3][index_local] += gl_array[index + 3 * row_stride] * Ainv_block[index_local];
      }
      __syncthreads();
    }
...
void calc_grad_lapl(float* Ainv_list[], float* grad_lapl_list[], float* out_list[], int N, int row_stride, int num_mats)
{
  dim3 dimBlock(RATIO_BS, RATIO_BS);
  dim3 dimGrid(num_mats);
  all_ratios_grad_lapl_kernel<float, RATIO_BS>
      <<<dimGrid, dimBlock>>>(Ainv_list, grad_lapl_list, out_list, N, row_stride);
}


void calc_grad_lapl(double* Ainv_list[],
                    double* grad_lapl_list[],
                    double* out_list[],
                    int N,
                    int row_stride,
                    int num_mats)
{
  dim3 dimBlock(RATIO_BS, RATIO_BS);
  dim3 dimGrid(num_mats);
  all_ratios_grad_lapl_kernel<double, RATIO_BS>
      <<<dimGrid, dimBlock>>>(Ainv_list, grad_lapl_list, out_list, N, row_stride);
}


#ifdef QMC_COMPLEX
void calc_grad_lapl(std::complex<float>* Ainv_list[],
                    std::complex<float>* grad_lapl_list[],
                    std::complex<float>* out_list[],
                    int N,
                    int row_stride,
                    int num_mats)
{
  ...
  all_ratios_grad_lapl_kernel<thrust::complex<float>, RATIO_BS>
      <<<dimGrid, dimBlock>>>((thrust::complex<float>**)Ainv_list,
                              (thrust::complex<float>**)grad_lapl_list,
                              (thrust::complex<float>**)out_list,
                              N,
                              row_stride);
}


void calc_grad_lapl(std::complex<double>* Ainv_list[],
                    std::complex<double>* grad_lapl_list[],
                    std::complex<double>* out_list[],
                    int N,
                    int row_stride,
                    int num_mats)
{
  ...
  all_ratios_grad_lapl_kernel<thrust::complex<double>, RATIO_BS>
      <<<dimGrid, dimBlock>>>((thrust::complex<double>**)Ainv_list,
                              (thrust::complex<double>**)grad_lapl_list,
                              (thrust::complex<double>**)out_list,
                              N,
                              row_stride);
}
#endif
...
template<typename T>
__global__ void multi_copy(T** dest, T** src, int len)
{
...
template<typename T>
__global__ void multi_copy(T** buff, int dest_off, int src_off, int len)
{
  __shared__ T *mysrc, *mydest;
  if (threadIdx.x == 0)
  {
...
void multi_copy(float* dest[], float* src[], int len, int num)
{
...
void multi_copy(double* dest[], double* src[], int len, int num)
{
  dim3 dimBlock(COPY_BS);
...
#ifdef QMC_COMPLEX
void multi_copy(std::complex<float>* dest[], std::complex<float>* src[], int len, int num)
{
  dim3 dimBlock(COPY_BS);
  dim3 dimGrid((len + COPY_BS - 1) / COPY_BS, num);

  multi_copy<thrust::complex<float>>
      <<<dimGrid, dimBlock>>>((thrust::complex<float>**)dest, (thrust::complex<float>**)src, len);
}

void multi_copy(std::complex<double>* dest[], std::complex<double>* src[], int len, int num)
{
  dim3 dimBlock(COPY_BS);
  dim3 dimGrid((len + COPY_BS - 1) / COPY_BS, num);

  multi_copy<thrust::complex<double>>
      <<<dimGrid, dimBlock>>>((thrust::complex<double>**)dest, (thrust::complex<double>**)src, len);
}
#endif


void multi_copy(float* buff[], int dest_off, int src_off, int len, int num)
{
  dim3 dimBlock(COPY_BS);
  dim3 dimGrid((len + COPY_BS - 1) / COPY_BS, num);
...
void multi_copy(double* buff[], int dest_off, int src_off, int len, int num)
{
  dim3 dimBlock(COPY_BS);
  dim3 dimGrid(len / COPY_BS, num);
  if (len % COPY_BS)
...
#ifdef QMC_COMPLEX
void multi_copy(std::complex<float>* buff[], int dest_off, int src_off, int len, int num)
{
  dim3 dimBlock(COPY_BS);
  dim3 dimGrid((len + COPY_BS - 1) / COPY_BS, num);
...
void multi_copy(std::complex<double>* buff[], int dest_off, int src_off, int len, int num)
{
  dim3 dimBlock(COPY_BS);
  dim3 dimGrid((len + COPY_BS - 1) / COPY_BS, num);
...
void test_all_grad_lapl_kernel()
{
  int N = 128;
  float *A, *A_d, *Ainv, *Ainv_d, *ratio, *ratio_d;
...
template<typename T>
__global__ void woodbury_update_16(T** Ainv_trans, T** delta, T** Ainv_delta, int N, int rowstride)
{
...
template<typename T>
__device__ inline void block_inverse_16(T A[16][17])
{
  int tid = threadIdx.x;
...
// product.
template<typename T>
__global__ void woodbury_update_16a(T** Ainv_trans,
                                    T** delta,
                                    T** Ainv_delta,
                                    T** inv_block,
                                    int N,
                                    ...
                                    int kblock)
{
  ...
  __shared__ T Ainv1[16][17], Ainv2[16][17], delta_s[4][17], Ainv_delta1[16][17], Ainv_delta2[16][17];
  int nb                    = (N + 15) / 16;
  int row                   = tid >> 4;
  int col                   = tid & 0x0f;
...
template<typename T>
__global__ void woodbury_update_16b(T** Ainv_trans,
                                    T** delta,
                                    T** Ainv_delta,
                                    T** inv_block,
                                    int N,
...
template<typename T>
__global__ void woodbury_update_32(T** Ainv_trans, T** delta, T** Ainv_delta, int N, int rowstride)
{
  T *myAinv, *mydelta, *myAinv_delta;
  int tid       = threadIdx.x;
  myAinv        = Ainv_trans[blockIdx.y];
...
void test_update_transpose()
{
  ...
  for (int i = 0; i < 1000; i++)
  {
    update_inverse_transpose_cuda<float, DET_BLOCK_SIZE, N>
        <<<dimGrid, dimBlock>>>(AList_d, AinvList_d, uList_d, N, N, row);
  ...
  for (int i = 0; i < N; i++)
    for (int j = 0; j < N; j++)
    {
      ...
      if ((i == j && fabs(ident - 1.0) > 1.0e-4) || (i != j && fabs(ident) > 1.0e-4))
        fprintf(stderr, "Error in matrix inverse, (%d, %d) = %1.8f\n", i, j, ident);
    }
  fprintf(stderr, "Finished.\n");
}
    ...
    if (mat == 0)
    {
      ...
      for (int i = 0; i < 16; i++)
        for (int j = 0; j < N; j++)
        {
          ...
          Anew[(updateBlock * 16 + i) * N + j] = Anew_inv[(updateBlock * 16 + i) * N + j] =
              A[(updateBlock * 16 + i) * N + j] + delta_h[i * N + j];
        }
      // for (int i=0; i<N; i++)
