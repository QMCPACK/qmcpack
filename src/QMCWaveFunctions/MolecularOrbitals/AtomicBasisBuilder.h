...
struct AtomicBasisBuilder : public SPOSetBuilder
{
  ...
  int expandYlm(const std::string& rnl,
  ...
  int expandYlmH5(const std::string& rnl,
                  const QuantumNumberType& nlms,
                  int num,
                  COT* aos,
                  hdf_archive& hin,
...
bool AtomicBasisBuilder<RFB>::putH5(hdf_archive& hin)
{
  ...
  app_log() << "<input node=\"atomicBasisSet\" name=\"" << basisName << "\" Morder=\"" << Morder << "\" angular=\""
            << sph << "\"  elementType=\"" << CenterID << "\"  normalized=\"" << Normalized << "\"/>" << std::endl;
  bool tmp_addsignforM = addsignforM;
  if (sph == "spherical")
    addsignforM = 1; //include (-1)^m
  if (Morder == "gaussian")
...
template<class RFB>
typename AtomicBasisBuilder<RFB>::COT* AtomicBasisBuilder<RFB>::createAOSet(xmlNodePtr cur)
{
  ...
  while (cur1 != NULL)
  {
    ...
    if (cname1 == "basisGroup")
    {
      ...
      std::istringstream stream(XMLAttrString{cur1, "l"});
      int l;
      stream >> l;
      Lmax  = std::max(Lmax, l);
...
template<class RFB>
typename AtomicBasisBuilder<RFB>::COT* AtomicBasisBuilder<RFB>::createAOSetH5(hdf_archive& hin)
{
...
template<class RFB>
int AtomicBasisBuilder<RFB>::expandYlm(const std::string& rnl,
                                       const QuantumNumberType& nlms,
                                       int num,
                                       COT* aos,
                                       xmlNodePtr cur1,
...
template<class RFB>
int AtomicBasisBuilder<RFB>::expandYlmH5(const std::string& rnl,
                                         const QuantumNumberType& nlms,
                                         int num,
                                         COT* aos,
                                         hdf_archive& hin,
                                         int expandlm)
{
  ...
  else
  {
    ...
    // //increment number of basis functions
    // num++;
    APP_ABORT(" Error: expandYlm='pyscf'  with angular='spherical' And HDF5 not implemented in AOS version of the "
              "code. Aborting.\n");
  }
  return num;
}
