...
template<class BS>
void LCOrbitalSetWithCorrection<BS, false>::createLCOSets(int centr,
...
bool LCOrbitalSetWithCorrection<BS, false>::transformSPOSet()
{
  ...
  // parameter is not new
  {
    for (int iat = 0; iat < Z.size(); iat++)
    {
      if (tspecies(cctag, sourcePtcl->GroupID[iat]) != 0)
      {
        ...
        app_log() << "Not using cusp correction algorithm in atoms of type: "
                  << tspecies.speciesName[sourcePtcl->GroupID[iat]] << std::endl;
      }
    }
  }
  if (Rcut < 0)
  ...
  for (int i = 0; i < rmv.size(); i++)
    if (!rmv[i])
      norb++;
  app_log() << "Found " << rmv.size() - norb << " spherically symmetric basis functions and " << norb
            << " non symmetric ones. \n";
  /*
