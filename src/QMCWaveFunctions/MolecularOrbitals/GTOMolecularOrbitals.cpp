...
{
GTOMolecularOrbitals::GTOMolecularOrbitals(ParticleSet& els, TrialWaveFunction& psi, ParticleSet& ions)
...
GTOMolecularOrbitals::BasisSetType* GTOMolecularOrbitals::addBasisSet(xmlNodePtr cur)
{
  if (!BasisSet)
  ...
  if (BasisSet)
  {
    ...
    XMLReport("The total number of basis functions " << BasisSet->TotalBasis) return BasisSet;
  }
  else
  {
...
int GTOMolecularOrbitals::expandYlm(const std::string& rnl,
                                    const QuantumNumberType& nlms,
                                    int num,
                                    CenteredOrbitalType* aos,
                                    xmlNodePtr cur1,
                                    int expandlm)
{
  if (Normalized)
  ...
  if (expandlm == GAUSSIAN_EXPAND)
  {
    ...
    if (rnl_it == RnlID.end())
    {
      ...
      XMLReport("Adding a Radial Orbital and Expanding Ylm according to Gaussian98.")
          XMLReport("Adding " << 2 * l + 1 << " spherical orbitals for l= " << l) switch (l)
      {
      case (0):
  ...
  else if (expandlm == NATURAL_EXPAND)
  {
    ...
    {
      XMLReport("Expanding Ylm as -l,-l+1,...,l-1,l") int nl = aos->Rnl.size();
      int l                                                  = nlms[q_l];
      RadialOrbitalType* sto                                 = new RadialOrbitalType(l, Normalized);
      sto->putBasisGroup(cur1);
      aos->Rnl.push_back(sto);
      aos->RnlID.push_back(nlms);
      RnlID[rnl] = nl;
      XMLReport("Adding " << 2 * l + 1 << " spherical orbitals") for (int tm = -l; tm <= l; tm++, num++)
      {
        aos->LM[num] = aos->Ylm.index(l, tm);
        aos->NL[num] = nl;
      }
  ...
  {
    XMLReport("Ylm is NOT expanded.") aos->LM[num] = aos->Ylm.index(nlms[q_l], nlms[q_m]);
    std::map<std::string, int>::iterator rnl_it    = RnlID.find(rnl);
    if (rnl_it == RnlID.end())
    {
      int nl                 = aos->Rnl.size();
