...
namespace qmcplusplus
{
  ...
  class GaussianFunctor: public OptimizableFunctorBase
  {
    ...
    {
       if(IsOptimizing) myVars.getIndex(active);
    }

    void checkInVariables(opt_variables_type& active)
    {
       if(IsOptimizing) active.insertFrom(myVars);
    }
   ...
   */
    void resetParameters(const opt_variables_type& active)
    {
        ...
        for(int i=0; i<NumFuns; i++) { 
          if (activeOptm[n]) {int ia=myVars.where(m); if(ia>-1) R0[i]=myVars[m++]=active[ia];}
          n++;
          if (activeOptm[n]) {int ia=myVars.where(m); if(ia>-1) alp[i]=myVars[m++]=active[ia];}
          n++;
          if (activeOptm[n]) {int ia=myVars.where(m); if(ia>-1) expo[i]=myVars[m++]=active[ia];}
          n++;
        }
    }
...
//         activeOptm.push_back(optm=="all" || optm=="yes" || optm=="1" || optm == "4" || optm =="5"); 
         ...
         if(activeOptm[activeOptm.size()-3]) IsOptimizing=true;
         if(activeOptm[activeOptm.size()-2]) IsOptimizing=true;
         if(activeOptm[activeOptm.size()-1]) IsOptimizing=true;
         ...
         if(activeOptm[activeOptm.size()-3]) myVars.insert(R0_id.back(),R0.back(),true);
         if(activeOptm[activeOptm.size()-2]) myVars.insert(alp_id.back(),alp.back(),true);
         if(activeOptm[activeOptm.size()-1]) myVars.insert(expo_id.back(),expo.back(),true);
       }
       cur = cur->next;
     }
     return true;
