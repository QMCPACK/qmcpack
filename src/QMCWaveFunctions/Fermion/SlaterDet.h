...
public:
  ...
  /////////////////////////////////////////////////////
  void GPU_XRAY_TRACE recompute(MCWalkerConfiguration &W, bool firstTime)
  {
    for (int id=0; id<Dets.size(); id++)
      Dets[id]->recompute(W, firstTime);
  }

  void GPU_XRAY_TRACE reserve (PointerPool<gpu::device_vector<CudaValueType> > &pool)
  {
    for (int id=0; id<Dets.size(); id++)
      Dets[id]->reserve(pool);
  }

  void GPU_XRAY_TRACE addLog (MCWalkerConfiguration &W, std::vector<RealType> &logPsi)
  {
    for (int id=0; id<Dets.size(); id++)
  ...
  void
  GPU_XRAY_TRACE ratio (MCWalkerConfiguration &W, int iat
  ...
  void
  GPU_XRAY_TRACE calcRatio (MCWalkerConfiguration &W, int iat
             , std::vector<ValueType> &psi_ratios,std::vector<GradType>  &grad, std::vector<ValueType> &lapl)
  {
  ...
  void
  GPU_XRAY_TRACE addRatio (MCWalkerConfiguration &W, int iat
            , std::vector<ValueType> &psi_ratios,std::vector<GradType>  &grad, std::vector<ValueType> &lapl)
  {
    Dets[getDetID(iat)]->addRatio(W, iat, psi_ratios, grad, lapl);
  }

  void GPU_XRAY_TRACE ratio (std::vector<Walker_t*> &walkers,    std::vector<int> &iatList,
              std::vector<PosType> &rNew, std::vector<ValueType> &psi_ratios,
              std::vector<GradType>  &grad, std::vector<ValueType> &lapl);

  void GPU_XRAY_TRACE calcGradient(MCWalkerConfiguration &W, int iat, std::vector<GradType> &grad)
  {
    Dets[getDetID(iat)]->calcGradient(W, iat, grad);
  }

  void GPU_XRAY_TRACE addGradient(MCWalkerConfiguration &W, int iat, std::vector<GradType> &grad)
  {
    Dets[getDetID(iat)]->addGradient(W, iat, grad);
  }

  void GPU_XRAY_TRACE update (std::vector<Walker_t*> &walkers, int iat)
  {
    Dets[getDetID(iat)]->update(walkers, iat);
  }

  void GPU_XRAY_TRACE update (const std::vector<Walker_t*> &walkers, const std::vector<int> &iatList) GPU_XRAY_TRACE;
  ...
  void GPU_XRAY_TRACE NLratios (MCWalkerConfiguration &W,  std::vector<NLjob> &jobList
