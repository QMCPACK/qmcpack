...
// myTable->Temp[jat].r1
    ...
    evaluatePbyP(const ParticleSet& P, int iat, ParticleSet::ParticlePos_t& newQP)
    {
      ...
      for(int i=0; i<iat; i++) {
        ...
        GradType u = (UIJ_temp(i)=uij*myTable->Temp[i].dr1)-UIJ(iat,i);
        newQP[iat] += u;
        newQP[i] -= u;
      }
...
// myTable->Temp[jat].r1
      ...
      for(int i=1; i<maxI; i++) {
        ...
        GradType u = (UIJ_temp(j)=uij*TMP[j].dr1)-UIJ(iat,j);
        newQP[iat] += u;
        newQP[j] -= u;
...
//           std::fill(derivs.begin(),derivs.end(),0.0);
          ...
          //TensorType drXdr(outerProduct(myTable->dr(nn),myTable->dr(nn))); 
          //convert(drXdr,op);
#else
