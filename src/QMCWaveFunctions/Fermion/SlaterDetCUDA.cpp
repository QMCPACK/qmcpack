...
void SlaterDet::ratio(std::vector<Walker_t*>& walkers,
                      ...
                      std::vector<ValueType>& lapl)
{
  ...
  for (int idet = 0; idet < Dets.size(); idet++)
  {
    ...
    if (sorted_walkers[idet].size())
      Dets[idet]->ratio(sorted_walkers[idet],
                        sorted_iatList[idet],
                        sorted_rNew[idet],
                        ratio_det[idet],
                        grad_det[idet],
...
void SlaterDet::update(const std::vector<Walker_t*>& walkers, const std::vector<int>& iatList)
{
  // Sort walkers by determinant number
  std::vector<std::vector<Walker_t*>> sorted_walkers(Dets.size());
  std::vector<std::vector<int>> sorted_iatList(Dets.size());
