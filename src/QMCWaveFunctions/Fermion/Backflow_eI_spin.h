    ...
    class Backflow_eI_spin: public BackflowFunctionBase
  {
    ...
    public:
        ...
        : BackflowFunctionBase(ions,els), Spin(false)
      {
        ...
        for(int i=0; i<RadFunc.size(); ++i) RadFunc(i)=0;
        ...
        for(int s=0;s<ions.groups(); ++s) s_offset[s+1]=ions.last(s);
        for(int t=0;t<els.groups(); ++t) t_offset[t+1]=els.last(t);
      ...
      ~Backflow_eI_spin() 
      {
        for(int i=0; i<RadFunc.size(); ++i) if(RadFunc[i]) delete RadFunc[i];
      }
        ...
        if(Spin)
        {
          for(int sg=0; sg<RadFunc.rows(); ++sg)
            for(int tg=0; tg<RadFunc.rows(); ++tg)
              if(RadFunc(sg,tg)) clone->addFunc(sg,new FT(*RadFunc(sg,tg)),tg);
        }
        else
        {
          for(int sg=0; sg<RadFunc.rows(); ++sg)
            if(RadFunc(sg,0)) clone->addFunc(sg,new FT(*RadFunc(sg,0)),-1);
        }
        return clone; 
      }
      ...
      void resetParameters(const opt_variables_type& active)
      {
        for(int i=0; i<RadFunc.size(); ++i)
          if(Fmask(i) == i) RadFunc(i)->resetParameters(active);
      }
      ...
      void checkInVariables(opt_variables_type& active)
      {
        //myVars.clear();
        for(int i=0; i<RadFunc.size(); ++i)
          if(Fmask(i) == i) RadFunc(i)->checkInVariables(active);
      }

      void checkOutVariables(const opt_variables_type& active)
      {
        for(int i=0; i<RadFunc.size(); ++i)
          if(Fmask(i) == i) RadFunc(i)->checkOutVariables(active);
      }

      inline bool isOptimizable()
      {
        for(int i=0; i<RadFunc.size(); ++i)
          if(Fmask(i) == i && RadFunc(i)->isOptimizable()) return true;
        return false;
      }
      ...
      inline int
        ...
        {
          for(int i=0; i<RadFunc.size(); ++i)
            if(Fmask(i) == i) return RadFunc(i)->myVars.where(0);
          return -1;
        }
