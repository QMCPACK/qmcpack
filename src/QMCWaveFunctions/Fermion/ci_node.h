...
namespace qmcplusplus
{
  ...
  struct ci_node
  {
    ...
    void build_tree(std::vector<configuration>& confg, configuration& baseC, std::vector<int>& map2det)
    {
      ...
      int m=inverse.cols();
      int n=dpsiM.cols();
      for(vector<configuration>::iterator it=confg.begin()+1; it!=confg.end(); ++it) it->taken=false;
...
// to avoid using incremented versions of dot?
      ...
      T utv=0.0;
      for(int i=0; i<m; i++) utv += inv0(from,i)*psi(i,to);
      ratios[my_id]=ratio_base*utv;
      if(children.size())
      {
        ratio_base=ratios[my_id];
        ...
        for(int iv=0; iv<peers.size(); ++iv)
        {
          ...
          T gamma=0.0;
          for(int i=0; i<m; i++) gamma-=inv0(vv,i)*psi(i,to);
          gamma*=inv_utv; 
          for(int i=0; i<m; i++) inverse(vv,i) += gamma*inv0(from,i);
        }
        ...
        , T ratio_base)
    {
      ...
      if(children.size())
      {
        ...
        {
          T gamma=0.0;
          for(int i=0; i<m; i++) gamma-=inv0(k,i)*psi(i,to);
          gamma*=inv_utv;
      //    if(iv<m && k==peers[iv]) {
      //      iv++;
            for(int i=0; i<m; i++) inverse(k,i) += gamma*inv0(from,i);
      //    } else {
      //      inverse(k,iat)+= gamma*inv0(from,iat);
      //    }
        }
      //  inverse(from,iat)=inv0(from,iat)*inv_utv;
        for(int i=0; i<m; i++) inverse(from,i)=inv0(from,i)*inv_utv;
      ...
      //    }
        ...
        grads(my_id,iat)=0.0;
        lapls(my_id,iat)=0.0;
        for(int j=0; j<m; j++) grads(my_id,iat)+=inverse(j,iat)*dpsiV[j];
        for(int j=0; j<m; j++) lapls(my_id,iat)+=inverse(j,iat)*d2psiV[j];
        ...
        , T ratio_base)
    {
      ...
      for(int i=0; i<m; i++) {
        ...
        for(int j=0; j<m; j++) grads(my_id,i)+=inverse(j,i)*dpsiM(i,j);
        for(int j=0; j<m; j++) lapls(my_id,i)+=inverse(j,i)*d2psiM(i,j);
      }
      if(children.size())
      {
        ratio_base=ratios[my_id];
        ...
        , bool replace_row=false)
    {
      ...
      if(replace_row) {
        ...
        for(int i=0; i<children.size();++i) children[i]->debugRatiosByColPromotion(psi0,psi,dets);
    }
        ...
        , bool replace_row=false)
    {
      ...
      if(replace_row) {
        ...
        for(int i=0; i<children.size();++i) children[i]->debugRatiosByColPromotion(psi0,psi,dpsi,d2psi,ratios,grads,lapls);
    }
...
*/
    ...
    void debugRatiosByColPromotion(const ValueMatrix& psi0, const ValueMatrix& psic, ValueVector& dets)
    {
      ...
      {
        if(myC.occup[i]) { 
          for(int j=0; j<m;++j) inverse(j,nq)=psic(j,i);
          nq++;
        }
      }
        ...
        , ValueMatrix& lapls)
    {
      ...
      {
        if(myC.occup[i]) {
          for(int j=0; j<m;++j) inverse(j,nq)=psi(j,i);
          for(int j=0; j<m;++j) dpsiM(j,nq)=dpsi(j,i);
          for(int j=0; j<m;++j) d2psiM(j,nq)=d2psi(j,i);
          nq++;
        }
      }
        ...
        if(children.size())
        {
          ...
          for(int i=0; i<peers.size(); ++i) {
            ...
            for(int j=0; j<p_peers.size(); ++j)
              if(p_peers[j]==peers[i]) found=true;
            if(!found) p_peers.push_back(peers[i]);
          }
        }
      }
