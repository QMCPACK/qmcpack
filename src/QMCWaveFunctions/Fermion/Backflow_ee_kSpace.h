...
namespace qmcplusplus
{
  class Backflow_ee_kSpace: public BackflowFunctionBase
  {
    ...
    void initialize(ParticleSet&P, vector<RealType>& yk)
    {
      ...
      if(Optimize) numParams = NumKShells;
    }
...
*/
    ...
    void resetParameters(const opt_variables_type& active)
    {
      if(Optimize) { 
        for (int i=0; i<Fk.size(); ++i)
        {
          ...
          if (loc>=0) Fk[i]=myVars[i]=active[loc];
        }
      }
    }
    ...
    {
      if(Optimize) active.insertFrom(myVars); 
    }

    void checkOutVariables(const opt_variables_type& active)
    {
      if(Optimize) myVars.getIndex(active); 
    }
    ...
    indexOffset()
    {
       if(Optimize) return myVars.where(0);
       else return 0;
    }
