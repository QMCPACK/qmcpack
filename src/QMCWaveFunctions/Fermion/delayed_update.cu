                 ...
                 int k, int N, int nw, int RowStride)
{
  ...
  cublasSgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, 1, N, k,
                                         &one,
                 ...
                 int k, int N, int nw, int RowStride)
{
  ...
  // A^-1 - { A^-1 * dU  * Lemma^-1 } * { V' * A^(-1) }
  // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
  cublasDgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, 1, N, k,
                                         &one,
                   ...
                   int k, int kd, int M, int N, int nw, int RowStride)
{
  ...
  if(M==1)
  {
    ...
    // z * U = -A^-1 *dU
    cublasStrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           M, k, &one, (const float**) lemma_lu, kd,
                                           AWorkList_d, pitch, nw);
    // y * L = z => y = {-A^-1 * dU } * Lemma^(-1)
    cublasStrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           M, k, &one, (const float**) lemma_lu, kd,
                                           AWorkList_d, pitch, nw);
    // A^-1 + { -A^-1 * dU *  Lemma^-1 } * { V' * A^(-1) }
    // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
  ...
  } else
  {
    ...
    // L * z = V' * A^(-1)
    cublasStrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           k, N, &one, (const float**) lemma_lu, kd,
                                           AWorkList_d, k, nw);
    // U * y = z => y = Lemma^(-1) * V' * A^(-1)
    cublasStrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           k, N, &one, (const float**) lemma_lu, kd,
                                           AWorkList_d, k, nw);
    // A^-1 + { -A^-1 * dU } * { Lemma^-1 * V' * A^(-1) }
  ...
  if(M==1) // row update can use different order to save flops
  {
    ...
    cublasSgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, k, k,
                                           &one,
                                           (const float**)AinvUList_d, RowStride,
  ...
  } else
  {
    ...
    cublasSgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, k, N, k,
                                           &one,
                   ...
                   int k, int kd, int M, int N, int nw, int RowStride)
{
  ...
  if(M==1)
  {
    // {-A^-1 * dU } * Lemma^(-1) => solve for y: Lemma y * (L * U) = (y * L) * U = -A^-1 * dU
    // z * U = -A^-1 *dU
    cublasDtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           M, k, &one, (const double**) lemma_lu, kd,
                                           AWorkList_d, pitch, nw);
    // y * L = z => y = {-A^-1 * dU } * Lemma^(-1)
    cublasDtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           M, k, &one, (const double**) lemma_lu, kd,
                                           AWorkList_d, pitch, nw);
    // A^-1 + { -A^-1 * dU *  Lemma^-1 } * { V' * A^(-1) }
    // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
    cublasDgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, k,
                                           &one,
  ...
  } else
  {
    // Lemma^(-1) * V' * A^(-1) => solve for y: Lemma (L * U) * y = L * (U * y) = V' * A^(-1)
    // L * z = V' * A^(-1)
    cublasDtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           k, N, &one, (const double**) lemma_lu, kd,
                                           AWorkList_d, k, nw);
    // U * y = z => y = Lemma^(-1) * V' * A^(-1)
    cublasDtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           k, N, &one, (const double**) lemma_lu, kd,
                                           AWorkList_d, k, nw);
    // A^-1 + { -A^-1 * dU } * { Lemma^-1 * V' * A^(-1) }
    // per walker: [M x N] - [M x k] * [k x N] = [M x N]
  ...
  if(M==1) // row update can use different order to save flops
  {
    // { -A^-1 * dU } * Lemma^-1
    // per walker: [M x k] * [k x k] = [M x k]
    cublasDgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, k, k,
                                           &one,
                                           (const double**)AinvUList_d, RowStride,
  ...
  } else
  {
    ...
    // Lemma^-1 * V' * A^(-1)
    // per walker: [k x k] * [k x N] = [k x N]
    cublasDgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, k, N, k,
                                           &one,
                 ...
                 int k, int N, int nw, int RowStride)
{
  ...
  // A^-1 - { A^-1 * dU  * Lemma^-1 } * { V' * A^(-1) }
  // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
  cublasCgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, 1, N, k,
                                         &one,
                   ...
                   int k, int kd, int M, int N, int nw, int RowStride)
{
  ...
  if(M==1)
  {
    // {-A^-1 * dU } * Lemma^(-1) => solve for y: Lemma y * (L * U) = (y * L) * U = -A^-1 * dU
    // z * U = -A^-1 *dU
    cublasCtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           M, k, &one, (const cuComplex**) lemma_lu, kd,
                                           (cuComplex**)AWorkList_d, pitch, nw);
    // y * L = z => y = {-A^-1 * dU } * Lemma^(-1)
    cublasCtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           M, k, &one, (const cuComplex**) lemma_lu, kd,
                                           (cuComplex**)AWorkList_d, pitch, nw);
    // A^-1 + { -A^-1 * dU *  Lemma^-1 } * { V' * A^(-1) }
    // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
    cublasCgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, k,
                                           &one,
  ...
  } else
  {
    // Lemma^(-1) * V' * A^(-1) => solve for y: Lemma (L * U) * y = L * (U * y) = V' * A^(-1)
    // L * z = V' * A^(-1)
    cublasCtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           k, N, &one, (const cuComplex**) lemma_lu, kd,
                                           (cuComplex**)AWorkList_d, k, nw);
    // U * y = z => y = Lemma^(-1) * V' * A^(-1)
    cublasCtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           k, N, &one, (const cuComplex**) lemma_lu, kd,
                                           (cuComplex**)AWorkList_d, k, nw);
    // A^-1 + { -A^-1 * dU } * { Lemma^-1 * V' * A^(-1) }
    // per walker: [M x N] - [M x k] * [k x N] = [M x N]
  ...
  if(M==1) // row update can use different order to save flops
  {
    // { -A^-1 * dU } * Lemma^-1
    // per walker: [M x k] * [k x k] = [M x k]
    cublasCgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, k, k,
                                           &one,
                                           (const cuComplex**)AinvUList_d, RowStride,
  ...
  } else
  {
    ...
    // Lemma^-1 * V' * A^(-1)
    // per walker: [k x k] * [k x N] = [k x N]
    cublasCgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, k, N, k,
                                           &one,
...
void
cublas_ainv_row (cublasHandle_t handle,
                 std::complex<double> *AinvkList_d[], std::complex<double> *AWorkList_d[], std::complex<double> *AinvList_d[],
                 int k, int N, int nw, int RowStride)
{
  cuDoubleComplex one = make_cuDoubleComplex(1.0f,0.0f);
  // A^-1 - { A^-1 * dU  * Lemma^-1 } * { V' * A^(-1) }
  // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
  cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, 1, N, k,
                                         &one,
                                         (const cuDoubleComplex**)AWorkList_d, 1,
                                         (const cuDoubleComplex**)AinvkList_d, RowStride, &one,
                                         (cuDoubleComplex**)AinvList_d, 1,
                                         nw);
}
                   ...
                   int k, int kd, int M, int N, int nw, int RowStride)
{
  ...
  if(M==1)
  {
    // {-A^-1 * dU } * Lemma^(-1) => solve for y: Lemma y * (L * U) = (y * L) * U = -A^-1 * dU
    // z * U = -A^-1 *dU
    cublasZtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           M, k, &one, (const cuDoubleComplex**) lemma_lu, kd,
                                           (cuDoubleComplex**)AWorkList_d, pitch, nw);
    // y * L = z => y = {-A^-1 * dU } * Lemma^(-1)
    cublasZtrsmBatched( handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           M, k, &one, (const cuDoubleComplex**) lemma_lu, kd,
                                           (cuDoubleComplex**)AWorkList_d, pitch, nw);
    // A^-1 + { -A^-1 * dU *  Lemma^-1 } * { V' * A^(-1) }
    // per walker: [1 x N] - [1 x k] * [k x N] = [1 x N]
    cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, k,
                                           &one,
                                           (const cuDoubleComplex**)AWorkList_d, M,
                                           (const cuDoubleComplex**)AinvkList_d, RowStride, &one,
                                           (cuDoubleComplex**)AinvList_d, pitch,
                                           nw);
  } else
  {
    // Lemma^(-1) * V' * A^(-1) => solve for y: Lemma (L * U) * y = L * (U * y) = V' * A^(-1)
    // L * z = V' * A^(-1)
    cublasZtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT,
                                           k, N, &one, (const cuDoubleComplex**) lemma_lu, kd,
                                           (cuDoubleComplex**)AWorkList_d, k, nw);
    // U * y = z => y = Lemma^(-1) * V' * A^(-1)
    cublasZtrsmBatched( handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                           k, N, &one, (const cuDoubleComplex**) lemma_lu, kd,
                                           (cuDoubleComplex**)AWorkList_d, k, nw);
    // A^-1 + { -A^-1 * dU } * { Lemma^-1 * V' * A^(-1) }
    // per walker: [M x N] - [M x k] * [k x N] = [M x N]
    cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, k,
                                           &one,
                                           (const cuDoubleComplex**)AinvUList_d, RowStride,
                                           (const cuDoubleComplex**)AWorkList_d, k, &one,
                                           (cuDoubleComplex**)AinvList_d, pitch,
                                           nw);
  }
#else
  ...
  if(M==1) // row update can use different order to save flops
  {
    // { -A^-1 * dU } * Lemma^-1
    // per walker: [M x k] * [k x k] = [M x k]
    cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, k, k,
                                           &one,
                                           (const cuDoubleComplex**)AinvUList_d, RowStride,
                                           (const cuDoubleComplex**)lemma_inv, k, &zero,
                                           (cuDoubleComplex**)AWorkList_d, M,
                                           nw);
    // A^-1 - { A^-1 * dU  * Lemma^-1 } * { V' * A^(-1) }
    // per walker: [M x N] - [M x k] * [k x N] = [M x N]
    cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, k,
  ...
  } else
  {
    ...
    // Lemma^-1 * V' * A^(-1)
    // per walker: [k x k] * [k x N] = [k x N]
    cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, k, N, k,
                                           &one,
                                           (const cuDoubleComplex**)lemma_inv, k,
                                           (const cuDoubleComplex**)AinvkList_d, RowStride, &zero,
                                           (cuDoubleComplex**)AWorkList_d, k,
                                           nw);
    // A^-1 + { -A^-1 * dU } * { Lemma^-1 * V' * A^(-1) }
    // per walker: [M x N] - [M x k] * [k x N] = [M x N]
    cublasZgemmBatched( handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, k,
