...
__global__ void phase_factor_kernel(T* kPoints,
                                    int* makeTwoCopies,
                                    T* pos,
                                    T** phi_in,
                                    T** phi_out,
...
{
  __shared__ T in_shared[2 * BS + 1], kPoints_s[BS][3], pos_s[BS][3];
...
#pragma unroll
  ...
  for (int block = 0; block < nb; block++)
  {
    ...
    int in_off = off / 2;
    int my_off =
        2 * ((in_off % num_split_splines) + num_walkers * num_split_splines * (in_off / num_split_splines)) + (off % 2);
    int off_end = off + end;
    in_off      = off_end / 2;
    int my_off_end =
        2 * ((in_off % num_split_splines) + num_walkers * num_split_splines * (in_off / num_split_splines)) +
        (off_end % 2);
    if (tid < end)
      for (int i = 0; i < numWrite; i++)
      {
        ...
        // Compute e^{-ikr}
        T phase =
            -(pos_s[i][0] * kPoints_s[tid][0] + pos_s[i][1] * kPoints_s[tid][1] + pos_s[i][2] * kPoints_s[tid][2]);
        sincos(phase, &s, &c);
...
template<typename T, int BS>
__global__ void phase_factor_kernel_new(T* kPoints,
                                        int* makeTwoCopies,
                                        T* pos,
                                        T** phi_in,
                                        T** phi_out,
                                        int num_splines)
{
  __shared__ T in_shared[2 * BS], out_shared[2 * BS], kPoints_s[BS][3], pos_s[3];
  ...
  for (int ib = 0; ib < NB; ib++)
  {
    ...
    T phase = -(kPoints_s[tid][0] * pos_s[0] + kPoints_s[tid][1] * pos_s[1] + kPoints_s[tid][2] * pos_s[2]);
    T s, c;
    sincosf(phase, &s, &c);
    int off             = 2 * ib * BS + tid;
                                    ...
                                    int row_stride)
{
  ...
  for (int block = 0; block < nb; block++)
  {
    ...
    T phase = -(pos_s[0] * kPoints_s[tid][0] + pos_s[1] * kPoints_s[tid][1] + pos_s[2] * kPoints_s[tid][2]);
    T s, c;
    sincos(phase, &s, &c);
    ...
    T k2  = (kPoints_s[tid][0] * kPoints_s[tid][0] + kPoints_s[tid][1] * kPoints_s[tid][1] +
            kPoints_s[tid][2] * kPoints_s[tid][2]);
    T lre = laplu_re - k2 * u_re +
        2.0 * (kPoints_s[tid][0] * gradu_im[0] + kPoints_s[tid][1] * gradu_im[1] + kPoints_s[tid][2] * gradu_im[2]);
    T lim = laplu_im - k2 * u_im -
        2.0 * (kPoints_s[tid][0] * gradu_re[0] + kPoints_s[tid][1] * gradu_re[1] + kPoints_s[tid][2] * gradu_re[2]);
                                    ...
                                    int num_split_splines)
{
  ...
  for (int block = 0; block < nb; block++)
  {
    ...
    // Now add on phase factors
    T phase = -(pos_s[0] * kPoints_s[0] + pos_s[1] * kPoints_s[1] + pos_s[2] * kPoints_s[2]);
    T s, c;
    sincos(phase, &s, &c);
    T u_re, u_im, gradu_re[3], gradu_im[3], laplu_re, laplu_im;
    u_re            = in_shared[0][0];
    ...
    // Add phase contribution to laplacian
    T k2  = (kPoints_s[0] * kPoints_s[0] + kPoints_s[1] * kPoints_s[1] + kPoints_s[2] * kPoints_s[2]);
    T lre = laplu_re - k2 * u_re +
        2.0 * (kPoints_s[0] * gradu_im[0] + kPoints_s[1] * gradu_im[1] + kPoints_s[2] * gradu_im[2]);
    T lim = laplu_im - k2 * u_im -
        2.0 * (kPoints_s[0] * gradu_re[0] + kPoints_s[1] * gradu_re[1] + kPoints_s[2] * gradu_re[2]);
...
void apply_phase_factors(float kPoints[],
                         int makeTwoCopies[],
                         float pos[],
                         float* phi_in[],
                         float* phi_out[],
                         ...
                         int num_walkers)
{
  ...
  phase_factor_kernel<float, BS><<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(kPoints,
                                                                              makeTwoCopies,
                                                                              pos,
                                                                              phi_in,
                                                                              phi_out,
...
void apply_phase_factors(double kPoints[],
                         int makeTwoCopies[],
                         double pos[],
                         double* phi_in[],
                         double* phi_out[],
                         ...
                         int num_walkers)
{
  ...
  phase_factor_kernel<double, BS><<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(kPoints,
                                                                               makeTwoCopies,
                                                                               pos,
                                                                               phi_in,
                                                                               phi_out,
                                    ...
                                    int num_split_splines)
{
  ...
  for (int ib = 0; ib < NB; ib++)
  {
    ...
    if (off < num_splines)
    {
      ...
      T phase = -(kPoints_s[0] * pos_s[0] + kPoints_s[1] * pos_s[1] + kPoints_s[2] * pos_s[2]);
      T s, c;
      sincos(phase, &s, &c);
      T2 e_mikr  = T2(c, s);
                                    ...
                                    int num_split_splines)
{
  ...
  for (int block = 0; block < nb; block++)
  {
    ...
    if (off < num_splines)
    {
      ...
      gradlaplu[3] = e_mikr *
          (gradlaplu[3] - k2 * u -
           T2(0.0, 2.0) * (kPoints_s[0] * gradlaplu[0] + kPoints_s[1] * gradlaplu[1] + kPoints_s[2] * gradlaplu[2]));
...
void apply_phase_factors(float kPoints[],
                         float pos[],
                         std::complex<float>* phi_in[],
                         std::complex<float>* phi_out[],
                         int num_splines,
                         int num_walkers)
{
  const int BS = 32;
  ...
  phase_factor_kernel<float, thrust::complex<float>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(kPoints,
                                                    pos,
                                                    (thrust::complex<float>**)phi_in,
                                                    (thrust::complex<float>**)phi_out,
                                                    num_splines,
                                                    num_walkers,
                                                    Nsplit);
}
                         ...
                         int num_walkers)
{
  ...
  phase_factor_kernel<double, thrust::complex<double>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(kPoints,
                                                    pos,
                                                    (thrust::complex<double>**)phi_in,
                                                    (thrust::complex<double>**)phi_out,
                                                    num_splines,
                                                    num_walkers,
                                                    Nsplit);
}
...
void apply_phase_factors(float kPoints[],
                         ...
                         std::complex<float>* phi_in[],
                         std::complex<float>* phi_out[],
                         std::complex<float>* GL_in[],
                         std::complex<float>* GL_out[],
                         int num_splines,
                         int num_walkers,
                         int row_stride)
{
  ...
  phase_factor_kernel<float, thrust::complex<float>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(kPoints,
                                                    ...
                                                    (thrust::complex<float>**)phi_in,
                                                    (thrust::complex<float>**)phi_out,
                                                    (thrust::complex<float>**)GL_in,
                                                    (thrust::complex<float>**)GL_out,
                                                    num_splines,
                                                    num_walkers,
                                                    row_stride,
                                                    Nsplit);
...
void apply_phase_factors(double kPoints[],
                         ...
                         std::complex<double>* phi_in[],
                         std::complex<double>* phi_out[],
                         std::complex<double>* GL_in[],
                         std::complex<double>* GL_out[],
                         int num_splines,
                         int num_walkers,
                         int row_stride)
{
  ...
  phase_factor_kernel<double, thrust::complex<double>, BS>
      <<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(kPoints,
                                                    ...
                                                    (thrust::complex<double>**)phi_in,
                                                    (thrust::complex<double>**)phi_out,
                                                    (thrust::complex<double>**)GL_in,
                                                    (thrust::complex<double>**)GL_out,
                                                    num_splines,
                                                    num_walkers,
                                                    row_stride,
                                                    Nsplit);
