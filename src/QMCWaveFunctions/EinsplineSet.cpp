...
{
  eval_multi_UBspline_3d_d_vghgh(spline,
                                 r[0],
                                 r[1],
                                 r[2],
                                 psi.data(),
...
{
  eval_multi_UBspline_3d_z_vghgh(spline,
                                 r[0],
                                 r[1],
                                 r[2],
                                 psi.data(),
  ...
  if (ionDerivs)
  {
    ...
    for (int dim = 0; dim < OHMMS_DIM; dim++)
    {
      ...
      for (int iel = first, i = 0; iel < last; iel++, i++)
      {
        ...
        EinsplineMultiEval(FirstOrderSplines[iat_src][dim],
                           ru,
                           StorageValueVector,
                           StorageGradVector,
                           StorageHessVector);
                                                             ...
                                                             RealGGGMatrix_t& grad_grad_grad_logdet)
{
  ...
  for (int iat = first, i = 0; iat < last; iat++, i++)
  {
    ...
    for (int n = 0; n < OHMMS_DIM; n++)
      ru[n] -= std::floor(ru[n]);
    EinsplineTimer.start();
    EinsplineMultiEval(MultiSpline,
                       ru,
                       StorageValueVector,
                       StorageGradVector,
                       StorageHessVector,
