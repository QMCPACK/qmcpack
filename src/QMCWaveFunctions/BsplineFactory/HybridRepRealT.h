...
class HybridRepRealT : public SPLINEBASE,
                       private HybridRepCenterOrbitalsT<typename SPLINEBASE::DataType, typename SPLINEBASE::ValueType>
{
public:
  using HYBRIDBASE       = HybridRepCenterOrbitalsT<typename SPLINEBASE::DataType, typename SPLINEBASE::ValueType>;
...
public:
  ...
  std::unique_ptr<SPOSetT<ValueType>> makeClone() const override { return std::make_unique<HybridRepRealT>(*this); }
  ...
  void evaluateValue(const ParticleSetT<ValueType>& P, const int iat, ValueVector& psi) override
  {
  ...
  void evaluateDetRatios(const VirtualParticleSetT<ValueType>& VP,
                         ValueVector& psi,
  ...
  void mw_evaluateDetRatios(const RefVectorWithLeader<SPOSetT<ValueType>>& spo_list,
                            const RefVectorWithLeader<const VirtualParticleSetT<ValueType>>& vp_list,
                            const RefVector<ValueVector>& psi_list,
                            const std::vector<const ValueType*>& invRow_ptr_list,
                            std::vector<std::vector<ValueType>>& ratios_list) const final
  {
    BsplineSetT<ValueType>::mw_evaluateDetRatios(spo_list, vp_list, psi_list, invRow_ptr_list, ratios_list);
  }

  void evaluateVGL(const ParticleSetT<ValueType>& P,
                   const int iat,
                   ValueVector& psi,
                   GradVector& dpsi,
  ...
  void mw_evaluateVGL(const RefVectorWithLeader<SPOSetT<ValueType>>& sa_list,
                      const RefVectorWithLeader<ParticleSetT<ValueType>>& P_list,
                      int iat,
                      const RefVector<ValueVector>& psi_v_list,
                      const RefVector<GradVector>& dpsi_v_list,
                      const RefVector<ValueVector>& d2psi_v_list) const final
  {
    BsplineSetT<ValueType>::mw_evaluateVGL(sa_list, P_list, iat, psi_v_list, dpsi_v_list, d2psi_v_list);
  }

  void mw_evaluateVGLandDetRatioGrads(const RefVectorWithLeader<SPOSetT<ValueType>>& spo_list,
                                      const RefVectorWithLeader<ParticleSetT<ValueType>>& P_list,
                                      int iat,
                                      const std::vector<const ValueType*>& invRow_ptr_list,
  ...
  {
    BsplineSetT<ValueType>::mw_evaluateVGLandDetRatioGrads(spo_list, P_list, iat, invRow_ptr_list, phi_vgl_v, ratios,
                                                           grads);
  }

  void evaluateVGH(const ParticleSetT<ValueType>& P,
                   const int iat,
                   ValueVector& psi,
                   GradVector& dpsi,
  ...
  void evaluateVGHGH(const ParticleSetT<ValueType>& P,
                     const int iat,
                     ValueVector& psi,
                     GradVector& dpsi,
  ...
  void evaluate_notranspose(const ParticleSetT<ValueType>& P,
                            int first,
                            int last,
                            ValueMatrix& logdet,
                            GradMatrix& dlogdet,
                            ValueMatrix& d2logdet) final
  {
    ...
    BsplineSetT<ValueType>::evaluate_notranspose(P, first, last, logdet, dlogdet, d2logdet);
  }
