...
template<typename SA>
  void SplineSetReader<SA>::readOneOrbitalCoefs(const std::string& s, hdf_archive& h5f, Vector<std::complex<double>>& cG) const
  {
...
template<typename SA>
  void SplineSetReader<SA>::initialize_spline_pio_gather(const int spin, const BandInfoGroup& bandgroup, SA& bspline) const
  {
