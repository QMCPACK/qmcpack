...
struct LCAOSpinorBuilderMaker
{
  ...
  std::unique_ptr<LCAOSpinorBuilderT<T>> operator()(TArgs&&...) const
  {
    throw std::runtime_error("lcao spinors not compatible with non-complex value types");
  }
};
...
{
  template<typename... TArgs>
  std::unique_ptr<LCAOSpinorBuilderT<std::complex<T>>> operator()(TArgs&&... args) const
  {
    return std::make_unique<LCAOSpinorBuilderT<std::complex<T>>>(std::forward<TArgs>(args)...);
  }
};

template<typename T>
const SPOSetT<T>* SPOSetBuilderFactoryT<T>::getSPOSet(const std::string& name) const
{
 ...
 */
template<typename T>
SPOSetBuilderFactoryT<T>::SPOSetBuilderFactoryT(Communicate* comm, ParticleSetT<T>& els, const PSetMap& psets)
...
template<typename T>
std::unique_ptr<SPOSetBuilderT<T>> SPOSetBuilderFactoryT<T>::createSPOSetBuilder(xmlNodePtr rootNode)
{
  ...
  else if (type == "jellium" || type == "heg" || type == "free")
  {
    ...
    bb = std::make_unique<FreeOrbitalBuilderT<T>>(targetPtcl, myComm, rootNode);
  }
  else if (type == "sho")
  {
  ...
  else if (type == "molecularorbital" || type == "mo")
  {
    ...
    if (targetPtcl.isSpinor())
    {
      try
      {
        bb = LCAOSpinorBuilderMaker<T>{}(targetPtcl, *ions, myComm, rootNode);
      }
      ...
      bb = std::make_unique<LCAOrbitalBuilderT<T>>(targetPtcl, *ions, myComm, rootNode);
  }
#endif // OHMMS_DIM==3
  PRE.flush();
  ...
  app_log() << "  Created SPOSetT builder named '" << name << "' of type " << type << std::endl;
  return bb;
}

template<typename T>
void SPOSetBuilderFactoryT<T>::buildSPOSetCollection(xmlNodePtr cur)
{
  ...
  processChildren(cur, [&](const std::string& cname, const xmlNodePtr element) {
    if (cname == "sposet")
    {
      addSPOSet(std::unique_ptr<SPOSetT<T>>(bb->createSPOSet(element)));
      nsposets++;
    }
    if (cname == "rotated_sposet")
    {
      addSPOSet(std::unique_ptr<SPOSetT<T>>(bb->createRotatedSPOSet(element)));
      nsposets++;
    }
  });

  if (nsposets == 0)
    myComm->barrier_and_abort("SPOSetBuilderFactoryT::buildSPOSetCollection  no <sposet/> "
                              "elements found");
...
template<typename T>
void SPOSetBuilderFactoryT<T>::addSPOSet(std::unique_ptr<SPOSetT<T>> spo)
{
