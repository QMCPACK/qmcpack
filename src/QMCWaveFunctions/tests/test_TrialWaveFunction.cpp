...
void setup_He_wavefunction(Communicate* c, ParticleSet** elec_ptr, ParticleSet** ions_ptr, TrialWaveFunction** psi)
{
  ...
  *ions_ptr         = ions;
  ...
  WaveFunctionFactory* wff = new WaveFunctionFactory(elec, particle_set_map, c);
...
TEST_CASE("TrialWaveFunction flex_evaluateParameterDerivatives", "[wavefunction]")
{
  ...
  ParticleSet* elec;
  ParticleSet* ions;
  setup_He_wavefunction(c, &elec, &ions, &psi);
  ...
  ParticleSet* elec2 = new ParticleSet(*elec);
  elec2->R[0][0]     = 0.9;
  elec2->update();
