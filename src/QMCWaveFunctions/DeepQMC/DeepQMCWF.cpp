...
DeepQMCWF::DeepQMCWF(std::string name,
                     const ParticleSet& ions,
                     std::unique_ptr<const DeepQMCBridge> bridge,
                     int mol_idx)
    : DeepQMCWF(std::move(name), ions, std::shared_ptr<const DeepQMCBridge>(std::move(bridge)), mol_idx)
{}
...
{
  if (!bridge_)
    throw std::runtime_error("DeepQMCWF requires a non-null DeepQMCBridge");
}
...
void DeepQMCWF::appendElectronCoords(const ParticleSet& electrons,
...
DeepQMCBridge::BatchResult DeepQMCWF::evaluateBatch(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                                                    const RefVectorWithLeader<ParticleSet>& p_list,
                                                    int active_iat)
{
  if (wfc_list.size() != p_list.size())
    throw std::runtime_error("DeepQMCWF::evaluateBatch list size mismatch");
  if (wfc_list.empty())
    return {};
...
DeepQMCBridge::BatchResult DeepQMCWF::evaluateOne(const ParticleSet& electrons, int active_iat) const
{
  ...
  RefVectorWithLeader<WaveFunctionComponent> wfc_list(*const_cast<DeepQMCWF*>(this));
  wfc_list.push_back(*const_cast<DeepQMCWF*>(this));
...
DeepQMCWF::LogValue DeepQMCWF::evaluateLog(const ParticleSet& P,
...
void DeepQMCWF::mw_evaluateLog(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                               const RefVectorWithLeader<ParticleSet>& p_list,
...
void DeepQMCWF::mw_evalGrad(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                            const RefVectorWithLeader<ParticleSet>& p_list,
                            int iat,
                            std::vector<GradType>& grad_now) const
{
  if (wfc_list.size() != p_list.size() || wfc_list.size() != grad_now.size())
    throw std::runtime_error("DeepQMCWF::mw_evalGrad list size mismatch");
  if (wfc_list.empty())
    return;
...
void DeepQMCWF::mw_calcRatio(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                             const RefVectorWithLeader<ParticleSet>& p_list,
                             int iat,
                             std::vector<PsiValue>& ratios) const
{
  if (wfc_list.size() != p_list.size())
    throw std::runtime_error("DeepQMCWF::mw_calcRatio list size mismatch");
  const int batch_size = static_cast<int>(wfc_list.size());
...
void DeepQMCWF::mw_ratioGrad(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                             const RefVectorWithLeader<ParticleSet>& p_list,
                             int iat,
                             ...
                             std::vector<GradType>& grad_new) const
{
  if (wfc_list.size() != p_list.size() || wfc_list.size() != grad_new.size())
    throw std::runtime_error("DeepQMCWF::mw_ratioGrad list size mismatch");
  const int batch_size = static_cast<int>(wfc_list.size());
  ratios.resize(batch_size);
  if (wfc_list.empty())
    return;
...
void DeepQMCWF::mw_accept_rejectMove(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                                     const RefVectorWithLeader<ParticleSet>& p_list,
                                     int iat,
                                     ...
                                     bool safe_to_delay) const
{
  if (wfc_list.size() != isAccepted.size())
    throw std::runtime_error("DeepQMCWF::mw_accept_rejectMove list size mismatch");
...
void DeepQMCWF::mw_completeUpdates(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list) const {}

void DeepQMCWF::mw_evaluateGL(const RefVectorWithLeader<WaveFunctionComponent>& wfc_list,
                              const RefVectorWithLeader<ParticleSet>& p_list,
                              const RefVector<ParticleSet::ParticleGradient>& G_list,
...
void DeepQMCWF::restore(int iat) { has_proposed_log_value_ = false; }

DeepQMCWF::PsiValue DeepQMCWF::ratio(ParticleSet& P, int iat)
{
...
DeepQMCWF::GradType DeepQMCWF::evalGrad(ParticleSet& P, int iat)
{
...
DeepQMCWF::PsiValue DeepQMCWF::ratioGrad(ParticleSet& P, int iat, GradType& grad_iat)
{
  const auto result       = evaluateOne(P, iat);
  proposed_log_value_     = LogValue(result.log_values[0]);
  has_proposed_log_value_ = true;
...
{
  return std::make_unique<DeepQMCWF>(my_name_, ions_, bridge_, mol_idx_);
}
