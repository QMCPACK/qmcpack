...
OptimizableSPOSet::put (xmlNodePtr node, SPOPool_t &spo_pool)
{
  ...
  else
  {
    ...
    GSOrbitals = dynamic_cast<SPOSet<Batching::SINGLE>*>(iter->second);
  }
  //////////////////////////////////////
  ...
  if (iter != spo_pool.end())
  {
    BasisOrbitals = dynamic_cast<SPOSet<Batching::SINGLE>*>(iter->second);
    app_log() << "  Found basis SPOSet \"" << basisName << "\".\n";
  }
  if (BasisOrbitals == GSOrbitals)
    BasisOrbitals = 0;
  if (BasisOrbitals)
    M = dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->getOrbitalSetSize();
  else
    M = dynamic_cast<SPOSet<Batching::SINGLE>*>(GSOrbitals)->getOrbitalSetSize() - N;
  resize(N,M);
...
{
  dynamic_cast<SPOSet<Batching::SINGLE>*>(GSOrbitals)->resetTargetParticleSet(P);
  if (BasisOrbitals)
    dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->resetTargetParticleSet(P);
}
 ...
 ValueMatrix_t& d_phi, ValueMatrix_t& d_lapl_phi)
{
  ...
  if (BasisOrbitals)
  {
    dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->evaluate(P,iat,BasisVal);
...
{
  dynamic_cast<SPOSet<Batching::SINGLE>*>(GSOrbitals)->evaluate(P,iat,GSVal);
  if (BasisOrbitals)
  {
    dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->evaluate(P,iat,BasisVal);
...
OptimizableSPOSet::evaluate(const ParticleSet& P, int iat,
                            ...
                            ValueVector_t& d2psi)
{
  dynamic_cast<SPOSet<Batching::SINGLE>*>(GSOrbitals)->evaluate(P,iat,GSVal,GSGrad,GSLapl);
  const ValueMatrix_t& cref(*C);
  if (BasisOrbitals)
  {
    dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->evaluate(P,iat,BasisVal,BasisGrad,BasisLapl);
    BLAS::gemv_trans (N, M, C->data(), &(BasisVal[0]),  &(psi[0]));
...
{
  if (BasisOrbitals)
    dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->evaluate_notranspose(P, first, last, basis_val, basis_grad, basis_lapl);
  else
  {
...
//     std::cerr << "first=" << first << "  last=" << last << std::endl;
  dynamic_cast<SPOSet<Batching::SINGLE>*>(GSOrbitals)->evaluate_notranspose
  (P, first, last, GSValMatrix, GSGradMatrix, GSLaplMatrix);
  if (BasisOrbitals)
  {
    dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->evaluate_notranspose(P, first, last, BasisValMatrix, BasisGradMatrix, BasisLaplMatrix);
    //Note to Ken:
...
{
  SPOSet<Batching::SINGLE> *gs, *basis(0);
  OptimizableSPOSet *clone;
  gs = GSOrbitals->makeClone();
  if (BasisOrbitals)
    basis = dynamic_cast<SPOSet<Batching::SINGLE>*>(BasisOrbitals)->makeClone();
