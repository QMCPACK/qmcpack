...
eval_multi_multi_UBspline_3d_cuda (multi_UBspline_3d_c_cuda *spline,
                                   float *pos, std::complex<float> *phi[], int N,
                                   std::complex<float> *spline_coefs[], cudaEvent_t spline_events[], cudaStream_t spline_streams[])
{
  int curr_gpu;
  int devicenr=0;
...
inline void eval_multi_multi_UBspline_3d_vgl_cuda (multi_UBspline_3d_c_cuda *spline, float *pos, float Linv[],
                                                   std::complex<float>* phi[], std::complex<float>* grad_lapl[], int N, int row_stride,
                                                   std::complex<float> *spline_coefs[], cudaEvent_t spline_events[], cudaStream_t spline_streams[])
{
  int curr_gpu;
...
//       fprintf (stderr, "kred2 = %8.3f %8.3f %8.3f\n", k_red2[0], k_red2[1], k_red2[2]);
  ...
  CudakPoints_reduced = hostkPoints_reduced; // make sure the last copy is synchronous so when we return from this function everything's finished
}
      ...
      for(unsigned int g=0; g<gpu::device_group_size; g++)
      {
        ...
        for(unsigned int j=0; j<CudaMultiSpline->num_split_splines; j++)
          fprintf(stderr,"walker %i, orbital %i: %f+%fi, (%f+%fi, %f+%fi, %f+%fi) | %f+%fi\n",iw,j+g*CudaMultiSpline->num_split_splines,
                        CudaValuePointers[iw][j+off].real(),CudaValuePointers[iw][j+off].imag(),
                        CudaGradLaplPointers[iw][0*CudaMultiSpline->num_split_splines+j+4*off].real(),CudaGradLaplPointers[iw][0*CudaMultiSpline->num_split_splines+j+4*off].imag(),
                        CudaGradLaplPointers[iw][1*CudaMultiSpline->num_split_splines+j+4*off].real(),CudaGradLaplPointers[iw][1*CudaMultiSpline->num_split_splines+j+4*off].imag(),
                        CudaGradLaplPointers[iw][2*CudaMultiSpline->num_split_splines+j+4*off].real(),CudaGradLaplPointers[iw][2*CudaMultiSpline->num_split_splines+j+4*off].imag(),
                        CudaGradLaplPointers[iw][3*CudaMultiSpline->num_split_splines+j+4*off].real(),CudaGradLaplPointers[iw][3*CudaMultiSpline->num_split_splines+j+4*off].imag());
     }
  }
#endif
...
template<> void
EinsplineSetExtended<double>::evaluate
(std::vector<PosType> &pos, gpu::device_vector<CTS::RealType*> &phi)
{
  int N = pos.size();
  CTS::RealType plus_minus[2] = {1.0, -1.0};
  if (NLcudapos.size() < N)
                                      ...
                                      int row_stride)
{
  ...
  MakeHybridJobList<CTS::RealType> ((CTS::RealType*)cudapos.data(), N, IonPos_GPU.data(),
                                   PolyRadii_GPU.data(), CutoffRadii_GPU.data(),
                                   AtomicOrbitals.size(), L_cuda.data(), Linv_cuda.data(),
                                   HybridJobs_GPU.data(), rhats_GPU.data(),
                                   HybridData_GPU.data());

  CalcYlmRealCuda<CTS::RealType> (rhats_GPU.data(), HybridJobs_GPU.data(),
  ...
  evaluate3DSplineReal<CTS::RealType> (HybridJobs_GPU.data(), (CTS::RealType*)cudapos.data(),
                                      (CTS::RealType*)CudakPoints_reduced.data(),CudaMultiSpline,
                                      Linv_cuda.data(), phi.data(), grad_lapl.data(),
                                      row_stride, NumOrbitals, newpos.size());

  evaluateHybridSplineReal<CTS::RealType> (HybridJobs_GPU.data(), rhats_GPU.data(), 
                                          Ylm_ptr_GPU.data(),
...
(std::vector<PosType> &pos, gpu::device_vector<CTS::RealType*> &phi)
{
  ...
  evaluateHybridSplineReal<CTS::RealType> (HybridJobs_GPU.data(), Ylm_ptr_GPU.data(),
                                          AtomicOrbitals_GPU.data(), HybridData_GPU.data(),
                                          (CTS::RealType*)CudakPoints_reduced.data(),
...
template<> void
EinsplineSetHybrid<double>::evaluate (std::vector<PosType> &pos, gpu::device_vector<CTS::ComplexType*> &phi)
{
  app_error() << "EinsplineSetHybrid<double>::evaluate \n"
              << "(std::vector<PosType> &pos, gpu::device_vector<CTS::ComplexType*> &phi)\n"
              << "     is not yet implemented.\n";
 ...
 int row_stride)
{
  ...
  CalcYlmComplexCuda<CTS::RealType> (rhats_GPU.data(), HybridJobs_GPU.data(),
                                    Ylm_ptr_GPU.data(), dYlm_dtheta_ptr_GPU.data(),
                                    dYlm_dphi_ptr_GPU.data(), lMax, newpos.size());
  ...
  evaluateHybridSplineComplexToReal<CTS::RealType>
                      (HybridJobs_GPU.data(), rhats_GPU.data(),
                       Ylm_ptr_GPU.data(),
                       dYlm_dtheta_ptr_GPU.data(), dYlm_dphi_ptr_GPU.data(),
                       AtomicOrbitals_GPU.data(), HybridData_GPU.data(),
                       (CTS::RealType*)CudakPoints_reduced.data(),
                       CudaMakeTwoCopies.data(), (CTS::RealType**)phi.data(),
...
(std::vector<PosType> &pos, gpu::device_vector<CTS::RealType*> &phi)
{
  ...
  evaluate3DSplineComplexToReal<CTS::RealType>
  (HybridJobs_GPU.data(), (CTS::RealType*)cudapos.data(),
