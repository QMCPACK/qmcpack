...
{
  eval_multi_UBspline_3d_z_vgh(spline,
                               r[0],
                               r[1],
                               r[2],
                               psi.data(),
                                              ...
                                              cudaStream_t spline_streams[])
{
...
#endif
    eval_multi_multi_UBspline_3d_s_sign_cudasplit(spline,
                                                  pos,
                                                  sign,
                                                  phi,
                                                  N,
                                                  spline_coefs[devicenr],
                                                  devicenr,
  ...
  {
    fprintf(stderr,
            "CUDA error in eval_multi_multi_UBspline_3d_cuda (rank %i):\n  %s\n",
                                              ...
                                              cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    std::cerr << "Rank " << OHMMS::Controller->rank() << ", GPU mem before: " << spline->coefs
              << "; after: " << spline_coefs[devicenr] << "\n";
#endif
    eval_multi_multi_UBspline_3d_d_sign_cudasplit(spline,
                                                  pos,
                                                  sign,
                                                  phi,
                                                  N,
                                              ...
                                              cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    std::cerr << "Rank " << OHMMS::Controller->rank() << ", GPU mem before: " << spline->coefs
              << "; after: " << spline_coefs[devicenr] << "\n";
#endif
    eval_multi_multi_UBspline_3d_d_cudasplit(spline,
                                             pos,
                                             phi,
                                             N,
                                             spline_coefs[devicenr],
                                                  ...
                                                  cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    cudaMemPrefetchAsync(pos, 3 * N * sizeof(float), curr_gpu, spline_streams[devicenr]);
#endif
    eval_multi_multi_UBspline_3d_s_vgl_sign_cudasplit(spline,
                                                      pos,
                                                      sign,
                                                      Linv,
                                                      phi,
  ...
  {
    fprintf(stderr,
            "CUDA error in eval_multi_multi_UBspline_3d_vgl_cuda (rank %i):\n  %s\n",
            OHMMS::Controller->rank(),
            cudaGetErrorString(err));
    abort();
  }
                                                  ...
                                                  cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    cudaMemPrefetchAsync(pos, 3 * N * sizeof(float), curr_gpu, spline_streams[devicenr]);
#endif
    eval_multi_multi_UBspline_3d_d_vgl_sign_cudasplit(spline,
                                                      pos,
                                                      sign,
                                                      Linv,
                                                      phi,
                                                  ...
                                                  cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    cudaMemPrefetchAsync(pos, 3 * N * sizeof(float), curr_gpu, spline_streams[devicenr]);
#endif
    eval_multi_multi_UBspline_3d_d_vgl_cudasplit(spline,
                                                 pos,
                                                 Linv,
                                                 phi,
                                                 grad_lapl,
                                              ...
                                              cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    std::cerr << "Rank " << OHMMS::Controller->rank() << ", GPU mem before: " << spline->coefs
              << "; after: " << spline_coefs[devicenr] << "\n";
#endif
    eval_multi_multi_UBspline_3d_c_cudasplit(spline,
                                             pos,
                                             phi,
                                             N,
                                             (float*)spline_coefs[devicenr],
                                             devicenr,
                                             spline_streams[devicenr]);
    cudaEventRecord(spline_events[devicenr], spline_streams[devicenr]);
  }
                                              ...
                                              cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    std::cerr << "Rank " << OHMMS::Controller->rank() << ", GPU mem before: " << spline->coefs
              << "; after: " << spline_coefs[devicenr] << "\n";
#endif
    eval_multi_multi_UBspline_3d_z_cudasplit(spline,
                                             pos,
                                             phi,
                                             N,
                                             (double*)spline_coefs[devicenr],
                                             devicenr,
                                             spline_streams[devicenr]);
    cudaEventRecord(spline_events[devicenr], spline_streams[devicenr]);
  }
                                                  ...
                                                  cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    cudaMemPrefetchAsync(pos, 3 * N * sizeof(float), curr_gpu, spline_streams[devicenr]);
#endif
    eval_multi_multi_UBspline_3d_c_vgl_cudasplit(spline,
                                                 pos,
                                                 Linv,
                                                 phi,
                                                 grad_lapl,
                                                  ...
                                                  cudaStream_t spline_streams[])
{
  ...
  for (unsigned int i = 0; i < gpu::device_group_size; i++)
  {
    ...
    cudaMemPrefetchAsync(pos, 3 * N * sizeof(double), curr_gpu, spline_streams[devicenr]);
#endif
    eval_multi_multi_UBspline_3d_z_vgl_cudasplit(spline,
                                                 pos,
                                                 Linv,
                                                 phi,
                                                 grad_lapl,
                                            ...
                                            gpu::device_vector<CTS::RealType*>& phi)
{
  ...
  if (split_splines)
  {
    eval_multi_multi_UBspline_3d_cuda(CudaMultiSpline,
                                      (CTS::RealType*)(cudapos.data()),
                                      cudaSign.data(),
                                      phi.data(),
                                      N,
                                      spline_rank_pointers.data(),
                                      spline_events.data(),
                                      spline_streams.data());
  }
  else
                                                          ...
                                                          gpu::device_vector<CTS::RealType*>& phi)
{
  ...
  if (cudapos.size() < N)
  {
    ...
    if (split_splines)
    {
      ...
      for (unsigned int i = 0; i < gpu::device_group_size; i++)
        cudaMemAdvise(cudapos.data(),
                      N * sizeof(CTS::PosType),
...
void EinsplineSetExtended<T>::get_split_spline_pointers()
{
  ...
  if ((size > 1) && split_splines)
  {
    ...
    MPI_Allgather(&rank_handle[0],
                  sizeof(cudaIpcMemHandle_t),
                  MPI_CHAR,
                  &spline_rank_handles[0],
                  sizeof(cudaIpcMemHandle_t),
                  MPI_CHAR,
    ...
    for (unsigned int i = 0; i < gpu::device_group_size; i++)
    {
      ...
      if (i != gpu::relative_rank % gpu::device_group_size)
      {
        ...
        {
          fprintf(stderr,
                  "CUDA error getting memory handles between GPUs %i and %i:\n  %s\n",
                  gpu::device_group_numbers[gpu::relative_rank % gpu::device_group_size],
                  gpu::device_group_numbers[i],
                  cudaGetErrorString(err));
          abort();
        }
...
void EinsplineSetExtended<T>::resize_cuda(int numWalkers)
{
  ...
  if (split_splines)
    for (unsigned int i = 0; i < gpu::device_group_size; i++)
    {
      cudaMemAdvise(CudaValuePointers.data(),
                    numWalkers * sizeof(CudaStorageType*),
                    cudaMemAdviseSetAccessedBy,
                    gpu::device_group_numbers[i]);
      cudaMemAdvise(CudaGradLaplPointers.data(),
                    numWalkers * sizeof(CudaStorageType*),
                    cudaMemAdviseSetAccessedBy,
                    gpu::device_group_numbers[i]);
      if (i == gpu::relative_rank)
      { // all of the output memory is accessed by this rank's GPU
        cudaMemAdvise(CudaValueVector.data(),
                      N * numWalkers * sizeof(CudaStorageType),
                      cudaMemAdviseSetAccessedBy,
                      gpu::device_group_numbers[i]);
        cudaMemAdvise(CudaGradLaplVector.data(),
                      4 * N * numWalkers * sizeof(CudaStorageType),
                      cudaMemAdviseSetAccessedBy,
                      gpu::device_group_numbers[i]);
      }
      else
      { // only a subsection of the output memory is written to by each other GPU
        cudaMemAdvise(&CudaValueVector.data()[i * numWalkers * Nsplit],
                      Nsplit * numWalkers * sizeof(CudaStorageType),
                      cudaMemAdviseSetAccessedBy,
                      gpu::device_group_numbers[i]);
        cudaMemAdvise(&CudaGradLaplVector.data()[4 * i * numWalkers * Nsplit],
                      4 * Nsplit * numWalkers * sizeof(CudaStorageType),
                      cudaMemAdviseSetAccessedBy,
                      gpu::device_group_numbers[i]);
      }
    }
                                            ...
                                            bool klinear)
{
  ...
  if (split_splines)
  {
    eval_multi_multi_UBspline_3d_vgl_cuda(CudaMultiSpline,
                                          (CTS::RealType*)cudapos.data(),
                                          cudaSign.data(),
                                          Linv_cuda.data(),
                                          &(phi.data()[offset]),
                                          &(grad_lapl.data()[offset]),
                                          N,
                                          row_stride,
                                          spline_rank_pointers.data(),
                                          spline_events.data(),
                                                          ...
                                                          bool klinear)
{
  ...
  if (split_splines)
  {
    eval_multi_multi_UBspline_3d_vgl_cuda(CudaMultiSpline,
                                          ...
                                          Linv_cuda.data(),
                                          CudaValuePointers.data(),
                                          CudaGradLaplPointers.data(),
                                          N,
                                          CudaMultiSpline->num_split_splines,
                                          spline_rank_pointers.data(),
                                          spline_events.data(),
                                          spline_streams.data());
  }
...
// AT debug:
    ...
    for (unsigned int iw = 0; iw < N; iw++)
      for (unsigned int g = 0; g < gpu::device_group_size; g++)
      {
        ...
        for (unsigned int j = 0; j < CudaMultiSpline->num_split_splines; j++)
          fprintf(stderr,
                  "walker %i, orbital %i: %f+%fi, (%f+%fi, %f+%fi, %f+%fi) | %f+%fi\n",
                  iw,
                  j + g * CudaMultiSpline->num_split_splines,
                  CudaValuePointers[iw][j + off].real(),
  ...
  */
  ...
  apply_phase_factors((CTS::RealType*)CudakPoints.data(),
                      ...
                      (CTS::RealType**)CudaValuePointers.data(),
                      &(phi.data()[offset]),
                      (CTS::RealType**)CudaGradLaplPointers.data(),
                      &(grad_lapl.data()[offset]),
                      CudaMultiSpline->num_splines,
                      N,
                      row_stride);
                                          ...
                                          int row_stride)
{
  ...
  MakeHybridJobList<CTS::RealType>((CTS::RealType*)cudapos.data(),
                                   N,
                                   IonPos_GPU.data(),
                                   PolyRadii_GPU.data(),
                                   CutoffRadii_GPU.data(),
                                   AtomicOrbitals.size(),
                                   L_cuda.data(),
                                   Linv_cuda.data(),
                                   HybridJobs_GPU.data(),
  ...
  CalcYlmRealCuda<CTS::RealType>(rhats_GPU.data(),
                                 HybridJobs_GPU.data(),
                                 Ylm_ptr_GPU.data(),
                                 dYlm_dtheta_ptr_GPU.data(),
                                 dYlm_dphi_ptr_GPU.data(),
                                 lMax,
                                 newpos.size());

  evaluate3DSplineReal<CTS::RealType>(HybridJobs_GPU.data(),
                                      (CTS::RealType*)cudapos.data(),
                                      (CTS::RealType*)CudakPoints_reduced.data(),
                                      CudaMultiSpline,
                                      Linv_cuda.data(),
                                      phi.data(),
                                      grad_lapl.data(),
  ...
  evaluateHybridSplineReal<CTS::RealType>(HybridJobs_GPU.data(),
                                          rhats_GPU.data(),
                                          Ylm_ptr_GPU.data(),
                                          dYlm_dtheta_ptr_GPU.data(),
                                          dYlm_dphi_ptr_GPU.data(),
                                          AtomicOrbitals_GPU.data(),
                                          HybridData_GPU.data(),
                                          (CTS::RealType*)CudakPoints_reduced.data(),
                                          phi.data(),
                                          grad_lapl.data(),
    ...
    if (false && HybridJobs_CPU[iw] == ATOMIC_POLY_JOB)
    {
      ...
      for (int j = 0; j < NumOrbitals; j++)
      {
        ...
        if (true || isnan(GL_CPU[0 * row_stride + j]))
        {
          ...
          fprintf(stderr,
                  "rhat[%d] = [%10.6f %10.6f %10.6f] dist = %10.6e\n",
                  iw,
                  rhats_CPU[3 * iw + 0],
                  rhats_CPU[3 * iw + 1],
                  rhats_CPU[3 * iw + 2],
          ...
          fprintf(stderr,
                  "grad[%2d] = %10.5e %10.5e  %10.5e %10.5e  %10.5e %10.5e\n",
                  j,
    ...
    else if (HybridJobs_CPU[iw] == BSPLINE_3D_JOB)
    {
      ...
      for (int j = 0; j < NumOrbitals; j++)
      {
        ...
        fprintf(stderr,
                ...
                GL_CPU[1 * row_stride + j],
                GL_CPU[2 * row_stride + j],
                GL_CPU[3 * row_stride + j]);
        fprintf(stderr,
                "CPU=%10.6f  %10.6f %10.6f %10.6f  %10.6f sign = %d\n",
                CPUvals[j],
                CPUgrad[j][0],
                CPUgrad[j][1],
                CPUgrad[j][2],
        ...
        if (std::isnan(GL_CPU[0 * row_stride + j]))
        {
          ...
          fprintf(stderr,
                  "3D grad[%2d] = %10.5e %10.5e  %10.5e %10.5e  %10.5e %10.5e\n",
                  j,
                  GL_CPU[0 * row_stride + j],
                  CPUgrad[j][0],
                  GL_CPU[1 * row_stride + j],
                                                        ...
                                                        int row_stride)
{
  ...
  CalcYlmComplexCuda<CTS::RealType>(rhats_GPU.data(),
                                    HybridJobs_GPU.data(),
                                    Ylm_ptr_GPU.data(),
                                    dYlm_dtheta_ptr_GPU.data(),
                                    dYlm_dphi_ptr_GPU.data(),
  ...
  evaluate3DSplineComplexToReal<CTS::RealType>(HybridJobs_GPU.data(),
                                               (CTS::RealType*)cudapos.data(),
                                               (CTS::RealType*)CudakPoints.data(),
                                               CudaMakeTwoCopies.data(),
                                               CudaMultiSpline,
                                               Linv_cuda.data(),
                                               phi.data(),
                                               grad_lapl.data(),
                                               row_stride,
                                               CudaMakeTwoCopies.size(),
                                               newpos.size());

  evaluateHybridSplineComplexToReal<CTS::RealType>(HybridJobs_GPU.data(),
                                                   rhats_GPU.data(),
                                                   Ylm_ptr_GPU.data(),
                                                   dYlm_dtheta_ptr_GPU.data(),
                                                   dYlm_dphi_ptr_GPU.data(),
...
void EinsplineSetExtended<double>::finalizeConstruction()
{
  ...
  if (split_splines)
  {
    ...
    for (unsigned int i = 0; i < gpu::device_group_size; i++)
    {
      ...
      cudaMemAdvise(Linv_cuda.data(),
                    9 * sizeof(CTS::RealType),
                    cudaMemAdviseSetAccessedBy,
                    gpu::device_group_numbers[i]);
