...
bool EinsplineSetBuilder::ReadOrbitalInfo_ESHDF(bool skipChecks)
{
  ...
  bool checkCellConsistency = true;
  if (SourcePtcl == nullptr && IonPos.size() == 0) {
    app_log() << "There are no ions in XML nor HDF5. No further checks will be performed.\n";
    checkCellConsistency = false;
  }
  if (Super2Prim.size() == 0 && checkCellConsistency)
    {
