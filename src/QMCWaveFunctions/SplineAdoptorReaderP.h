...
namespace qmcplusplus {
  ...
  struct BsplineReaderBase
  {
    ...
    template<typename GT, typename BCT>
      inline bool set_grid(const TinyVector<int,3>& halfg, GT* xyz_grid, BCT* xyz_bc)
      {
        ...
        xyz_grid[0].start = 0.0;  xyz_grid[0].end = 1.0;  xyz_grid[0].num = mybuilder->MeshSize[0];
        xyz_grid[1].start = 0.0;  xyz_grid[1].end = 1.0;  xyz_grid[1].num = mybuilder->MeshSize[1];
        xyz_grid[2].start = 0.0;  xyz_grid[2].end = 1.0;  xyz_grid[2].num = mybuilder->MeshSize[2];
    ...
    struct SplineAdoptorReader: BsplineReaderBase
  {
    ...
    void initialize_spline_slow(int spin)
    {
      ...
      if(bspline->is_complex) data_i.resize(ny,ny,nz);
    ...
    void initialize_spline_pio_bcast(int spin)
    {
      ...
      for(int ip=0; ip<np; ++ip)
      {
        for(int iorb=OrbGroups[ip], ib=0; iorb<OrbGroups[ip+1]; ++iorb, ++ib)
        {
          ...
          {
            dense_r=spline_r[ib];
            if(bspline->is_complex) dense_i=spline_i[ib];
          }
          else
          {//everyone else
            dense_r=spline_r[iorb_target]; 
            if(bspline->is_complex) dense_i=spline_i[iorb_target];
          }
