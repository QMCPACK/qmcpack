...
void TWFFastDerivWrapper::computeMDDerivative(const std::vector<ValueMatrix>& Minv,
                                              ...
                                              IndexType id;
                                              const std::vector<int>& ndet_per_exc_level,
                                              const std::vector<ValueType>& md_sign,
                                              ...
                                              std::vector<ValueType>& dvals) const
{
  ...
  BLAS::gemm('n', 'n', nvirt, nOcc, nelec, 1.0, M[id].data() + virt_offset, M[id].cols(), Minv[id].data(),
             Minv[id].cols(), 0.0, a_Ov.data(), a_Ov.cols());
  ...
  BLAS::gemm('n', 'n', nvirt, nOcc, nelec, 1.0, B[id].data() + virt_offset, B[id].cols(), Minv[id].data(),
             Minv[id].cols(), 0.0, f_Ov.data(), f_Ov.cols());

  // g[O,v] = X[O,e].M[e,v]
  ValueMatrix g_Ov(nOcc, nvirt);
  BLAS::gemm('n', 'n', nvirt, nOcc, nelec, 1.0, M[id].data() + virt_offset, M[id].cols(), X[id].data(), X[id].cols(),
             0.0, g_Ov.data(), g_Ov.cols());
  ...
  {
    size_t ndet_exc = (*ndets_per_excitation_level_)[ext_level];
    // calc vals
    ...
    {
      std::vector<IndexType> hlist(iexc_level, 0);
      std::vector<IndexType> plist(iexc_level, 0);
      size_t excdata_offset = data_offset + idet * (3 * iexc_level + 1);
      for (size_t i = 0; i < iexc_level; i++)
      {
        hlist[i] = excdata[excdata_offset + iexc_level + 1 + i];
        plist[i] = excdata[excdata_offset + 2 * iexc_level + 1 + i];
      }
    }
