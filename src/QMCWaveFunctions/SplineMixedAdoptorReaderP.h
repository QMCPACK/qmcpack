    ...
    struct SplineMixedAdoptorReader: BsplineReaderBase
    {
      ...
      SPOSetBase* create_spline_set(int spin, EinsplineSet* orbitalSet)
      {
        ...
        if(foundspline)
        {
          ...
          app_log() << "   Bcast Time for the big table = " << now.elapsed() << endl;
        }
        else
        {
      ...
      void initialize_spline_pio_bcast(int spin)
      {
        ...
        for(int ip=0; ip<np; ++ip)
        {
          for(int iorb=OrbGroups[ip], ib2=0; iorb<OrbGroups[ip+1]; ++iorb, ib2+=2)
          {
            if(ip==myComm->rank())
            {
              dense_r=spline_r[ib2]; coarse_r=spline_r[ib2+1];
              if(bspline->is_complex)
              {dense_i=spline_i[ib2]; coarse_i=spline_i[ib2+1]; }
            }
            else
            {//everyone else
              dense_r=spline_r[iorb_target]; coarse_r=spline_r[iorb_target+1];
              if(bspline->is_complex)
              {dense_i=spline_i[iorb_target]; coarse_i=spline_i[iorb_target+1];}
            }
