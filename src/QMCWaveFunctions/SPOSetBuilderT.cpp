...
template<>
std::unique_ptr<SPOSetT<float>> SPOSetBuilderT<float>::createSPOSet(xmlNodePtr cur)
{
  ...
  if (optimize == "rotation" || optimize == "yes")
  {
    ...
    auto rot_spo    = std::make_unique<RotatedSPOsT<float>>(sposet_ref.getName(), std::move(sposet));
...
template<>
std::unique_ptr<SPOSetT<double>> SPOSetBuilderT<double>::createSPOSet(xmlNodePtr cur)
{
  std::string spo_object_name;
  std::string optimize("no");
  ...
  if (optimize == "rotation" || optimize == "yes")
  {
    ...
    if (!sposet_ref.isRotationSupported())
      myComm->barrier_and_abort("Orbital rotation not supported with '" + sposet_ref.getName() + "' of type '" +
                                sposet_ref.getClassName() + "'.");
    auto rot_spo    = std::make_unique<RotatedSPOsT<double>>(sposet_ref.getName(), std::move(sposet));
    xmlNodePtr tcur = cur->xmlChildrenNode;
    while (tcur != NULL)
    {
      std::string cname((const char*)(tcur->name));
...
template<>
std::unique_ptr<SPOSetT<std::complex<float>>> SPOSetBuilderT<std::complex<float>>::createSPOSet(xmlNodePtr cur)
{
  std::string spo_object_name;
  std::string optimize("no");
  ...
  // process general sposet construction requests
  //   and preserve legacy interface
  std::unique_ptr<SPOSetT<std::complex<float>>> sposet;
...
template<>
std::unique_ptr<SPOSetT<std::complex<double>>> SPOSetBuilderT<std::complex<double>>::createSPOSet(xmlNodePtr cur)
{
  std::string spo_object_name;
  std::string optimize("no");
  ...
  // process general sposet construction requests
  //   and preserve legacy interface
  std::unique_ptr<SPOSetT<std::complex<double>>> sposet;
