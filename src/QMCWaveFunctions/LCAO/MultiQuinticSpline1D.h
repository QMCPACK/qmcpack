...
public:
  ...
  */
  ...
  {
    const size_t nElec           = r.size(0);
    const size_t Nxyz_per_center = r.size(1) / num_centers; // Assuming r.size(1) = Nxyz * num_centers
    assert(3 == vgl.size(0));
    assert(nElec == vgl.size(1));
    assert(r.size(1) == vgl.size(2));
    const size_t nRnl = vgl.size(3);

    const size_t total_positions = nElec * Nxyz_per_center * num_centers;
    ...
    for (size_t i_c = 0; i_c < num_centers; ++i_c)
    {
      ...
      {
        for (size_t i_xyz = 0; i_xyz < Nxyz_per_center; ++i_xyz)
        {
          const size_t global_idx = i_xyz + Nxyz_per_center * (i_e + i_c * nElec);

          if (r_ptr[global_idx] >= Rmax)
          {
            for (size_t i = 0; i < nsplines; ++i)
            {
              const size_t vgl_idx = global_idx * nRnl + i;
              u_ptr[vgl_idx]       = 0.0;
              du_ptr[vgl_idx]      = 0.0;
              d2u_ptr[vgl_idx]     = 0.0;
            }
          }
          else if (r_ptr[global_idx] < lower_bound)
          {
            const T dr = r_ptr[global_idx] - lower_bound;
            for (size_t i = 0; i < nsplines; ++i)
            {
              const size_t vgl_idx = global_idx * nRnl + i;
              u_ptr[vgl_idx]       = coeff_ptr[i] + first_deriv_ptr[i] * dr;
              du_ptr[vgl_idx]      = first_deriv_ptr[i];
              d2u_ptr[vgl_idx]     = 0.0;
            }
          }
          else
          {
            int loc;
            const auto cL = LogGridLight<T>::getCL(r_ptr[global_idx], loc, one_over_log_delta, lower_bound, dlog_ratio);
            const size_t offset = loc * 6;
            ...
            {
              const size_t vgl_idx = global_idx * nRnl + i;
              u_ptr[vgl_idx]       = a[i] + cL * (b[i] + cL * (c[i] + cL * (d[i] + cL * (e[i] + cL * f[i]))));
              du_ptr[vgl_idx]      = b[i] + cL * (2 * c[i] + cL * (3 * d[i] + cL * (4 * e[i] + cL * 5 * f[i])));
              d2u_ptr[vgl_idx]     = 2 * c[i] + cL * (6 * d[i] + cL * (12 * e[i] + cL * 20 * f[i]));
            }
          }
        }
      }
