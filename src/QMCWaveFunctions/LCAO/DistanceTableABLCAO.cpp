                                      ...
                                      OffloadPinnedVector<RealType>& Tv_list)
{
  ...
  const auto& coords_leader = dynamic_cast<const RealSpacePositionsOMPTarget&>(elec_list.getLeader().getCoordinates());
  ...
  // Get the ion stride - critical!
  const auto& ion_rsoa    = ion_coords.getAllParticlePos();
  const size_t ion_stride = ion_rsoa.capacity(); // This is the RSoA stride for ions
  ...
  // GPU kernel with correct ion indexing
  ...
  for (int iw = 0; iw < static_cast<int>(nw); ++iw)
    for (int c = 0; c < num_centers; ++c)
      for (int d = 0; d < 3; ++d)
      {
        ...
        //RSoA format for ions
        const RealType ion_pos = ion_ptr[c + d * ion_stride];
