  ...
  template<class T>
    struct DPadeDBFunctor:public OptimizableFunctorBase<T> {
      ...
      void resetParameters(const OptimizableSetType& optVariables) 
      {
        ...
        if(it_b != optVariables.end()) B=(*it_b).second;
        reset(A,B);
      }
    };
      ...
      bool put(xmlNodePtr cur) {
        ...
        while(cur != NULL) {
          ...
          if(cname == "parameter")
          {
            ...
            rAttrib.add(iname,"id"); rAttrib.add(iname,"ref"); //accept either id/ref
            rAttrib.put(cur);
            if(pname[0] == 'A')
            {
              ID_A=iname;
 ...
 * $Revision: 1763 $   $Date: 2007-02-17 17:25:48 -0600 (Sat, 17 Feb 2007) $
 * $Id: PadeFunctors.h 1763 2007-02-17 23:25:48Z jnkim $ 
 ***************************************************************************/
