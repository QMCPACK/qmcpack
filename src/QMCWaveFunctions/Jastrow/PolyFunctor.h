  ...
  template<class T>
    struct PolyFunctor: public OptimizableFunctorBase<T> {
      ...
      inline real_type evaluate(real_type r) {
        ...
        real_type rp=r;
        real_type s=C[0];
        for(int i=1; i<N; i++) {s += C[i]*rp; rp*=r;}
      ...
      inline real_type df(real_type r) {
        ...
        real_type ds=dC[1];
        for(int i=2; i<N; i++) {real_type rp2=rp*r; s+=C[i]*rp2; ds += dC[i]*rp; rp=rp2;}
        real_type x=(r-L)*OneOverL;
      ...
      {
        for(int i=0; i<N; i++) vlist[ID_C[i]]=C[i];
      }
      ...
      inline void resetInternals()
      {
        ...
        for(int i=2; i<N; i++) dC[i]=static_cast<T>(i*(i-1))*C[i];
      }
