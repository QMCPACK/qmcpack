...
(PointerPool<gpu::device_vector<CTA::RealType> > &pool)
{
}
...
TwoBodyJastrowOrbitalBspline<FT>::addFunc(int ia, int ib, FT* j)
{
  ...
  CudaSpline<CTA::RealType> *newSpline = new CudaSpline<CTA::RealType>(*j);
                                      ...
                                      std::vector<RealType> &logPsi)
{
  ...
  for (int iw=0; iw<walkers.size(); iw++)
  {
    ...
    SumHost[iw] = 0.0;
    CTA::RealType *dest = (CTA::RealType*)walker.R_GPU.data();
  }
  SumGPU = SumHost;
...
// 	  }
      CudaSpline<CTA::RealType> &spline = *(GPUSplines[group1*this->NumGroups+group2]);
      if (UsePBC)
...
{
  // for (int iw=0; iw<walkers.size(); iw++)
  //   UpdateListHost[iw] = (CTA::RealType*)walkers[iw]->R_GPU.data();
 ...
 std::vector<ValueType> &lapl)
{
  ...
  for (int group=0; group<PtclRef.groups(); group++)
  {
    ...
    CudaSpline<CTA::RealType> &spline = *(GPUSplines[group*this->NumGroups+newGroup]);
    // two_body_ratio (W.RList_GPU.data(), first, last,
    // 		      (CTA::RealType*)W.Rnew_GPU.data(), iat,
    // 		      spline.coefs.data(), spline.coefs.size(),
    // 		      spline.rMax, L.data(), Linv.data(),
    // 		      SumGPU.data(), walkers.size());
    if (UsePBC)
...
{
  CTA::RealType sim_cell_radius = W.Lattice.SimulationCellRadius;
  std::vector<Walker_t*> &walkers = W.WalkerList;
  ...
  for (int ijob=0; ijob < njobs; ijob++)
  {
    ...
    NLjobGPU<CTA::RealType> &jobGPU = NL_JobListHost[ijob];
    jobGPU.R             = (CTA::RealType*)walkers[job.walker]->R_GPU.data();
...
#ifdef CUDA_DEBUG
  std::vector<CTA::RealType> CPU_GradLapl(4*this->N);
  DTD_BConds<double,3,SUPERCELL_BULK> bconds;
  int iw = 0;
  for (int group1=0; group1<PtclRef.groups(); group1++)
  {
