...
#include "OneBodyJastrowOrbitalBsplineAoS.h"
#include "CudaSpline.h"
#include "Lattice/ParticleBConds.h"

namespace qmcplusplus
{
void OneBodyJastrowOrbitalBsplineAoS::recompute(MCWalkerConfiguration& W, bool firstTime) {}

void OneBodyJastrowOrbitalBsplineAoS::reserve(PointerPool<gpu::device_vector<CTS::RealType>>& pool) {}

void OneBodyJastrowOrbitalBsplineAoS::checkInVariables(opt_variables_type& active)
{
  OneBodyJastrowOrbital<BsplineFunctor<WaveFunctionComponent::RealType>>::checkInVariables(active);
  for (int i = 0; i < NumCenterGroups; i++)
    GPUSplines[i]->set(*Funique[i]);
}

void OneBodyJastrowOrbitalBsplineAoS::addFunc(int ig, FT* j, int jg)
{
...
void OneBodyJastrowOrbitalBsplineAoS::addLog(MCWalkerConfiguration& W, std::vector<RealType>& logPsi)
{
...
void OneBodyJastrowOrbitalBsplineAoS::update(MCWalkerConfiguration* W,
...
void OneBodyJastrowOrbitalBsplineAoS::ratio(MCWalkerConfiguration& W,
                                            int iat,
...
void OneBodyJastrowOrbitalBsplineAoS::calcRatio(MCWalkerConfiguration& W,
                                                int iat,
                                                std::vector<ValueType>& psi_ratios,
                                                std::vector<GradType>& grad,
                                                std::vector<ValueType>& lapl)
...
void OneBodyJastrowOrbitalBsplineAoS::addRatio(MCWalkerConfiguration& W,
                                               int iat,
                                               int k,
                                               std::vector<ValueType>& psi_ratios,
                                               std::vector<GradType>& grad,
...
void OneBodyJastrowOrbitalBsplineAoS::NLratios(MCWalkerConfiguration& W,
                                               ...
                                               std::vector<ValueType>& psi_ratios)
{
  ...
  for (int group = 0; group < NumCenterGroups; group++)
  {
    ...
    if (GPUSplines[group])
    {
      CudaSpline<CTS::RealType>& spline = *(GPUSplines[group]);
      if (UsePBC)
        one_body_NLratios_PBC(NL_JobListGPU.data(),
                              C.data(),
                              first,
                              last,
                              spline.coefs.data(),
        ...
        one_body_NLratios(NL_JobListGPU.data(),
                          C.data(),
                          first,
                          last,
                          spline.coefs.data(),
...
void OneBodyJastrowOrbitalBsplineAoS::calcGradient(MCWalkerConfiguration& W,
                                                   int iat,
                                                   int k,
                                                   std::vector<GradType>& grad)
{
  ...
  for (int group = 0; group < NumCenterGroups; group++)
  {
    ...
    if (GPUSplines[group])
    {
      CudaSpline<CTS::RealType>& spline = *(GPUSplines[group]);
      if (UsePBC)
        one_body_gradient_PBC(W.RList_GPU.data(),
                              iat,
                              C.data(),
                              first,
                              last,
        ...
        one_body_gradient(W.RList_GPU.data(),
                          iat,
                          C.data(),
                          first,
                          last,
...
void OneBodyJastrowOrbitalBsplineAoS::addGradient(MCWalkerConfiguration& W, int iat, std::vector<GradType>& grad)
{
  std::vector<Walker_t*>& walkers = W.WalkerList;
...
void OneBodyJastrowOrbitalBsplineAoS::gradLapl(MCWalkerConfiguration& W, GradMatrix_t& grad, ValueMatrix_t& lapl)
{
  std::vector<Walker_t*>& walkers = W.WalkerList;
  ...
  for (int iw = 0; iw < walkers.size(); iw++)
  {
    for (int ptcl = 0; ptcl < N; ptcl++)
    {
      ...
      {
        fprintf(stderr, "NAN in OneBodyJastrowOrbitalBsplineAoS laplacian.\n");
        abort();
      }
...
void OneBodyJastrowOrbitalBsplineAoS::resetParameters(const opt_variables_type& active)
{
  OneBodyJastrowOrbital<BsplineFunctor<WaveFunctionComponent::RealType>>::resetParameters(active);
  for (int i = 0; i < NumCenterGroups; i++)
    GPUSplines[i]->set(*Funique[i]);
}

void OneBodyJastrowOrbitalBsplineAoS::evaluateDerivatives(MCWalkerConfiguration& W,
