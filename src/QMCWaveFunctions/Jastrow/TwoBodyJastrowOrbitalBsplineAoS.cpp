...
#include "TwoBodyJastrowOrbitalBsplineAoS.h"
...
{
void TwoBodyJastrowOrbitalBsplineAoS::freeGPUmem()
{
  UpdateListGPU.clear();
  SumGPU.clear();
  GradLaplGPU.clear();
...
void TwoBodyJastrowOrbitalBsplineAoS::recompute(MCWalkerConfiguration& W, bool firstTime) {}

void TwoBodyJastrowOrbitalBsplineAoS::reserve(PointerPool<gpu::device_vector<CTS::RealType>>& pool) {}

void TwoBodyJastrowOrbitalBsplineAoS::checkInVariables(opt_variables_type& active)
{
...
void TwoBodyJastrowOrbitalBsplineAoS::addFunc(int ia, int ib, FT* j)
{
...
void TwoBodyJastrowOrbitalBsplineAoS::addLog(MCWalkerConfiguration& W, std::vector<RealType>& logPsi)
{
...
void TwoBodyJastrowOrbitalBsplineAoS::update(MCWalkerConfiguration* W,
...
void TwoBodyJastrowOrbitalBsplineAoS::ratio(MCWalkerConfiguration& W,
                                            int iat,
...
// This currently does not actually compute the gradient or laplacian
void TwoBodyJastrowOrbitalBsplineAoS::calcRatio(MCWalkerConfiguration& W,
                                                int iat,
                                                std::vector<ValueType>& psi_ratios,
                                                std::vector<GradType>& grad,
                                                std::vector<ValueType>& lapl)
...
void TwoBodyJastrowOrbitalBsplineAoS::addRatio(MCWalkerConfiguration& W,
                                               int iat,
                                               int k,
                                               std::vector<ValueType>& psi_ratios,
                                               std::vector<GradType>& grad,
...
void TwoBodyJastrowOrbitalBsplineAoS::NLratios(MCWalkerConfiguration& W,
...
void TwoBodyJastrowOrbitalBsplineAoS::calcGradient(MCWalkerConfiguration& W,
                                                   int iat,
                                                   int k,
                                                   std::vector<GradType>& grad)
{
...
void TwoBodyJastrowOrbitalBsplineAoS::addGradient(MCWalkerConfiguration& W, int iat, std::vector<GradType>& grad)
{
  std::vector<Walker_t*>& walkers = W.WalkerList;
...
void TwoBodyJastrowOrbitalBsplineAoS::gradLapl(MCWalkerConfiguration& W, GradMatrix_t& grad, ValueMatrix_t& lapl)
{
  CTS::RealType sim_cell_radius   = W.Lattice.SimulationCellRadius;
  std::vector<Walker_t*>& walkers = W.WalkerList;
  int numGL                       = 4 * N * walkers.size();
  ...
  for (int iw = 0; iw < walkers.size(); iw++)
  {
    for (int ptcl = 0; ptcl < N; ptcl++)
    {
      ...
      if (std::isnan(GradLaplHost[4 * N * iw + +4 * ptcl + 3]))
      {
        ...
        fprintf(stderr, "NAN in TwoBodyJastrowOrbitalBsplineAoS laplacian.  Host=%s\n", buff);
        abort();
      }
...
void TwoBodyJastrowOrbitalBsplineAoS::resetParameters(const opt_variables_type& active)
{
...
void TwoBodyJastrowOrbitalBsplineAoS::evaluateDerivatives(MCWalkerConfiguration& W,
