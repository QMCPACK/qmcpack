...
#include "TwoBodyJastrowOrbitalBsplineSoA.h"
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::freeGPUmem()
{
  UpdateListGPU.clear();
  SumGPU.clear();
  GradLaplGPU.clear();
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::recompute(MCWalkerConfiguration &W,
                                        bool firstTime)
{
}
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::reserve
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::checkInVariables(opt_variables_type& active)
{
  J2OrbitalSoA<BsplineFunctor<OrbitalBase::RealType> >::checkInVariables(active);
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::addFunc(int ia, int ib, FT* j)
{
  J2OrbitalSoA<BsplineFunctor<OrbitalBase::RealType> >::addFunc(ia, ib, j);
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::addLog (MCWalkerConfiguration &W,
...
// 	  }
      CudaSpline<CudaReal> &spline = *(GPUSplines[group1*this->NumGroups+group2]);
      if (UsePBC)
...
//     fprintf (stderr, "host = %25.16f\n", host_sum);
  ...
  template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::update (std::vector<Walker_t*> &walkers, int iat)
{
  // for (int iw=0; iw<walkers.size(); iw++)
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::ratio
 ...
 std::vector<ValueType> &lapl)
{
  ...
  for (int group=0; group<PtclRef.groups(); group++)
  {
    ...
    CudaSpline<CudaReal> &spline = *(GPUSplines[group*this->NumGroups+newGroup]);
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::calcRatio
(MCWalkerConfiguration &W, int iat,
 std::vector<ValueType> &psi_ratios, std::vector<GradType>  &grad,
 std::vector<ValueType> &lapl)
{
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::addRatio
(MCWalkerConfiguration &W, int iat,
 std::vector<ValueType> &psi_ratios, std::vector<GradType>  &grad,
 std::vector<ValueType> &lapl)
{
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::NLratios
...
template<class FT>
void TwoBodyJastrowOrbitalBsplineSoA<FT>::calcGradient(MCWalkerConfiguration &W, int iat,
...
template<class FT>
void TwoBodyJastrowOrbitalBsplineSoA<FT>::addGradient(MCWalkerConfiguration &W, int iat,
    std::vector<GradType> &grad)
{
  std::vector<Walker_t*> &walkers = W.WalkerList;
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::gradLapl (MCWalkerConfiguration &W,
                                        GradMatrix_t &grad,
                                        ValueMatrix_t &lapl)
{
  CudaReal sim_cell_radius = W.Lattice.SimulationCellRadius;
...
#ifdef CUDA_DEBUG
  std::vector<CudaReal> CPU_GradLapl(4*this->N);
  DTD_BConds<double,3,SUPERCELL_BULK> bconds;
  int iw = 0;
  for (int group1=0; group1<PtclRef.groups(); group1++)
  {
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::resetParameters(const opt_variables_type& active)
{
  J2OrbitalSoA<BsplineFunctor<OrbitalBase::RealType> >::resetParameters(active);
  for (int i=0; i<this->NumGroups*this->NumGroups; i++)
    GPUSplines[i]->set(*this->F[i]);
}
...
template<class FT>
void
TwoBodyJastrowOrbitalBsplineSoA<FT>::evaluateDerivatives
(MCWalkerConfiguration &W, const opt_variables_type& optvars,
 TwoBodyJastrowOrbitalBsplineSoA<FT>::RealMatrix_t &d_logpsi,
 TwoBodyJastrowOrbitalBsplineSoA<FT>::RealMatrix_t &dlapl_over_psi)
{
  CudaReal sim_cell_radius = W.Lattice.SimulationCellRadius;
  std::vector<Walker_t*> &walkers = W.WalkerList;
  int nw = walkers.size();
...
template class TwoBodyJastrowOrbitalBsplineSoA<BsplineFunctor<double>>;
