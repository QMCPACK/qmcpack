...
///
///////////////////////////////////////////////////////////////////////////////////////////////
FDLRWfn::FDLRWfn(TrialWaveFunction* const wfn_1,
                 TrialWaveFunction* const wfn_2,
                 ParticleSet& P,
                 const bool opt_x,
                 const bool opt_d,
...
///
///////////////////////////////////////////////////////////////////////////////////////////////
void FDLRWfn::extract_xpd_and_xmd_vars(const opt_variables_type& active,
                                       opt_variables_type& xpd_vars,
                                       opt_variables_type& xmd_vars)
{
...
void FDLRWfn::reportStatus(std::ostream& os) { throw std::runtime_error("FDLRWfn::reportStatus not yet implemented"); }
                                       ...
                                       ParticleSet::ParticleLaplacian_t& L)
{
  ...
  // wave functions.
  LogValue = evaluateLogFDLR(P, G, L, m_wfn_xpd->getLogPsi(), m_wfn_xmd->getLogPsi(), m_wfn_xpd->getPhase(),
                                  ...
                                  std::vector<FDLRWfn::RealType>& dhpsioverpsi)
{
  ...
  if (opt_x_vars)
  {
    for (int i = 0; i < x_vars_driver.size(); i++)
    {
      ...
      dhpsioverpsi_fdlr_x[i] = scaling_fac_1 * (dhpsioverpsi_xpd[i] + kinetic_plus * dlogpsi_xpd[i]) -
          scaling_fac_2 * (dhpsioverpsi_xmd[i] + kinetic_minus * dlogpsi_xmd[i]) - kinetic_FDLR * dlogpsi_fdlr_x[i] -
  ...
  if (opt_d_vars)
  {
    for (int i = 0; i < d_vars_driver.size(); i++)
    {
      ...
      dhpsioverpsi_fdlr_d[i] = scaling_fac_1 * (dhpsioverpsi_xpd[i] + kinetic_plus * dlogpsi_xpd[i]) +
          scaling_fac_2 * (dhpsioverpsi_xmd[i] + kinetic_minus * dlogpsi_xmd[i]) - kinetic_FDLR * dlogpsi_fdlr_d[i] -
  ...
  if (opt_x_vars)
  {
    ...
    std::copy(dhpsioverpsi_fdlr_x.begin(), dhpsioverpsi_fdlr_x.begin() + x_vars_driver.size(),
              dhpsioverpsi.begin() + x_loc);
  }
  if (opt_d_vars)
  {
    int d_loc = d_vars_driver.where(0);
    std::copy(dlogpsi_fdlr_d.begin(), dlogpsi_fdlr_d.begin() + d_vars_driver.size(), dlogpsi.begin() + d_loc);
    std::copy(dhpsioverpsi_fdlr_d.begin(), dhpsioverpsi_fdlr_d.begin() + d_vars_driver.size(),
              dhpsioverpsi.begin() + d_loc);
  }
}
...
void FDLRWfn::evaluateDerivRatios(VirtualParticleSet& VP,
                                  const opt_variables_type& optvars,
                                  std::vector<FDLRWfn::ValueType>& ratios,
                                  Matrix<FDLRWfn::ValueType>& dratios)
{
