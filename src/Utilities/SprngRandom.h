...
public:
  ...
  ~SprngRandom() 
  {
    if(myStream) free_stream(myStream);
  }
...
#error "Unsupported sprng library. Only versions 1 and 2 are supported."
  ...
  inline Return_t getRandom() { return sprng(myStream);}//!< return [0,1)
  inline Return_t operator()() { return getRandom();} //!< return [0,1)
  inline int irand() { return isprng(myStream);} //!< return random integer
  inline void reset() 
  { 
    if(myStream) free_stream(myStream);
    baseSeed = make_sprng_seed();
