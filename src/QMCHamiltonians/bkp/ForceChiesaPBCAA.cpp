...
void ForceChiesaPBCAA::evaluateSR_AA()
{
  ...
  if (d_aa.DTType == DT_SOA)
  {
    for (size_t ipart = 1; ipart < NptclA; ipart++)
    {
      ...
      {
        RealType rinv = RealType(1) / dist[jpart];
        RealType V    = -AB->srDf(dist[jpart], rinv);
        PosType drhat = rinv*d_aa.Displacements[ipart][jpart];
        PosType grad  = -Zat[jpart]*Zat[ipart]*V*drhat;
