...
bool StaticStructureFactor::put(xmlNodePtr cur)
{
  using std::sqrt;
  reset();
  const auto& k2_init = Pinit.getSimulationCell().getKLists().getKSQ();
...
void StaticStructureFactor::registerCollectables(std::vector<ObservableHelper>& h5desc, hdf_archive& file) const
{
  ...
  auto& oh = h5desc.back();
  oh.addProperty(const_cast<std::vector<PosType>&>(Pinit.getSimulationCell().getKLists().getKptsCart()), "value", file);
