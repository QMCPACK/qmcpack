...
template<typename T>
__device__ T min_dist_only(T x, T y, T z, T L[3][3], T Linv[3][3])
{
  T u0 = Linv[0][0] * x + Linv[0][1] * y + Linv[0][2] * z;
  T u1 = Linv[1][0] * x + Linv[1][1] * y + Linv[1][2] * z;
  T u2 = Linv[2][0] * x + Linv[2][1] * y + Linv[2][2] * z;
...
template<typename T, int BS>
__global__ void find_core_electrons_PBC_kernel(T** R,
                                               int numElec,
                                               T* I,
                                               int firstIon,
                                               int lastIon,
                                               ...
                                               int* numPairs)
{
  ...
  for (int iBlock = 0; iBlock < numIonBlocks; iBlock++)
  {
    ...
    for (int eBlock = 0; eBlock < numElecBlocks; eBlock++)
    {
      ...
      {
        d[tid] = min_dist_only(r[tid][0] - i[ion][0], r[tid][1] - i[ion][1], r[tid][2] - i[ion][2], L, Linv);
...
void find_core_electrons_PBC(float* R[],
                             int numElec,
                             float I[],
                             int firstIon,
                             int lastIon,
                             ...
                             int numWalkers)
{
  ...
  find_core_electrons_PBC_kernel<float, BS>
      <<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, L, Linv, pairs, dist, numPairs);
}

void find_core_electrons_PBC(double* R[],
                             int numElec,
                             double I[],
                             int firstIon,
                             int lastIon,
                             ...
                             int numWalkers)
{
  ...
  find_core_electrons_PBC_kernel<double, BS>
      <<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, L, Linv, pairs, dist, numPairs);
}
        ...
        for (int elec = 0; elec < elecEnd; elec++)
        {
          ...
          if (dist[elec] < rcut)
          {
            ...
            if (numQuadPoints + posIndex <= BS)
            {
              if (tid < numQuadPoints)
              {
                ...
                blockCosTheta[posIndex + tid] =
                    (disp[elec][0] * qp[tid][0] + disp[elec][1] * qp[tid][1] + disp[elec][2] * qp[tid][2]) / dist[elec];
              }
            ...
            else
            {
              ...
              for (int j = 0; j < 3; j++)
                myRatioPos[(posBlockNum * 3 + j) * BS + tid] = blockPos[0][j * BS + tid];
              ...
              if (tid < (numQuadPoints - numWrite))
              {
                ...
                blockCosTheta[tid] = (disp[elec][0] * qp[tid + numWrite][0] + disp[elec][1] * qp[tid + numWrite][1] +
                                      disp[elec][2] * qp[tid + numWrite][2]) /
                    dist[elec];
              }
                             ...
                             int numWalkers)
{
  ...
  find_core_electrons_PBC_kernel<float, BS><<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, L, Linv,
                                                                   quadPoints, numQuadPoints, elecs, ratioPos, dist,
                                                                   cosTheta, numPairs);
}
                             ...
                             int numWalkers)
{
  ...
  find_core_electrons_PBC_kernel<double, BS><<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, L, Linv,
                                                                    quadPoints, numQuadPoints, elecs, ratioPos, dist,
                                                                    cosTheta, numPairs);
}
...
template<typename T, int BS>
__global__ void find_core_electrons_kernel(T** R,
                                           int numElec,
                                           T* I,
                                           int firstIon,
                                           int lastIon,
...
void find_core_electrons(float* R[],
                         int numElec,
                         float I[],
                         int firstIon,
                         int lastIon,
                         ...
                         int numWalkers)
{
  ...
  find_core_electrons_kernel<float, BS>
      <<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, pairs, dist, numPairs);
}

void find_core_electrons(double* R[],
                         int numElec,
                         double I[],
                         int firstIon,
                         int lastIon,
                         ...
                         int numWalkers)
{
  ...
  find_core_electrons_kernel<double, BS>
      <<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, pairs, dist, numPairs);
}
  ...
  for (int iBlock = 0; iBlock < numIonBlocks; iBlock++)
  {
    ...
    for (int eBlock = 0; eBlock < numElecBlocks; eBlock++)
    {
      ...
      for (int ion = 0; ion < ionEnd; ion++)
      {
        ...
        disp[tid][1] = r[tid][1] - i[ion][1];
        disp[tid][2] = r[tid][2] - i[ion][2];
        dist[tid]    = sqrtf(disp[tid][0] * disp[tid][0] + disp[tid][1] * disp[tid][1] + disp[tid][2] * disp[tid][2]);
        for (int elec = 0; elec < elecEnd; elec++)
        {
          __syncthreads();
          if (dist[elec] < rcut)
                         ...
                         int numWalkers)
{
  ...
  find_core_electrons_kernel<float, BS><<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, quadPoints,
                                                               numQuadPoints, elecs, ratioPos, dist, cosTheta,
                                                               numPairs);
}
                         ...
                         int numWalkers)
{
  ...
  find_core_electrons_kernel<double, BS><<<dimGrid, dimBlock>>>(R, numElec, I, firstIon, lastIon, rcut, quadPoints,
                                                                numQuadPoints, elecs, ratioPos, dist, cosTheta,
                                                                numPairs);
}
...
// number
template<typename T, int BS>
__global__ void make_work_list_kernel(int2** pairs,
                                      T** dist,
                                      int* numPairs,
                                      T* I,
                                      int numIons,
