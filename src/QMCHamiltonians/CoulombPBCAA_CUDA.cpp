...
{
#ifdef QMC_CUDA
  gpu::host_vector<CUDA_AA_PRECISION> LHost(9), LinvHost(9);
  for (int i=0; i<3; i++)
    for (int j=0; j<3; j++)
    {
...
CoulombPBCAA_CUDA::setupLongRangeGPU(ParticleSet &P)
{
  if (is_active)
  {
    StructFact &SK = *(P.SK);
    Numk = SK.KLists.numk;
    gpu::host_vector<CUDA_AA_PRECISION> kpointsHost(OHMMS_DIM*Numk);
    for (int ik=0; ik<Numk; ik++)
      for (int dim=0; dim<OHMMS_DIM; dim++)
        kpointsHost[ik*OHMMS_DIM+dim] = SK.KLists.kpts_cart[ik][dim];
    kpointsGPU = kpointsHost;
    gpu::host_vector<CUDA_AA_PRECISION> FkHost(Numk);
    for (int ik=0; ik<Numk; ik++)
      FkHost[ik] = AA->Fk[ik];
    FkGPU = FkHost;
  }
                             ...
                             vector<RealType> &LocalEnergy)
{
  ...
  gpu::host_vector<CUDA_AA_PRECISION> RhokHost;
  RhokHost = RhokGPU;
  for (int ik=0; ik<Numk; ik++)
  {
