...
namespace qmcplusplus {
  ...
  struct observable_helper
  {
    ...
    ~observable_helper()
    {
      if(space1_id>-1) H5Sclose(space1_id);
      if(space2_id>-1) H5Sclose(space2_id);
      if(data_id>-1) H5Gclose(data_id);
    }
    ...
    void set_dimensions(std::vector<int>& dims, int first)
    {
      ...
      mydims.resize(rank,1); std::copy(dims.begin(),dims.end(),mydims.begin()+1);
      maxdims=mydims; 
      curdims=mydims;
