            ...
            for (int i=-kgrid;i<(kgrid+1);i++) for (int j=-kgrid;j<(kgrid+1);j++) for (int k=-kgrid;k<(kgrid+1);k++)
                  {
                    if (std::sqrt(i*i+j*j+k*k)<=kgrid)
                      {
                        PosType kpt;
                        kpt[0]=RealType(i+twist[0])*BasisMatrix(0,0)+RealType(j+twist[1])*BasisMatrix(1,0)+RealType(k+twist[2])*BasisMatrix(2,0);
                        kpt[1]=RealType(i+twist[0])*BasisMatrix(0,1)+RealType(j+twist[1])*BasisMatrix(1,1)+RealType(k+twist[2])*BasisMatrix(2,1);
                        kpt[2]=RealType(i+twist[0])*BasisMatrix(0,2)+RealType(j+twist[1])*BasisMatrix(1,2)+RealType(k+twist[2])*BasisMatrix(2,2);
