...
void LocalECPotential_CUDA::add(int groupID, std::unique_ptr<RadialPotentialType>&& ppot, RealType z)
{
  ...
  if (rfunc != savefunc)
  {
    ...
    SRSplines[groupID]->set(&(scaledData[0]), rfunc->size(), rfunc->grid().rmin(), rfunc->grid().rmax());
  }
}
...
void LocalECPotential_CUDA::addEnergy(MCWalkerConfiguration& W, std::vector<RealType>& LocalEnergy)
{
  ...
  for (int sp = 0; sp < NumIonSpecies; sp++)
  {
    if (SRSplines[sp])
    {
      local_ecp_sum(W.RList_GPU.data(), N, IGPU.data(), IonFirst[sp], IonLast[sp], SRSplines[sp]->rMax,
                    SRSplines[sp]->NumPoints, SRSplines[sp]->MyTexture, SumGPU.data(), nw);
      SumHost = SumGPU;
      for (int iw = 0; iw < nw; iw++)
        esum[iw] += SumHost[iw];
    }
