...
protected:
  ...
  void setupCUDA(ParticleSet &elecs) GPU_XRAY_TRACE;
  void resizeCUDA(int nw) GPU_XRAY_TRACE;

public:
  NonLocalECPotential_CUDA(ParticleSet& ions, ParticleSet& els,
                           TrialWaveFunction& psi, bool usePBC,
                           bool doForces=false) GPU_XRAY_TRACE;

  QMCHamiltonianBase* makeClone(ParticleSet& qp, TrialWaveFunction& psi) GPU_XRAY_TRACE;

  void addEnergy(MCWalkerConfiguration &W, std::vector<RealType> &LocalEnergy) GPU_XRAY_TRACE;
  void addEnergy(MCWalkerConfiguration &W, std::vector<RealType> &LocalEnergy,
                 std::vector<std::vector<NonLocalData> > &Txy) GPU_XRAY_TRACE;
};
