...
#To verify that the formalism is correct, we perform a monte carlo integration w.r.t. s1 and s2 for the magnetization density using the same determinant trial function as stated previously.  Chosing a=cos(theta/2) b=e^(i*phi)sin(theta/2), we find that the spin expectation value for each spin should be [0.5,0.5,1/sqrt(2)].  
