...
void test_splines()
{
  ...
  int num_splines = 4;  // must be multiple of 4 for alignment on BG/Q
  BCtype_d bc[3];
  Ugrid grid[3];
  ...
  REQUIRE(bs.num_splines() == num_splines);
...
//  Code from here to the end of the function is generated by gen_bspline_values.py
  ...
  Array<T, 1> v(num_splines);
  bs.evaluate(pos, v);
  REQUIRE(v(0) == Approx(-3.529930688e-12));

  Array<T, 2> dv(3,num_splines);  // 3 - ndim
  Array<T, 2> hess(6, num_splines); // 6 - number of unique hessian components
  bs.evaluate_vgh(pos, v, dv, hess);
  // Gradient
  REQUIRE(dv(0,0) == Approx(    6.178320809));
  REQUIRE(dv(1,0) == Approx(   -7.402942564));
  REQUIRE(dv(2,0) == Approx(   -6.178320809));

  // Hessian
  for (int i = 0; i < 6; i++) {
    REQUIRE(hess(i,0) == Approx(0.0));
  }
  ...
  REQUIRE(dv(1,0) == Approx(    5.989106342));
  REQUIRE(dv(2,0) == Approx(    1.952244379));
  // Hessian
  REQUIRE(hess(0,0) == Approx(   -21.34557341));
  REQUIRE(hess(1,0) == Approx(1.174505743e-09));
  REQUIRE(hess(2,0) == Approx( -1.1483271e-09));
  REQUIRE(hess(3,0) == Approx(    133.9204891));
  REQUIRE(hess(4,0) == Approx(-2.15319293e-09));
  REQUIRE(hess(5,0) == Approx(    34.53786329));


  Array<T, 2> lap(3,num_splines);
