...
def gen_coeffs_1D() -> None:
    """Construct and solve the matrix equation for the coefficients.

    This produces a minimal length coefficient array. For uniformity of
    access, the actual coefficient array has the first three values
    replicated onto the end.
    """
    xs = Symbol("x")
                ...
                for jx in range(m):
                    for jy in range(m):
                        for jz in range(m):
                            ...
                            e = (
                                spline_x.subs(subslist_x)
                                * spline_y.subs(subslist_y)
                                * spline_z.subs(subslist_z)
                            )
                            # print j,i,e
                            mat[j, i] = e
                print(j, i, e)
...
def read_coefs() -> tuple[list, dict]:
    coef_list = list()
    coef_map = dict()
...
def evaluate_spline_3D():
    """Evaluate the spline and derivatives at various points and
    generate code for ``test_splines()`` in ``test_multi_spline.cpp``.
    """
    xs = Symbol("x")
    ys = Symbol("y")
    zs = Symbol("z")
    ...
    print(
        "  VectorSoaContainer<T,6> hess(npad); // 6 - number of unique hessian components"
    )
