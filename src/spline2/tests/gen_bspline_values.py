...
def evaluate_spline_3D():
  ...
  pos = {xs:0, ys:0, zs:0}
  print 'pos = %g, %g, %g'%(pos[xs], pos[ys], pos[zs])
  print ' symbolic value = ',e.subs(pos)

  print ' numeric value = ',e.subs(pos).expand().subs(subslist)
  # the 'expand' after substitution pos is necessary for the triples of coefficients (cx*cy*cz) to
  # be be formed so the substitution of coefficients works
