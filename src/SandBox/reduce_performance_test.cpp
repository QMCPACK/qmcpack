...
int main(int argc, char* argv[])
{
  ...
  int color=value/2;
  mpi::communicator twoway=world.split(color);
  int result;
  ...
  for (int outer = 0; outer < outer_repeat_count; ++outer)
  {
    ...
    double reduce_raw_mpi_total_time = (end - start);
    ...
    double reduce_int_sum_total_time = (end - start);
    // MPI_INT/MPI_Op case
    start = env.wtime();
    for (int i = 0; i < repeat_count; ++i)
    {
    ...
    double reduce_int_op_total_time = (end - start);
    ...
    double reduce_type_op_total_time = (end - start);
    // Serialized/MPI_Op case
    start = env.wtime();
    for (int i = 0; i < repeat_count; ++i)
    {
      //reduce(world, si_value, si_result, std::plus<serialized_int>(), 0);
      reduce(twoway, si_value, si_result, std::plus<serialized_int>(), 0);
    }
    end = env.wtime();
    double reduce_ser_op_total_time = (end - start);
    if (world.rank() == 0)
  ...
  std::cout << " Using color " << twoway.size() << " " << twoway.rank() << " " << si_value.value << " " << si_result.value << std::endl;
  return 0;
}
