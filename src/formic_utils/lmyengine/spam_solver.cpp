...
const formic::Matrix<double> cqmc::engine::SpamLMHD::debug_build()
{
  //formic::Matrix<double> X = _kvecs.leftCols(_nkry_full);
  //formic::Matrix<double> P = X * X.transpose();
  //formic::Matrix<double> Q = formic::Matrix<double>::Identity(P.rows(), P.cols()) - P;
  //formic::Matrix<double> H = _der_rat.transpose() * _le_der;
  //formic::Matrix<double> S = _der_rat.transpose() * _der_rat;
  //int Ns = _der_rat.rows() / _appro_degree;
  //formic::Matrix<double> H_bar = (_der_rat.topRows(Ns)).transpose() * _le_der.topRows(Ns) * _appro_factor;
  //formic::Matrix<double> H_hy = P * H * P + P * H * Q + Q * H * P + Q * H_bar * Q;
  //formic::Matrix<double> S_bar = (_der_rat.topRows(Ns)).transpose() * _der_rat.topRows(Ns) * _appro_factor;
  //formic::Matrix<double> S_hy = P * S * P + P * S * Q + Q * S * P + Q * S_bar * Q;
  //for (int i = 0; i < H_hy.rows(); i++) { 
  //  for (int j = 0; j < H_hy.cols(); j++) {
  //    std::cout << boost::format("%10.8e  ") % H_hy(i, j);
  //  }
  //  std::cout << std::endl;
  //}
  
  //std::cout << std::endl;
  //for (int i = 0; i < S_hy.rows(); i++) {
  //  for (int j = 0; j < S_hy.cols(); j++) { 
  //    std::cout << boost::format("%10.8e  ") % S_hy(i, j);
  //  }
  //  std::cout << std::endl;
  //}
