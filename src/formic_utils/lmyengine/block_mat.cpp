...
void cqmc::engine::LMBlockerMatData::prep_lm_block_plus_other_ou_matrix(const int b, const int x, formic::Matrix<double> & mat) {
  ...
  // <wfn|var>
  for (int i = 0; i < len; i++) 
    mat.at(0, 1+i) = m_wv.at(b).at(i);

  // <var|wfn>
  for (int i = 0; i < len; i++)
    mat.at(1+i, 0) = m_vw.at(b).at(i);

  // <var|var>
  for (int i = 0; i < len; i++) {
    for (int j = 0; j < len; j++) {
      mat.at(1+i,1+j) = m_vv.at(b).at(i,j);
    }
  }

  // <wfn|old_update>
  for (int k = 0; k < m_nou; k++) 
    mat.at(0, 1+len+k) = m_wo.at(x).at(k);

  // <old_update_wfn>
  for (int k = 0; k < m_nou; k++)
    mat.at(1+len+k, 0) = m_ow.at(x).at(k);

  // <var|old_update>
  for (int i = 0; i < len; i++) {
    for (int k = 0; k < m_nou; k++) {
      mat.at(1+i, 1+len+k) = m_vo.at(b).at(i, y*m_nou+k);
    }
  }
  ...
  for (int k = 0; k < m_nou; k++) {
    for (int i = 0; i < len; i++) {
      mat.at(1+len+k, 1+i) = m_ov.at(b).at(y*m_nou+k, i);
    }
  }
  ...
  for (int k = 0; k < m_nou; k++) {
    for (int l = 0; l < m_nou; l++) {
      mat.at(1+len+k, 1+len+l) = m_oo.at(x).at(k,l);
    }
  }
}
...
void cqmc::engine::LMBlockerMatData::acc(const double d, const std::vector<double> & lr, const std::vector<double> &rr, const formic::Matrix<double> & ou_mat) {
  ...
  if ( lr.size() != 1 + this->tot_rows(m_vv) )
    throw formic::Exception("lr vector length was %i but should be %i LMBlockerMatData::acc") % lr.size() % ( 1 + this->tot_rows(m_vv) );
  if ( rr.size() != 1 + this->tot_rows(m_vv) )
    throw formic::Exception("rr vector length was %i but should be %i LMBlockerMatData::acc") % rr.size() % ( 1 + this->tot_rows(m_vv) );

  // prep intermediates for old updates
  this->prep_block_ou_contractions(lr, ou_mat, m_boulr);
  this->prep_block_ou_contractions(rr, ou_mat, m_bourr);

  // <wfn|wfn>
  m_ww += d * lr.at(0) * rr.at(0);
  ...
  for (int b = 0; b < this->nb(); b++) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      // <wfn|var>
      m_wv.at(b).at(i) += d * lr.at(0) * rr.at(ibeg + i);

      // <var|wfn>
      m_vw.at(b).at(i) += d * lr.at(ibeg + i) * rr.at(0);
    ...
    for (int k = 0; k < m_nou; k++) {
      ...
      // <wfn|old_updates>
      m_wo.at(b).at(k) += d * lr.at(0) * m_bourr.at(b).at(k);
      
      // <old_updates|wfn>
      m_ow.at(b).at(k) += d * m_boulr.at(b).at(k) * rr.at(0);
    }

    // <var|var>
    { formic::Matrix<double> & vv_mat = m_vv.at(b);
      for (int i = 0; i < len; i++) {
        for (int j = 0; j < len; j++) {
    ...
    { formic::Matrix<double> & vo_mat = m_vo.at(b);
      for (int x = 0, y = 0; x < this->nb(); x++) {
        if ( x == b )
          continue;
        const formic::ColVec<double> & rr_vec = m_bourr.at(x);
     ...
     { formic::Matrix<double> & ov_mat = m_ov.at(b);
       for (int i = 0; i < len; i++) {
         for (int x = 0, y = 0; x < this->nb(); x++) {
           if ( x == b ) 
             continue;
           const formic::ColVec<double> & lr_vec = m_boulr.at(x);
           for (int k = 0; k < m_nou; k++) {
     ...
     // <old_update|old_update? 
     { formic::Matrix<double> & oo_mat = m_oo.at(b);
       for (int k = 0; k < m_nou; k++) {
         for (int l = 0; l < m_nou; l++) {
           oo_mat.at(k,l) += d * m_boulr.at(b).at(k) * m_bourr.at(b).at(l);
         }
       }
     }
   }
...
void cqmc::engine::LMBlockerMatData::finalize(const double total_weight) {
  ...
  // loop over blocks
  for (int b = 0; b < this->nb(); b++) {
    m_wv.at(b) /= total_weight;
    m_vw.at(b) /= total_weight;
    m_vv.at(b) /= total_weight;
    m_wo.at(b) /= total_weight;
    m_ow.at(b) /= total_weight;
    m_ov.at(b) /= total_weight;
    m_vo.at(b) /= total_weight;
    m_oo.at(b) /= total_weight;
  }
}
...
void cqmc::engine::LMBlockerMatData::mpi_finalize(const double total_weight) {
  ...
  if ( my_rank == 0 ) {
    ...
    // loop over blocks
    for (int b = 0; b < this->nb(); b++) {
      m_vw.at(b) = m_vw_tot.at(b) / total_weight;
      m_wv.at(b) = m_wv_tot.at(b) / total_weight;
      m_vv.at(b) = m_vv_tot.at(b) / total_weight;
      m_ow.at(b) = m_ow_tot.at(b) / total_weight;
      m_wo.at(b) = m_wo_tot.at(b) / total_weight;
      m_vo.at(b) = m_vo_tot.at(b) / total_weight;
      m_ov.at(b) = m_ov_tot.at(b) / total_weight;
      m_oo.at(b) = m_oo_tot.at(b) / total_weight;
      // print out to debug
      //for (int i = 0; i < m_ow.at(b).size(); i++) {
      //  std::cout << m_ow.at(b).at(i) << "   ";
      //}
...
void cqmc::engine::LMBlockerMatData::reset(const int nv, const int nblock, const int nou) {
  ...
  m_wv.clear();
  for (int i = 0; i < this->nb(); i++)
    m_wv.push_back(formic::ColVec<double>(this->bl(i), 0.0));

  m_vw.clear();
  for (int i = 0; i < this->nb(); i++) 
    m_vw.push_back(formic::ColVec<double>(this->bl(i), 0.0));

  m_vv.clear();
  for (int i = 0; i < this->nb(); i++)
    m_vv.push_back(formic::Matrix<double>(this->bl(i), this->bl(i), 0.0));

  m_wo.clear();
  for (int i = 0; i < this->nb(); i++) 
    m_wo.push_back(formic::ColVec<double>(m_nou, 0.0));

  m_ow.clear();
  for (int i = 0; i < this->nb(); i++) 
    m_ow.push_back(formic::ColVec<double>(m_nou, 0.0));

  m_vo.clear();
  for (int i = 0; i < this->nb(); i++) 
    m_vo.push_back(formic::Matrix<double>(this->bl(i), (this->nb()-1)*m_nou, 0.0));

  m_ov.clear();
  for (int i = 0; i < this->nb(); i++)
    m_ov.push_back(formic::Matrix<double>((this->nb()-1)*m_nou, this->bl(i), 0.0));

  m_oo.clear();
  for (int i = 0; i < this->nb(); i++)
    m_oo.push_back(formic::Matrix<double>(m_nou, m_nou, 0.0));

  m_boulr.clear();
  m_bourr.clear();
  for (int i = 0; i < this->nb(); i++) {
    m_boulr.push_back(formic::ColVec<double>(m_nou, 0.0));
    m_bourr.push_back(formic::ColVec<double>(m_nou, 0.0));
  }
}
