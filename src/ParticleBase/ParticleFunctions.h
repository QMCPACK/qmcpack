...
bool ExpandSuperCell(PT& in_, PT& out_, GIV& uc_grid, GIM& grid)
{
  //Prepare lattice
...
template<class PT, class GIV, class GIM>
bool ExpandSuperCellOMP(PT& in_, PT& out_, GIV& uc_grid, GIM& grid)
{
  ///Prepare lattice
  out_.Lattice.set(in_.Lattice, &uc_grid[0]);
  typename PT::ParticleLayout_t& lattice = out_.Lattice;
...
inline bool ExpandSuperCell(PT& inout_, GIV& uc_grid)
{
  PT temp(inout_);
