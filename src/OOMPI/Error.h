...
public:
  ...
  inline OOMPI_Comm &Get_comm(void) { return *oompi_comm; };
  inline int Get_code(void) { return code; }; 
  inline int Get_class(void) 
    { int c; MPI_Error_class(code, &c); return c; };
  inline char *Get_string(void)
    { char *msg = new char[MPI_MAX_ERROR_STRING]; int len;
      MPI_Error_string(code, msg, &len); return msg; };
