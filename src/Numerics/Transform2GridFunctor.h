...
template<class FnIn, class FnOut>
Transform2GridFunctorBase<FnOut>* createTransform2GridFunctor(FnIn& in, FnOut& out)
{
...
struct TestTransform
{
  ...
  static void check(FnIn& af, FnOut& nf, point_type ri, point_type rf, int n)
  {
    ...
    Transform2GridFunctorBase<FnOut>* transformer = createTransform2GridFunctor(af, nf);
    /// In = out/r^np
    ...
    for (int i = 1; i < nf.size() - 1; i++)
    {
      ...
      std::cout << std::setw(10) << x0 << std::setw(20) << std::setprecision(10) << y << std::setw(20) << (y - z) / z
                << std::setw(20) << yp << std::setw(20) << (yp - zp) / zp << std::setw(20) << y2p << std::setw(20)
                << (y2p - z2p) / z2p << std::endl;
    }
    delete transformer;
  }
};
