    ...
    bool openend)
{
  ...
  BC0.init(Nx,xi,xf); dx=BC0.Delta;dxInv=BC0.OneOverDelta;
  BC1.init(Ny,yi,yf); dy=BC1.Delta;dyInv=BC1.OneOverDelta;
  BC2.init(Nz,zi,zf); dz=BC2.Delta;dzInv=BC2.OneOverDelta;
}

template<typename T, bool PBC0, bool PBC1, bool PBC2>
inline bool TricubicBsplineGridBC<T,PBC0,PBC1,PBC2>::Find(real_type x, real_type y, real_type z)
{
  if(BC0.outofbound(x,ix0)) return false;
  ix1=ix0+1; ix2=ix0+2; ix3=ix0+3;
  if(BC1.outofbound(y,iy0)) return false;
  iy1=iy0+1; iy2=iy0+2; iy3=iy0+3;
  if(BC2.outofbound(z,iz0)) return false;
