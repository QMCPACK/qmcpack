...
QuinticSplineSolve(int N, const Tg* X, T* Y, T* B, T* C, T* D, T* E, T* F) {
  ...
  QR=Q+R;
  D[0]=E[0]=D[1]=0.0;
  if(std::abs(Q) > eps) D[1]=6.0*Q*Q2/(QR*QR);
  for(int i=1; i<M; i++) {
    P=Q; 
    Q=R;
    R=X[i+2]-X[i+1]; 
  ...
  if(std::abs(R) > eps) D[N-3] += 6.0*R*R2/(QR*QR);
  for(int i=1; i<N; i++) { 
  ...
  F[0]=0.0;
  if(std::abs(Q) > eps ) F[0]=V/Q;
  for(int i=1; i<M; i++) {
    P=Q;
    Q=R;
    R=0.0;
    if(i != (M-1)) R=X[i+2]-X[i+1];
    P3=Q3;
    Q3=Q*Q*Q;
    PQ=QR;
    QR=Q+R; 
    S=TT;
    TT=0;
    if(std::abs(QR) > eps )TT=(C[i+1]-C[i])/QR;
    U=V;    
    V=TT-S;
    if(std::abs(PQ) < eps ) { 
      C[i]=C[i-1];
