...
struct HDFAttribIO<double> : public HDFAttribIOBase
{
  ...
  inline void write(hid_t grp, const char* name) override
  {
    ...
    hid_t ret       = H5Dwrite(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &ref);
    H5Sclose(dataspace);
    H5Dclose(dataset);
  }
...
struct HDFAttribIO<std::vector<TinyVector<int, D>>> : public HDFAttribIOBase
{
  ...
  inline void write(hid_t grp, const char* name) override
  {
    ...
    hid_t dataspace = H5Screate_simple(2, dim, NULL);
    hid_t dataset   = H5Dcreate(grp, name, H5T_NATIVE_INT, dataspace, H5P_DEFAULT);
    hid_t ret       = H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(ref[0][0]));
    H5Sclose(dataspace);
    H5Dclose(dataset);
  }
...
struct HDFAttribIO<Vector<int>> : public HDFAttribIOBase
{
  ...
  inline void write(hid_t grp, const char* name) override
  {
    ...
    hid_t dataspace = H5Screate_simple(1, &dim, NULL);
    hid_t dataset   = H5Dcreate(grp, name, H5T_NATIVE_INT, dataspace, H5P_DEFAULT);
    hid_t ret       = H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, ref.data());
    H5Sclose(dataspace);
    H5Dclose(dataset);
  }
