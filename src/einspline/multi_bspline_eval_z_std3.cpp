...
eval_multi_UBspline_3d_z (const multi_UBspline_3d_z *spline,
                          ...
                          complex_double* restrict vals)
{
  // wrapper function by Ye Luo, Mar. 12th 2015
  multi_UBspline_3d_d wrapper_spline;
  
  wrapper_spline.x_grid.start = spline->x_grid.start;
  wrapper_spline.y_grid.start = spline->y_grid.start;
  wrapper_spline.z_grid.start = spline->z_grid.start;
  ...
  wrapper_spline.num_splines = spline->num_splines*2;

  wrapper_spline.coefs = (double*) spline->coefs;

  eval_multi_UBspline_3d_d(&wrapper_spline, x, y, z, (double*) vals);
}
			      ...
			      complex_double* restrict hess)
{
  ...
  eval_multi_UBspline_3d_d_vgh(&wrapper_spline, x, y, z, (double*) vals, (double*) grads, (double*) hess);

  double* h = (double*) hess;
  double* g = (double*) grads;

  for(int i=0; i < spline->num_splines*2; i+=2){
    double temp_swap;
