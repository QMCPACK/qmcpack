...
#pragma omp master
    ...
    J2OrbitalSoA<BsplineFunctorSoA<RealType> > J2(els,ip);
    TwoBodyJastrowOrbital<BsplineFunctor<RealType> > J2_aos(els_aos,ip);
    ...
    //for(int mc=0; mc<nsteps; ++mc)
    {
      els.G=czero;
      els.L=czero;
      J2.evaluateLogAndStore(els,els.G,els.L);

      els_aos.G=czero;
      els_aos.L=czero;
      J2_aos.evaluateLogAndStore(els_aos,els_aos.G,els_aos.L);

      cout << "Check values " << J2.LogValue << " " << els.G[0] << " " << els.L[0] << endl;
      cout << "evaluateLog::V Error = " << J2.LogValue-J2_aos.LogValue<< endl;
      {
        double g_err=0.0;
      ...
      {
        els.setActive(iel);
        PosType grad_soa=J2.evalGrad(els,iel);

        els_aos.setActive(iel);
        PosType grad_aos=J2_aos.evalGrad(els_aos,iel)-grad_soa;
        ...
        RealType r_soa=J2.ratioGrad(els,iel,grad_soa);

        grad_aos=0;
        els_aos.makeMoveAndCheck(iel,dr); 
        RealType r_aos=J2_aos.ratioGrad(els_aos,iel,grad_aos);
        ...
        {
          els.acceptMove(iel);
          J2.acceptMove(els,iel);

          els_aos.acceptMove(iel);
          J2_aos.acceptMove(els_aos,iel);
          naccepted++;
        }
        else
        {
      ...
      for(int iel=0; iel<nels; ++iel)
      {
        ...
        for(int k=0; k<nknots;++k)
        {
          ...
          RealType r_soa=J2.ratio(els,iel);
          els.rejectMove(iel);

          els_aos.makeMoveOnSphere(iel,delta[k]);
          RealType r_aos=J2_aos.ratio(els_aos,iel);
          els_aos.rejectMove(iel);
          r_ratio += abs(r_soa/r_aos-1);
        }
      }
