...
namespace cqmc {
  ...
  template<typename S> class DavidsonLMHD : public EigenSolver<S>{
	...
	void solve_subspace_nonsymmetric(const bool outer)
  {
    ...
    for(int i = 0; i < truncate_index; i++){
      for(int j = 0; j < truncate_index; j++){
        trun_sin_val_matrix.at(i, j) = (trun_sin_val_matrix.at(i, j) == formic::zero(S()) ? trun_sin_val_matrix.at(i, j) : formic::unity(S()) / trun_sin_val_matrix.at(i, j));
      }
    }
...
public:
  ...
  void tau_and_correct_ham()
  {
    ...
    if ( my_rank == 0 ) {
      ...
      // evaluate new tau
      this->_tau = formic::real(formic::dotc(this->_ind_evecs, _smat * this->_ind_evecs)  / formic::dotc(this->_ind_evecs, _lmsmat * this->_ind_evecs));
	...
	void add_krylov_vector(const formic::ColVec<S> & v)
  {
    ...
    if (my_rank == 0 && !_build_lm_matrix && v.size() != this->_der_rat.cols())  
      throw formic::Exception("bad vector length of %d in DavidsonLMHD::add_krylov_vector: expected length of %d") % v.size() % this->_der_rat.cols();

    else if (my_rank == 0 && _build_lm_matrix && v.size() != this->_nfds) 
      throw formic::Exception("bad vector length of %d in DavidsonLMHD::add_krylov_vector: expected length of %d") % v.size() % this->_nfds;
  ...
  void HMatVecOp(const formic::ColVec<S> & x, formic::ColVec<S> & y, const bool transpose = false, const bool approximate = false)  
  {
    ...
    // temp2 = Lx
      ...
      if ( this->_ground ) {
        ...
        formic::xgemv('N', Ns, Nind, formic::unity(S()), &this->_le_der.at(0,0), Ns, &x.at(0), 1, formic::zero(S()), &temp.at(0), 1);
        ...
        // call blas level-2 function 
        formic::xgemv('N', Ns, Nind, formic::unity(S()) * this->_hd_shift, &this->_der_rat.at(0,0), Ns, &x.at(0), 1, formic::zero(S()), &temp1.at(0), 1);

        // call blas level-2 function 
        formic::xgemv('N', Ns, Nind, formic::unity(S()), &this->_le_der.at(0,0), Ns, &x.at(0), 1, formic::zero(S()), &temp2.at(0), 1);
  ...
  void SMatVecOp(const formic::ColVec<S> & x, formic::ColVec<S> & y, const bool approximate = false)
  {
    ...
    // temp2 = Lx
      ...
      if ( this->_ground ) {
        ...
        // call blas level-2 function
        formic::xgemv('N', Ns, Nind, formic::unity(S()), &this->_der_rat.at(0,0), Ns, &x.at(0), 1, formic::zero(S()), &temp.at(0), 1);
        ...
        formic::xgemv('T', Ns, Nind, formic::unity(S())*this->_hd_shift, &this->_der_rat.at(0,0), Ns, &temp1.at(0), 1, formic::zero(S()), &y.at(0), 1);

        // L^T * (omega * D - L) * x
        formic::xgemv('T', Ns, Nind, formic::unity(S()), &this->_le_der.at(0, 0), Ns, &temp1.at(0), 1, formic::zero(S()), &temp3.at(0), 1);
  ...
  void LSMatVecOp(const formic::ColVec<S> & x, formic::ColVec<S> & y, const bool approximate = false)
  {
	...
	void update_hamovlp(formic::Matrix<S> & hmat, formic::Matrix<S> & smat)
  {
    ...
    bool right_size = (hmat.rows() == this->_nfds && hmat.cols() == this->_nfds && smat.rows() == this->_nfds && smat.cols()); 
