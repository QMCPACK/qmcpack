...
namespace formic {
  ...
  class ScopedTimer {
    public:
      ScopedTimer(std::pair<std::string,formic::Stopwatch> * pp) : _pp(pp) {
        if ( !_pp )
    ...
    private:
      std::pair<std::string,formic::Stopwatch> * _pp; ///< pointer to name and underlying stopwatch
  ...
  namespace timers {
    extern boost::scoped_ptr<std::pair<std::string,formic::Stopwatch> > propose_1e;
    extern boost::scoped_ptr<std::pair<std::string,formic::Stopwatch> > propose_2e;
  }
