 ...
 */
template<class T1, class T2, bool ORTHO>
struct DotProduct
{};
...
struct DotProduct<TinyVector<T1, D>, Tensor<T2, D>, false>
{
  ...
  inline static TinyVector<Type_t, D> apply(const TinyVector<T1, D>& lhs, const Tensor<T2, D>& rhs)
  {
...
struct DotProduct<TinyVector<T1, 3>, Tensor<T2, 3>, false>
{
  ...
  inline static TinyVector<Type_t, 3> apply(const Tensor<T2, 3>& lhs, const TinyVector<T1, 3>& rhs)
  {
...
struct CartesianNorm2<TinyVector<T1, D>, Tensor<T2, D>, false>
{
   ...
   */
  inline static Type_t apply(const TinyVector<T1, D>& a, const Tensor<T2, D>& X, const TinyVector<T1, D>& b)
  {
    return dot(a, dot(X, b));
  }
};
...
struct CartesianNorm2<TinyVector<T1, 3>, Tensor<T2, 3>, false>
{
   ...
   * @return dot(a,dot(X,b))
   */
  inline static Type_t apply(const TinyVector<T1, 3>& a, const Tensor<T2, 3>& X, const TinyVector<T1, 3>& b)
  {
    return a[0] * (X[0] * b[0] + X[1] * b[1] + X[2] * b[2]) + a[1] * (X[3] * b[0] + X[4] * b[1] + X[5] * b[2]) +
...
{
  inline static bool inside(const TinyVector<T, 1>& u) { return (u[0] > 0.0 && u[0] < 1.0); }
