...
void Uniform3DGridLayout::checkGrid(value_type int_rad) {
  ...
  ncx = (ncx)? ncx:ncx+1; ncy = (ncy)? ncy:ncy+1; ncz = (ncz)? ncz:ncz+1;
  ...
  if((Grid[SPATIAL_GRID][0]*Grid[SPATIAL_GRID][1]*Grid[SPATIAL_GRID][2]) == 1) {
    Grid[SPATIAL_GRID][0] = ncx; Grid[SPATIAL_GRID][1] = ncy; Grid[SPATIAL_GRID][2] = ncz;
...
int Uniform3DGridLayout::connectGrid(value_type int_rad, value_type con_rad) {
  ...
  if(int_rad<numeric_limits<Scalar_t>::epsilon()) return 1;
  ...
  NCMax[0] = nx; NCMax[1] = ny; NCMax[2] = nz;
  ...
  for(int ig=0; ig<NP[0]; ig++) {
	...
	for(int ixyz=0; ixyz<RShell.size(); ixyz++) {
	  ...
	  if(!BoxBConds[0] && d[0]<0 || d[0]>= NP[0]) continue;
	  if(!BoxBConds[1] && d[1]<0 || d[1]>= NP[1]) continue;
	  if(!BoxBConds[2] && d[2]<0 || d[2]>= NP[2]) continue;
          d[0]=d0[0]%NP[0]; d[0]= (d[0]<0)?d[0]+NP[0] : d[0]; 
          d[1]=d0[1]%NP[1]; d[1]= (d[1]<0)?d[1]+NP[1] : d[1];
          d[2]=d0[2]%NP[2]; d[2]= (d[2]<0)?d[2]+NP[2] : d[2];
