...
public:
  ...
  inline TraceManagerNew* makeClone()
  {
    ...
    if (verbose)
      app_log() << "TraceManagerNew::makeClone " << master_copy << std::endl;
    if (!master_copy)
      APP_ABORT("TraceManagerNew::makeClone  only the master copy should call this function");
    TraceManagerNew* tm = new TraceManagerNew();
  ...
  inline void transfer_state_from(const TraceManagerNew& tm)
  {
  ...
  {
    app_log()<<"JTK: TraceManagerNew::initialize_traces "<<streaming_traces<<std::endl;
    if (streaming_traces)
    {
      if (verbose)
        app_log() << "TraceManagerNew::initialize_traces " << master_copy << std::endl;
  ...
  {
    app_log()<<"JTK: TraceManagerNew::finalize_traces "<<std::endl;
    if (verbose)
      app_log() << "TraceManagerNew::finalize_traces " << master_copy << std::endl;
  ...
  {
    app_log()<<"JTK: TraceManagerNew::checkout_int"<<std::endl;
  ...
  {
    app_log()<<"JTK: TraceManagerNew::checkout_real"<<std::endl;
  ...
  {
    app_log() << "JTK: TraceManagerNew::reset_buffers"<<std::endl;
    if (writing_traces)
    {
      if (verbose)
        app_log() << "TraceManagerNew::reset_buffers " << master_copy << std::endl;
      int_buffer.reset();
      real_buffer.reset();
    }
  }
  ...
  {
    app_log() << "JTK: TraceManagerNew::buffer_sample "<<writing_traces<<" "<<current_step<<std::endl;
    if (writing_traces && current_step % throttle == 0)
    {
      if (verbose)
        app_log() << " TraceManagerNew::buffer_sample() " << master_copy << std::endl;
  ...
  {
    app_log() << "JTK: TraceManagerNew::startBlock " << master_copy << std::endl;
    if (verbose)
      app_log() << "TraceManagerNew::startBlock " << master_copy << std::endl;
    reset_buffers();
  }
  ...
  inline void write_summary(std::string pad = "  ")
  {
    std::string pad2 = pad + "  ";
    app_log() << std::endl;
    app_log() << pad << "TraceManagerNew (detailed summary)" << std::endl;
    ...
    app_log() << pad << "end TraceManagerNew" << std::endl;
  }
