...
namespace qmcplusplus {
  ...
  struct ReptileCorrelationEstimator: public ReptileEstimator {
    ...
    void put(xmlNodePtr cur, int Rlength, int SFH) {
      ...
      if ((sumObservable=="false")||(sumObservable=="no")) sumO=false;
	...
	for(RealType i=0;i<Rsize;i+=interval){
	  int pnt;
	  if (i<=0.5*Rsize) pnt =static_cast<int>(i+0.5);
	  else pnt =static_cast<int>(i);
...
//       app_log()<<"outputStride "<<outputStride.size()<<endl;
    ...
    void addNames(vector<string>& names){
      ...
      {
	if (headObservable==names[j]) headO=j-1+FirstHamiltonian ;
	if (bodyObservable==names[j]) bodyO=j-1+FirstHamiltonian ;
      }
	...
	headObservable="LocPot";
      ...
      else if(headObservable=="LogPsi") headO=LOGPSI;
      else if(headObservable=="LocalEnergy")
      {
	headObservable="LE";
	headO=LOCALENERGY;
      }
	...
	bodyObservable="LocPot";
      ...
      else if(bodyObservable=="LogPsi") bodyO=LOGPSI;
      else if(bodyObservable=="LocalEnergy") 
      {
	bodyObservable="LE";
	bodyO=LOCALENERGY;
      }
