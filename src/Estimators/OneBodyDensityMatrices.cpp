...
//   if (scale > 1.0 + 1e-10)
//   {
//     APP_ABORT("OneBodyDensityMatrices::put  scale must be less than one");
//   }
//   else if (scale < 0.0 - 1e-10)
//     APP_ABORT("OneBodyDensityMatrices::put  scale must be greater than zero");

//   // get volume and cell information
//   lattice_.reset();
//   if (!center_defined)
//     center = lattice_.Center;
//   volume   = lattice_.Volume * std::exp(OHMMS_DIM * std::log(scale));
...
//     samples     = pow(points, OHMMS_DIM);
//     metric      = volume / samples;
//     ind_dims[0] = pow(points, OHMMS_DIM - 1);
...
//   else
//     APP_ABORT(
//         "OneBodyDensityMatrices::set_state  invalid integrator\n  valid options are: uniform_grid, uniform, density");
...
//     evaluator = matrix;
//   else
//     APP_ABORT("OneBodyDensityMatrices::set_state  invalid evaluator\n  valid options are: loop, matrix");
...
//   if (sposets.size() == 0)
//     APP_ABORT("OneBodyDensityMatrices::put  basis must have at least one sposet");

//   for (int i = 0; i < sposets.size(); ++i)
//   {
//     SPOSet* sposet = wf_factory_.getSPOSet(sposets[i]);
//     if (sposet == 0)
//       APP_ABORT("OneBodyDensityMatrices::put  sposet " + sposets[i] + " does not exist");
//     basis_functions.add(sposet->makeClone());
//   }
//   basis_size = basis_functions.size();

//   if (basis_size < 1)
//     APP_ABORT("OneBodyDensityMatrices::put  basis_size must be greater than one");
// }


// void OneBodyDensityMatrices::set_state(OneBodyDensityMatrices& master)
// {
...
// void OneBodyDensityMatrices::initialize()
// {
...
//   if (isize == natt)
//     APP_ABORT("OneBodyDensityMatrices::set_state  Species set does not have the required attribute 'membersize'");
...
//       {{DM_eval, "OneBodyDensityMatrices::evaluate"},
//        {DM_gen_samples, "OneBodyDensityMatrices::generate_samples"},
//        {DM_gen_sample_basis, "OneBodyDensityMatrices::generate_sample_basis"},
//        {DM_gen_sample_ratios, "OneBodyDensityMatrices::generate_sample_ratios"},
//        {DM_gen_particle_basis, "OneBodyDensityMatrices::generate_particle_basis"},
//        {DM_matrix_products, "OneBodyDensityMatrices::evaluate_matrix_products"},
//        {DM_accumulate, "OneBodyDensityMatrices::evaluate_matrix_accum"}};
...
// void OneBodyDensityMatrices::finalize()
// {
...
// void OneBodyDensityMatrices::report(const std::string& pad)
// {
...
//   out << pad << "OneBodyDensityMatrices" << std::endl;
...
//   out << pad << "end OneBodyDensityMatrices" << std::endl;
// }

void OneBodyDensityMatrices::setRandomGenerator(RandomGenerator_t* rng) { uniform_random = rng; }

// void OneBodyDensityMatrices::addObservables(PropertySetType& plist, BufferType& collectables)
// {
...
// void OneBodyDensityMatrices::registerCollectables(std::vector<ObservableHelper>& h5desc, hid_t gid) const
// {
// #if defined(QMC_COMPLEX)
//   std::vector<int> ng(3);
//   ng[0]        = basis_size;
...
// OneBodyDensityMatrices::FullPrecReal OneBodyDensityMatrices::evaluate(ParticleSet& P)
// {
...
//       evaluate_matrix(P);
//     else
//       APP_ABORT("OneBodyDensityMatrices::evaluate  invalid evaluator");
//   }
//   return 0.0;
// }
...
// template<typename T>
// inline void accum_sample(std::vector<Value_t>& E_samp, CombinedTraceSample<T>* etrace, Real weight = 1.0)
// {
//   if (etrace)
//   {
//     etrace->combine();
...
// template<typename T>
// inline void accum_sample(std::vector<Value_t>& E_samp, TraceSample<T>* etrace, Real weight = 1.0)
// {
// #ifdef QMC_COMPLEX
//   if (etrace)
//     for (int p = 0; p < etrace->sample.size(); ++p)
...
// void OneBodyDensityMatrices::get_energies(std::vector<Vector<Value>*>& E_n)
// {
//   Value Vc = 0;
