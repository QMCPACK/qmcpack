...
private:
  /** In the batched version this Walkers unique ID
   *  This id is generated by the MCPopulation instance of the rank via MCPopulation::nextWalkerID
   *  It is assigned when the walker is created alive or when a walker is born via MCPopulation::spawnWalker
   *  either from a dead walker or as a discrete act of creation if the MCPopulation instance has no dead walkers.
   *
   ...
   */
  long walker_id_ = 0;
  /** In batched version WalkerID this walker was copied from
   *  or itself if it walker created without a walker as template.
   *  The way in which this is done in the code is still not very clean.
   *
   *  in legacy the ancients have said only:
   *  id reserved for forward walking
   *  PD: but likely incorrect and/or unused.
...
public:
  ...
  int Age = 0;
  ///Age of this walker age is incremented when a walker is not moved after a sweep (legacy only)
  int ReleasedNodeAge = 0;
  ///Weight of the walker
  FullPrecRealType Weight = 1.0;
  ///Weight of the walker (legacy only)
  FullPrecRealType ReleasedNodeWeight = 1.0;
...
public:
   ...
   *  \todo remove api when legacy is dropped
   */
  ...
  void copyFromBuffer()
  {
    assert(DataSet.size() != 0);
    DataSet.rewind();
    DataSet >> walker_id_ >> parent_id_ >> Generation >> Age >> ReleasedNodeAge >> ReleasedNodeWeight;
    // vectors
    assert(R.size() != 0);
