...
struct SoaDistanceTableAA : public DTD_BConds<T, D, SC>, public DistanceTableData
{
  ...
  inline void evaluate(ParticleSet& P)
  {
    ...
    //P.RSoA->getAllParticlePos().copyIn(P.R);
    for (int iat = 0; iat < N_targets; ++iat)
    {
      DTD_BConds<T, D, SC>::computeDistances(P.R[iat], P.RSoA->getAllParticlePos(), distances_[iat].data(), displacements_[iat], 0, N_targets,
                                             iat);
  ...
  {
    DTD_BConds<T, D, SC>::computeDistances(rnew, P.RSoA->getAllParticlePos(), temp_r_.data(), temp_dr_, 0, N_targets, P.activePtcl);
    // set up old_r_ and old_dr_ for moves may get accepted.
    if (prepare_old)
    {
      //recompute from scratch
      DTD_BConds<T, D, SC>::computeDistances(P.R[iat], P.RSoA->getAllParticlePos(), old_r_.data(), old_dr_, 0, N_targets, iat);
