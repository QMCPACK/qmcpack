...
#pragma ivdep
    for(int i=first; i<last; i++) {
      pout[i][0]=pin[i][0]*xx; pout[i][1]=pin[i][1]*yy; 
    }
  }
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      ...
      TWO_DIM_BOUNDARY_BLOCK(x,y,epsilon,plus_one);
      pinout[i][0]=r00*x; pinout[i][1]=r11*y;
    }
  }
