 ...
 */
template<class T1, class T2, bool ORTHO> struct ConvertPosUnit { };

/** Specialized ConvertPosUnit for ParticleAttrib<T,3>, Tensor<T,3> and true
 */
template<class T>
struct ConvertPosUnit<ParticleAttrib<TinyVector<T,3> >,Tensor<T,3>, true> {
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      pout[i][0]=pin[i][0]*xx; pout[i][1]=pin[i][1]*yy; pout[i][2]=pin[i][2]*zz;
    }
  }
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      pinout[i][0]*=xx; pinout[i][1]*=yy; pinout[i][2]*=zz;
    }
  }
...
template<class T>
struct ConvertPosUnit<ParticleAttrib<TinyVector<T,3> >,Tensor<T,3>, false> {
...
template<class T1, class T2, bool ORTHO> struct ApplyBConds { };

template<class T>
struct ApplyBConds<ParticleAttrib<TinyVector<T,3> >, Tensor<T,3>, true> {
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      ...
      pout[i][0]=x; pout[i][1]=y; pout[i][2]=z;
    }
  }
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      T x(pin[i][0]),y(pin[i][1]),z(pin[i][2]);
      THREE_DIM_BOUNDARY_BLOCK(x,y,z,epsilon,plus_one)
      pout[i][0]=r00*x; pout[i][1]=r11*y; pout[i][2]=r22*z;
    }
  }
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      ...
      THREE_DIM_BOUNDARY_BLOCK(x,y,z,epsilon,plus_one)
      pinout[i][0]=x; pinout[i][1]=y; pinout[i][2]=z;
    }
  }
...
#pragma ivdep
    for(int i=first; i<last; i++) {
      ...
      THREE_DIM_BOUNDARY_BLOCK(x,y,z,epsilon,plus_one)
      pinout[i][0]=r00*x; pinout[i][1]=r11*y; pinout[i][2]=r22*z;
    }
  }
...
template<class T>
struct ApplyBConds<ParticleAttrib<TinyVector<T,3> >, Tensor<T,3>, false> {
