...
void accept_move_GPU_cuda(float** Rlist, float new_pos[], int toAccept[], int iat, int N, int k)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  accept_kernel<float, BS><<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(Rlist, new_pos, toAccept, iat, N, k);
}


void accept_move_GPU_cuda(double** Rlist, double* new_pos, int* toAccept, int iat, int N, int k)
{
  const int BS = 32;
  int NB       = N / BS + ((N % BS) ? 1 : 0);
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  accept_kernel<double, BS><<<dimGrid, dimBlock, 0, gpu::kernelStream>>>(Rlist, new_pos, toAccept, iat, N, k);
}
...
void NL_move_cuda(float** Rlist, float* new_pos, int N)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  NL_move_kernel<float, BS><<<dimGrid, dimBlock>>>(Rlist, new_pos, N);
}
...
void NL_move_cuda(double** Rlist, double new_pos[], int N)
{
  ...
  dim3 dimBlock(BS);
  dim3 dimGrid(NB);
  NL_move_kernel<double, BS><<<dimGrid, dimBlock>>>(Rlist, new_pos, N);
}
