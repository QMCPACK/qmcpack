...
namespace qmcplusplus {
  ...
  template<typename T>
    struct BoxBCHandler<T,3,true>
    {
      ...
      {
        pout[0]=pin[0]*L0Inv; pout[0]=L0*(pout[0]-std::floor(pout[0]));
        pout[1]=pin[1]*L1Inv; pout[1]=L1*(pout[1]-std::floor(pout[1]));
        pout[2]=pin[2]*L2Inv; pout[2]=L2*(pout[2]-std::floor(pout[2]));
      }

      template<typename PAT1, typename PAT2>
      inline void put2box(const PAT1& pin, PAT2& pout, int n)
      {
        for(int i=0; i<n; ++i) put2box(pin[i],pout[i]);
      }
    };
 ...
 * $Revision: 4572 $   $Date: 2010-02-08 10:40:11 -0600 (Mon, 08 Feb 2010) $
 * $Id: MixedBCDistanceTableData.h 4572 2010-02-08 16:40:11Z jeongnim.kim $ 
 ***************************************************************************/
