...
public:
  ...
  int CurrentParticle;
  GPU_XRAY_TRACE void proposeMove_GPU(std::vector<PosType>& newPos, int iat);
  GPU_XRAY_TRACE void acceptMove_GPU(std::vector<bool>& toAccept, int k);
  GPU_XRAY_TRACE void acceptMove_GPU(std::vector<bool>& toAccept) { acceptMove_GPU(toAccept, 0); }
  GPU_XRAY_TRACE void NLMove_GPU(std::vector<Walker_t*>& walkers, std::vector<PosType>& Rnew, std::vector<int>& iat);
#endif
   ...
   */
  GPU_XRAY_TRACE bool dumpEnsemble(std::vector<MCWalkerConfiguration*>& others,
                                   HDFWalkerOutput* out,
                                   int np,
                                   int nBlock);
  ///clear the ensemble
