...
public:
  ...
  /// copy constructor
  ParticleSetT(const ParticleSetT<T>& p);
  ...
  /** add a distance table
     ...
     */
  int addTable(const ParticleSetT<T>& psrc, DTModes modes = DTModes::ALL_OFF);
  ...
  static void mw_update(const RefVectorWithLeader<ParticleSetT<T>>& p_list, bool skipSK = false);
  ...
  static void mw_makeSpinMove(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
                              Index_t iat,
  ...
  /// CoordsType
  template<CoordsType CT>
  static void mw_accept_rejectMoveT(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
                                    Index_t iat,
                                    const std::vector<bool>& isAccepted,
                                    bool forward_mode = true);

  /// batched version of acceptMove and rejectMove fused
  static void mw_accept_rejectMove(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
                                   Index_t iat,
                                   const std::vector<bool>& isAccepted,
                                   bool forward_mode = true);
  ...
  /** batched version  of acceptMove and reject Move fused, but only for spins
     ...
     */
  static void mw_accept_rejectSpinMove(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
                                       Index_t iat,
                                       const std::vector<bool>& isAccepted);
  ...
  static void mw_loadWalker(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
  ...
  /** batched version of saveWalker
     ...
     *  just the R, G, and L
     */
  static void mw_saveWalker(const RefVectorWithLeader<ParticleSetT<T>>& psets, const RefVector<Walker_t>& walkers);
  ...
  static void mw_donePbyP(const RefVectorWithLeader<ParticleSetT<T>>& p_list, bool skipSK = false);
  ...
  /** Initialize particles around another ParticleSet
     ...
     */
  void randomizeFromSource(ParticleSetT<T>& src);
  ...
  /** acquire external resource and assocaite it with the list of ParticleSet
     ...
     */
  static void acquireResource(ResourceCollection& collection, const RefVectorWithLeader<ParticleSetT<T>>& p_list);
  /** release external resource
     * Note: use RAII ResourceCollectionTeamLock whenever possible
     */
  static void releaseResource(ResourceCollection& collection, const RefVectorWithLeader<ParticleSetT<T>>& p_list);

  static RefVectorWithLeader<DistanceTableT<T>> extractDTRefList(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
                                                                 int id);
  static RefVectorWithLeader<DynamicCoordinatesT<T>> extractCoordsRefList(
      const RefVectorWithLeader<ParticleSetT<T>>& p_list);
  static RefVectorWithLeader<StructFactT<T>> extractSKRefList(const RefVectorWithLeader<ParticleSetT<T>>& p_list);
...
protected:
  ...
  /** compute temporal DistTables and SK for a new particle position for each
     ...
     * move will not be accepted.
     */
  static void mw_computeNewPosDistTables(const RefVectorWithLeader<ParticleSetT<T>>& p_list,
                                         Index_t iat,
