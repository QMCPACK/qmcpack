...
// use in_place_cast from DP to SP and back for input G 
  ...
  atomcentered_correlators(afqmc::TaskGroup_& tg_, AFQMCInfo& info, xmlNodePtr cur, WALKER_TYPES wlk, 
                ...
                walker_type(wlk),ns2(0),writer(false),
                ...
                NwI({0,0,0},shared_allocator<ComplexType>{TG.TG_local()})
  {
    ...
    if( TG.Node().root() ) {
      ...
      for(int i=0; i<NAO; i++)  
        for(int j=0; j<NAO; j++) 
          S[0][i][j] = ComplexType( std::conj( S[0][i][j] ) );  
                                               ...
                                               HostCVec2&& Xw, HostCVec3&& ovlp, bool impsamp)
  {
    ...
    if(iref == 0) {
      ...
      if( DMWork2D.size(0) != nw ||
          DMWork2D.size(1) != 3 ||
          DMWork2D.size(2) != ns2 ) {
        DMWork2D = std::move(mpi3CTensor({nw,3,ns2},
                                      shared_allocator<ComplexType>{TG.TG_local()}));
      }
      if( NwIJ.size(0) != nsp ||
          NwIJ.size(1) != nw ||
          NwIJ.size(2) != nsites ||
          NwIJ.size(3) != nsites ) {
        NwIJ = std::move(mpi3C4Tensor({nsp,nw,nsites,nsites},
                                      shared_allocator<ComplexType>{TG.TG_local()}));
      }
      if( NwI.size(0) != nsp ||
          NwI.size(1) != nw ||
          NwI.size(2) != nsites ) { 
        NwI = std::move(mpi3CTensor({nsp,nw,nsites},
                                      shared_allocator<ComplexType>{TG.TG_local()}));
      }
    ...
    while(iw0 < nw) 
    {
      ...
      for(int is=0; is<nsp; ++is)   { 
        ...
        gemmBatched('N','N',NMO,int(iN-i0),NMO,ComplexType(1.0),Barray.data(),NMO,
                  Aarray.data(),NMO,
        ...
        // careful with fortran ordering
        gemmBatched('T','N',NAO,int(iN-i0),NMO,ComplexType(1.0),Barray.data(),NMO,
                  Carray.data(),NMO,
