...
#ifndef QMCPLUSPLUS_AFQMC_EKT_HPP
#define QMCPLUSPLUS_AFQMC_EKT_HPP
...
/* 
 * Observable class that calculates the walker averaged generalized fock matrices 
 */
class ekt: public AFQMCInfo
{
  ...
  ekt(afqmc::TaskGroup_& tg_, AFQMCInfo& info, xmlNodePtr cur, WALKER_TYPES wlk, 
                ...
                DMAverage({0,0},shared_allocator<ComplexType>{TG.TG_local()})
  {
    ...
    if(rot_file != "") {
      ...
      if( TG.Node().root() ) {      
        ...
        if(!dump.push(path,false)) APP_ABORT("Error in ekt: path not found."); 
        stdCMatrix R;
