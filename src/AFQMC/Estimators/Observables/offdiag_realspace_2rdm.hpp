...
class offdiag_realspace_2rdm: public AFQMCInfo
{
                                               ...
                                               HostCVec2&& Xw, HostCVec3&& ovlp, bool impsamp)
  {
    ...
    if(iref == 0) {
      ...
      if( DMWork.size(0) != nw ||
        ...
        DMWork = std::move(mpi3CTensor({nw,3,dm_size},
                                      shared_allocator<ComplexType>{TG.TG_local()}));
      }
      if( Gr_host.size(0) != nw ||
          Gr_host.size(1) != nsp ||
          Gr_host.size(2) != npts ||
          Gr_host.size(3) != npts ) {
        Gr_host = std::move(mpi3C4Tensor({nw,nsp,npts,npts},
                                      shared_allocator<ComplexType>{TG.TG_local()}));
      }
    ...
    // if memory becomes a problem, then batch over walkers
    {
      ...
      std::vector<decltype(ma::pointer_dispatch(Orbitals.origin()))> Aarray; Aarray.reserve(nw*nsp);
      std::vector<decltype(ma::pointer_dispatch(Orbitals.origin()))> Barray; Barray.reserve(nw*nsp);
      std::vector<decltype(ma::pointer_dispatch(Orbitals.origin()))> Carray; Carray.reserve(nw*nsp);
      ...
      gemmBatched('N','C',npts,npts,NMO,ComplexType(1.0),Barray.data(),npts,
                  Aarray.data(),npts,
  ...
  void accumulate_block(int iav, HostCVec&& wgt, bool impsamp) 
  {
    ...
    if(TG.TG_local().root()) { 
      ...
      ma::product( ComplexType(1.0,0.0), ma::T( DMWork2D ),  denom, 
