...
class generalizedFockMatrix: public AFQMCInfo
{
  ...
  generalizedFockMatrix(afqmc::TaskGroup_& tg_, AFQMCInfo& info, xmlNodePtr cur, WALKER_TYPES wlk, 
           HamiltonianOperations* HOps_, int nave_=1, int bsize=1):
                AFQMCInfo(info),TG(tg_),walker_type(wlk),HamOp(HOps_),writer(false),
  ...
  void accumulate_block(int iav, HostCVec&& wgt, bool impsamp) 
  {
    ...
    ma::product( ComplexType(1.0,0.0), ma::T( DMWork[0]( {0, nw}, {i0,iN}) ),  denom, 
                 ComplexType(1.0,0.0), DMAverage[0][iav].sliced(i0,iN)); 
    ma::product( ComplexType(1.0,0.0), ma::T( DMWork[1]( {0, nw}, {i0,iN}) ),  denom, 
                 ComplexType(1.0,0.0), DMAverage[1][iav].sliced(i0,iN)); 
    TG.TG_local().barrier();
  }
