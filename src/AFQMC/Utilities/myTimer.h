...
public:
  ...
  double average(const std::string& str)
  {
    int n = getPos(str);
    if (n >= 0)
      return (std::get<0>(timer[n]) == 0) ? (0.0)
                                          : (std::get<2>(timer[n]) / static_cast<double>(std::get<0>(timer[n])));
    return -1;
  }
  ...
  void print_average(const std::string& str, std::ostream& out)
  {
    int n = getPos(str);
    if (n >= 0)
      out << " Average time in " << str << ": "
          << ((std::get<0>(timer[n]) == 0) ? (0.0)
                                           : (std::get<2>(timer[n]) / static_cast<double>(std::get<0>(timer[n]))))
          << "\n";
    else
