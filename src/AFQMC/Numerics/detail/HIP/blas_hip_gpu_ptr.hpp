...
namespace device
{
  ...
  {
    static_assert(std::is_same<typename std::decay<Q>::type,T>::value,"Wrong dispatch.\n");
    if(hipSuccess != hipMemcpy2D(to_address(y),sizeof(Q)*incy,
                                   x,sizeof(T)*incx,
                                   sizeof(T),n,hipMemcpyHostToDevice))
  ...
  inline static void copy(int n, device_pointer<Q> x, int incx, T* y, int incy)
  {
    ...
    if(hipSuccess != hipMemcpy2D(y,sizeof(T)*incy,
...
// replace with single call to hipMalloc and hipMemcpy
  ...
  inline static void copy2D(int N, int M, device_pointer<T> src, int lda,
    ...
    if(hipSuccess != hipMemcpy2D(to_address(dst),sizeof(T2)*ldb,
                    to_address(src),sizeof(T)*lda,M*sizeof(T),N,hipMemcpyDeviceToDevice))
  ...
  inline static void copy2D(int N, int M, T const* src, int lda,
    ...
    static_assert(std::is_same<typename std::decay<T>::type,T2>::value,"Wrong dispatch.\n");
    if(hipSuccess != hipMemcpy2D(to_address(dst),sizeof(T2)*ldb,
                    src,sizeof(T)*lda,M*sizeof(T),N,hipMemcpyHostToDevice))
      throw std::runtime_error("Error: hipMemcpy2D returned error code in copy2D.");
  }
  ...
  inline static void copy2D(int N, int M, device_pointer<T> src, int lda,
                                          T2* dst, int ldb){
    static_assert(std::is_same<typename std::decay<T>::type,T2>::value,"Wrong dispatch.\n");
    if(hipSuccess != hipMemcpy2D(dst,sizeof(T2)*ldb,
                    to_address(src),sizeof(T)*lda,M*sizeof(T),N,hipMemcpyDeviceToHost))
      throw std::runtime_error("Error: hipMemcpy2D returned error code in copy2D.");
  }
