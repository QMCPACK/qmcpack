...
namespace device
{
  ...
  extern boost::multi::array<std::complex<double>,1,
                             device::device_allocator<std::complex<double>>> *hipsparse_buffer;
        ...
        device_pointer<T> y  )
  {
    ...
    if(hipSuccess != hipMemcpy(std::addressof(pe),to_address(pntre+(M-1)),sizeof(int),hipMemcpyDeviceToHost))
     throw std::runtime_error("Error: hipMemcpy returned error code in csrmv.");
    int nnz = pe-pb;
    if(HIPSPARSE_STATUS_SUCCESS != hipsparse::hipsparse_csrmv(*A.handles.hipsparse_handle,transa,
            M,K,nnz,alpha,qmc_hip::afqmc_hipsparse_matrix_descr,
...
// /*
      ...
      if(hipsparse_buffer == nullptr) {
        hipsparse_buffer = new boost::multi::array<std::complex<double>,1,
