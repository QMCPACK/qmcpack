...
template<typename T>
__global__ void kernel_acAxpbB(int m, int n,
                           T const alpha,
                           T const* A, int lda,
                           T const* x, int incx,
                           T const beta,
...
__global__ void kernel_acAxpbB(int m, int n,
                           ...
                           thrust::complex<T> const* A, int lda,
                           thrust::complex<T> const* x, int incx,
                           thrust::complex<T> const beta,
                           thrust::complex<T> * B, int ldb)
{
  int i = threadIdx.x + blockDim.x*blockIdx.x;
  int j = threadIdx.y + blockDim.y*blockIdx.y;
  if( (i < m) && (j < n) )
...
void acAxpbB(int m, int n, double const alpha, double const* A, int lda,
                            ...
                            double const beta, double * B, int ldb)
{
  int xblock_dim = 16;
                           ...
                           std::complex<double> * B, int ldb)
{
  ...
  dim3 block_dim(xblock_dim,yblock_dim);
  dim3 grid_dim(xgrid_dim,ygrid_dim);
  kernel_acAxpbB<<<grid_dim, block_dim>>>(m,n,
