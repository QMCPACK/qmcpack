...
{
  assert(std::get<0>(A.strides()) > 0);
  assert(std::get<1>(A.strides()) == 1);

  int res;
  getrf_bufferSize(std::get<1>(A.sizes()), std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), res);
  return res;
}
...
{
  assert(std::get<0>(m.strides()) >= std::max(std::size_t(1), std::size_t(std::get<1>(m.sizes()))));
  assert(std::get<1>(m.strides()) == 1);
  assert(pivot.size() >= std::min(std::get<1>(m.sizes()), std::get<0>(m.sizes()) + 1));

  int status = -1;
  getrf(std::get<1>(m.sizes()), std::get<0>(m.sizes()), pointer_dispatch(m.origin()), m.stride(), pointer_dispatch(pivot.data()), status,
...
int getri_optimal_workspace_size(MultiArray2D&& A)
{
  ...
  int lwork = -1;
  getri_bufferSize(std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), lwork);
  return lwork;
}
...
MultiArray2D&& getri(MultiArray2D&& A, MultiArray1D const& IPIV, Buffer&& WORK)
{
  ...
  assert(std::get<1>(A.strides()) == 1);
  assert(IPIV.size() >= size_t(std::get<0>(A.sizes())));
  assert(WORK.size() >= std::max(std::size_t(1), size_t(std::get<0>(A.sizes()))));
...
int geqrf_optimal_workspace_size(MultiArray2D&& A)
{
  ...
  int res;
  geqrf_bufferSize(std::get<1>(A.sizes()), std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), res);
  return res;
}
...
MultiArray2D&& geqrf(MultiArray2D&& A, Array1D&& TAU, Buffer&& WORK)
{
  ...
  assert(std::get<1>(A.strides()) == 1);
  assert(std::get<0>(TAU.strides()) == 1);
  assert(TAU.size() >= std::max(std::size_t(1), size_t(std::min(std::get<0>(A.sizes()), std::get<1>(A.sizes())))));
  assert(WORK.size() >= std::max(std::size_t(1), size_t(std::get<0>(A.sizes()))));

  int status = -1;
  geqrf(std::get<1>(A.sizes()), std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), pointer_dispatch(TAU.origin()),
        pointer_dispatch(WORK.data()), WORK.size(), status);
  assert(status == 0);
  return std::forward<MultiArray2D>(A);
}
...
int gelqf_optimal_workspace_size(MultiArray2D&& A)
{
  ...
  int res;
  gelqf_bufferSize(std::get<1>(A.sizes()), std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), res);
  return res;
}
...
MultiArray2D&& gelqf(MultiArray2D&& A, Array1D&& TAU, Buffer&& WORK)
{
  ...
  int status = -1;
  gelqf(std::get<1>(A.sizes()), std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), pointer_dispatch(TAU.data()),
        pointer_dispatch(WORK.data()), WORK.size(), status);
  assert(status == 0);
  return std::forward<MultiArray2D>(A);
}
...
MultiArray2D&& potrf(MultiArray2D&& A)
{
  assert(std::get<0>(A.sizes()) == std::get<1>(A.sizes()));
  int INFO;
  potrf('U', std::get<0>(A.sizes()), pointer_dispatch(A.origin()), std::get<0>(A.strides()), INFO);
  if (INFO != 0)
                     ...
                     RBuffer&& RWORK)
{
  ...
  int status = -1;
  gesvd(jobVT, jobU, std::get<1>(A.sizes()), std::get<0>(A.sizes()), pointer_dispatch(A.origin()), A.stride(), pointer_dispatch(S.origin()),
