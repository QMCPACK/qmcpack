...
TEST_CASE("determinant_from_getrf", "[Numerics][determinant]")
{
  ...
  double ovlp = determinant_from_getrf(x.size(0), lu.origin(), lu.size(1),
                                       pivot.origin(), log_factor);
...
TEST_CASE("strided_determinant_from_getrf", "[Numerics][determinant]")
{
  ...
  strided_determinant_from_getrf(x.size(0), lus.origin(), lu.size(1), lu.num_elements(),
                                 pivot.origin(), pivot.size(1),
...
TEST_CASE("batched_determinant_from_getrf", "[Numerics][determinant]")
{
  ...
  batched_determinant_from_getrf(x.size(0), lu_array.data(), lu.size(1),
                                 pivot.origin(), pivot.size(1),
                                 log_factor, to_address(ovlps.origin()), lu_array.size());
  REQUIRE(ovlps[0] == Approx(detx));
  REQUIRE(ovlps[1] == Approx(detx));
  REQUIRE(ovlps[2] == Approx(detx));
}
