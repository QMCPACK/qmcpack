...
protected:
  ...
  using const_pointer = typename std::pointer_traits<Ptr>::template rebind<const_element>;
...
public:
  ...
  void getProperty(walker_data id, TVec&& v) const
  {
    ...
    auto W_(boost::multi::static_array_cast<element, const_pointer>(walker_buffer).template const_array_cast<element, pointer>());
