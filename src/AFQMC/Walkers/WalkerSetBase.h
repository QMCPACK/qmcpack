...
public:
  ...
  const_reference operator[](int i) const
  {
    if (i < 0 || i > tot_num_walkers)
      APP_ABORT("error: index out of bounds.\n");
    assert(std::get<1>(walker_buffer.sizes()) == walker_size);
    return const_reference(boost::multi::static_array_cast<element, const_pointer>(walker_buffer)[i], data_displ, wlk_desc);
  }
  ...
  void getProperty(walker_data id, TVec&& v) const
  {
    ...
    auto W_(boost::multi::static_array_cast<element, const_pointer>(walker_buffer));
