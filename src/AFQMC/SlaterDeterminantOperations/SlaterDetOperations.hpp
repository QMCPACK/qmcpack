...
class SlaterDetOperations 
{
  ...
  public:
    ...
    T MixedDensityMatrix(const MatA& hermA, const MatB& B, MatC&& C, bool compact=false) {
      ...
      boost::multi::array_ref<T,2> TNN(TMat_NN.data(), extents[NAEA][NAEA]);
      assert(TMat_NM.num_elements() >= NAEA*NMO);
      boost::multi::array_ref<T,2> TNM(TMat_NM.data(), extents[NAEA][NMO]);
    ...
    T MixedDensityMatrix(const MatA& hermA, const MatB& B, MatC&& C, communicator& comm, bool compact=false) {
      ...
      boost::multi::array_ref<T,2> TNN(SM_TMats->data(), extents[NAEA][NAEA]);
      boost::multi::array_ref<T,2> TNM(SM_TMats->data()+NAEA*NAEA, extents[NAEA][NMO]);  
    ...
    T MixedDensityMatrixForWoodbury(const MatA& hermA, const MatB& B, MatC&& C, 
      ...
      boost::multi::array_ref<T,2> TNN(TMat_NN.data(), extents[NEL][NEL]);
      assert(TMat_NM.num_elements() >= Nact*NEL);
      boost::multi::array_ref<T,2> TAB(TMat_NM.data(), extents[Nact][NEL]);
      assert(TMat_MM.num_elements() >= NMO*NEL);
      boost::multi::array_ref<T,2> TNM(TMat_MM.data(), extents[NEL][NMO]);
    ...
    T MixedDensityMatrixForWoodbury(const MatA& hermA, const MatB& B, MatC&& C,
      ...
      size_t cnt=0;
      boost::multi::array_ref<T,2> TNN(SM_TMats->data(), extents[NEL][NEL]);
      cnt+=TNN.num_elements();
      boost::multi::array_ref<T,2> TAB(SM_TMats->data()+cnt, extents[Nact][NEL]);
      cnt+=TAB.num_elements();
      boost::multi::array_ref<T,2> TNM(SM_TMats->data()+cnt, extents[NEL][NMO]);
    ...
    T OverlapForWoodbury(const MatA& hermA, const MatB& B, integer* ref, MatC&& QQ0) {
      ...
      boost::multi::array_ref<T,2> TNN(TMat_NN.data(), extents[NEL][NEL]);
      boost::multi::array_ref<T,2> TMN(TMat_MM.data(), extents[Nact][NEL]);
    ...
    T OverlapForWoodbury(const MatA& hermA, const MatB& B, integer* ref, MatC&& QQ0, communicator& comm) {
      ...
      boost::multi::array_ref<T,2> TNN(std::addressof(*SM_TMats->data()), extents[NEL][NEL]);
      boost::multi::array_ref<T,2> TMN(std::addressof(*(SM_TMats->data()+NEL*NEL)), extents[Nact][NEL]);
    ...
    void Propagate(Mat&& A, const MatP1& P1, const MatV& V, int order=6) {
      ...
      if(TMat_NM.num_elements() < NMO*NAEA)
        TMat_NM.resize(extents[NAEA][NMO]);
      boost::multi::array_ref<T,2> TMN(TMat_MN.data(), extents[NMO][NAEA]);
      boost::multi::array_ref<T,2> T1(TMat_NM.data(), extents[NMO][NAEA]);
      boost::multi::array_ref<T,2> T2(TMat_MM.data(), extents[NMO][NAEA]);
    ...
    void Propagate(Mat&& A, const MatP1& P1, const MatV& V, communicator& comm, int order=6) {
      ...
      boost::multi::array_ref<T,2> T0(SM_TMats->data(), extents[NMO][NAEA]);
      boost::multi::array_ref<T,2> T1(SM_TMats->data()+NMO*NAEA, extents[NMO][NAEA]);
      boost::multi::array_ref<T,2> T2(SM_TMats->data()+2*NMO*NAEA, extents[NMO][NAEA]);
      if(comm.root()) 
