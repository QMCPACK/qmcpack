...
TEST_CASE("SDetOps_double_serial", "[sdet_ops]")
{
  ...
  multi::array_ref Aref(m_a.data(),{NMO,NEL});
  ...
  // Test array_view
  REQUIRE(SDet.Overlap(A.sliced(),B) == Approx(5.10443199999999));
  REQUIRE(SDet.Overlap(A,B.sliced()) == Approx(5.10443199999999));

  multi::array A_ = A[indices[range_t(0,3)][range_t(0,2)]];
...
TEST_CASE("SDetOps_double_mpi3", "[sdet_ops]")
{
  ...
  // Test array_view
  REQUIRE(SDet.Overlap(A.sliced(),B,node) == Approx(5.10443199999999));
  REQUIRE(SDet.Overlap(A,B.sliced(),node) == Approx(5.10443199999999));
