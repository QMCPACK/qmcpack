...
void free_projection_walker_update(Wlk&& w, RealType dt, OMat&& overlap, Mat&& MFfactor,
                                   RealType Eshift, Mat&& hybrid_weight, WMat& work)
{
  int nwalk = w.size();
...
template<class Wlk, class OMat, class Mat, class WMat>
void hybrid_walker_update(Wlk&& w, RealType dt, bool apply_constrain, bool imp_sampl, 
                          RealType Eshift, OMat&& overlap,
                          Mat&& MFfactor, Mat&& hybrid_weight, WMat& work)
{
  int nwalk = w.size();
  // constexpr if can be used to avoid the memory copy, by comparing the pointer types
  // between WMat and Mat/OMat
  ...
  for(int i=0; i<nwalk; i++) {
    ...
    if(imp_sampl) ratioOverlaps = work[4][i]/old_ovlp; 
    if( !std::isfinite(ratioOverlaps.real()) && apply_constrain && imp_sampl ) {
      scale = 0.0;
      eloc = old_eloc;
    } else {
      scale = (apply_constrain?(std::max(0.0,
             std::cos( std::arg(ratioOverlaps)-work[5][i].imag() ) )):1.0);
      if(imp_sampl)
    ...
    if( (!std::isfinite(eloc.real())) || 
      ...
      eloc = ComplexType(std::max( std::min( eloc.real(), Eshift+std::sqrt(2.0/dt) ), 
...
void local_energy_walker_update(Wlk&& w, RealType dt, bool apply_constrain, RealType Eshift, 
                                OMat&& overlap, EMat&& energies,
                                Mat&& MFfactor, Mat&& hybrid_weight, WMat& work)
{
  int nwalk = w.size();
  // constexpr if can be used to avoid the memory copy, by comparing the pointer types
  ...
  for(int i=0; i<nwalk; i++) {
    ...
    if( !std::isfinite( (ratioOverlaps*work[8][i]).real()) && apply_constrain ) {
      ...
      scale = (apply_constrain?(std::max(0.0,
             std::cos( std::arg(ratioOverlaps)-work[8][i].imag() ) )):1.0);
