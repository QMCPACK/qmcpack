...
class PropagatorFactory
{
  ...
  Propagator& getPropagator(TaskGroup_& TG, const std::string& ID, 
                            Wavefunction& wfn, RandomGenerator_t* rng) 
  {
    ...
    auto p0 = propagators.find(ID);
    if( p0 == propagators.end() ) {
      auto newp = propagators.insert(
                std::make_pair(ID,buildPropagator(TG,xml->second,wfn,rng)));
      if(not newp.second)
  ...
  Propagator buildPropagator(TaskGroup_& TG, xmlNodePtr cur, 
                             Wavefunction& wfn, RandomGenerator_t* rng) {
    ...
    if(type == "afqmc")
      ...
      app_error()<<"Unknown Propagator type in PropagatorFactory::buildPropagator(): " 
                 <<type <<std::endl;
  ...
  Propagator buildAFQMCPropagator(TaskGroup_& TG, xmlNodePtr cur, 
                                  Wavefunction& wfn, RandomGenerator_t* r);
