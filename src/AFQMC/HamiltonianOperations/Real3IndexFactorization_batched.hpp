...
class Real3IndexFactorization_batched
{
  ...
  public:
        ...
        TBuff(iextensions<1u>{1},sp_allocator_)
    {
      ...
      size_t lank(0);
      for(auto& v: Lank) lank += v.num_elements();
      app_log()<<"****************************************************************** \n"
    ...
    void energy(Mat&& E, MatB const& Gc, int nd, MatC* KEleft, MatD* KEright, bool addH1=true, bool addEJ=true, bool addEXX=true) {
      ...
      if(E.size(0) != nwalk || E.size(1) < 3)
        APP_ABORT(" Error in AFQMC/HamiltonianOperations/Real3IndexFactorization_batched::energy(...). Incorrect matrix dimensions \n");
      ...
      if(addEXX) {
        ...
        for(int ispin=0, is0=0; ispin<nspin; ispin++) {
          ...
          SpCMatrix_ref Lan(make_device_ptr(Lank[nd*nspin + ispin].origin()),
  ...
  private:
    ...
    void set_buffer(size_t N) {
      if(TBuff.num_elements() < N) {
        app_log()<<" Resizing buffer space in Real3IndexFactorization_batched to " <<N*sizeof(SPComplexType)/1024.0/1024.0 <<" MBs. \n";
        {
