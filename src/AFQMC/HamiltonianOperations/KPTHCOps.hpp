...
app_log()<<" E time: " <<Timer.total("T0") <<" " <<Timer.total("T1") <<" " <<Timer.total("T2") <<"\n";
    ...
    void vHS(MatA & X, MatB&& v, double a=1., double c=0.) {
        boost::multi::array_ref<ComplexType,2> X_(X.origin(),{X.size(0),1});
        boost::multi::array_ref<ComplexType,2> v_(v.origin(),{1,v.size(0)});
        vHS(X_,v_,a,c);
    }
...
// combine n+ and n- by maping X({n+,n-},w) -> X(n,{w+,w-})
            ma::product(ma::T(X.sliced(nc0,nc0+nchol)),
      ...
      for(int Q=0, nc0=0; Q<nkpts; ++Q) {      // momentum conservation index
        if(Q%comm->size() == comm->rank()) {
          ...
          for(int G=0; G<nG; ++G) {
// combine n+ and n- by maping X({n+,n-},w) -> X(n,{w+,w-})
            ma::product(ma::T(X.sliced(nc0+nchol,nc0+2*nchol)),
    ...
    void vbias(MatA const& G, MatB&& v, double a=1., double c=0., int k=0) {
        boost::multi::array_cref<ComplexType,2> G_(G.origin(),{1,G.size(0)});
        boost::multi::array_ref<ComplexType,2> v_(v.origin(),{v.size(0),1});
        vbias(G_,v_,a,c,k);
    }
