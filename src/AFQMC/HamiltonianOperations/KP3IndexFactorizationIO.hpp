...
inline HamiltonianOperations loadKP3IndexFactorization(hdf_archive& dump, WALKER_TYPES type, int NMO, int NAEA, int NAEB, std::vector<PsiT_Matrix>& PsiT, TaskGroup_& TGprop, TaskGroup_& TGwfn, RealType cutvn, RealType cutv2)
{
  ...
  for(int Q=0; Q<nkpts; Q++)
    if( Q==Q0 )
      LQKikn.emplace_back( shmSpMatrix({nkpts,nmo_max*nmo_max*nchol_per_kp[Q]},
                                   shared_allocator<SPComplexType>{TGwfn.Node()}) );
  ...
  shmCMatrix haj({ndet*nkpts,(type==COLLINEAR?2:1)*nocc_max*nmo_max},shared_allocator<ComplexType>{TGwfn.Node()});
  if(TGwfn.Node().root()) std::fill_n(haj.origin(),haj.num_elements(),ComplexType(0.0));
...
// change get_PsiK to cast to the value_type of the result 
  ...
  if(TGwfn.Node().root()) {
    TGwfn.Cores().all_reduce_in_place_n(std::addressof(*haj.origin()),
                                     haj.num_elements(),std::plus<>());
    for(int Q=0; Q<LQKank.size(); Q++)
      TGwfn.Cores().all_reduce_in_place_n(std::addressof(*LQKank[Q].origin()),
...
inline void writeKP3IndexFactorization(hdf_archive& dump, WALKER_TYPES type, 
                              int NMO, int NAEA, int NAEB, 
                              ...
                              shmC3Tensor & vn0,
                              int nsampleQ, std::vector<RealType> & gQ,
