...
namespace afqmc
{
auto host_buffer_generator = std::make_shared<host_allocator_generator_type>(host_memory_resource{},
...
void update_buffer_generators()
{
  ...
  {
    app_error() << " Uninitialized host buffer generator in update_buffer_generator." << std::endl;
    APP_ABORT("");
  }
  ...
  if (localTG_buffer_generator != nullptr)
    localTG_buffer_generator->update();
}

void make_localTG_buffer_generator(mpi3::shared_communicator& local, std::size_t sz)
{
#if defined(ENABLE_CUDA) || defined(ENABLE_HIP)
  // do nothing, since localTG_buffer_generator is in device and setup in arch::INIT
#else
  if (localTG_buffer_generator == nullptr)
    localTG_buffer_generator =
        std::make_shared<localTG_allocator_generator_type>(shm::memory_resource_shm_ptr_with_raw_ptr_dispatch{local},
