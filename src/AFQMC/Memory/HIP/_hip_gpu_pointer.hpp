...
public:
  ...
  hip_gpu_reference& operator++(void)
  {
  ...
  value_type operator+(hip_gpu_reference const& rhs) const { return this->val() + rhs.val(); }
  value_type operator-(hip_gpu_reference const& rhs) const { return this->val() - rhs.val(); }
  value_type operator*(hip_gpu_reference const& rhs) const { return this->val() * rhs.val(); }
  value_type operator/(hip_gpu_reference const& rhs) const { return this->val() / rhs.val(); }

  friend value_type operator+(value_type lhs, hip_gpu_reference const& rhs) { return lhs + rhs.val(); }
  friend value_type operator-(value_type lhs, hip_gpu_reference const& rhs) { return lhs - rhs.val(); }
  friend value_type operator*(value_type lhs, hip_gpu_reference const& rhs) { return lhs * rhs.val(); }
  friend value_type operator/(value_type lhs, hip_gpu_reference const& rhs) { return lhs / rhs.val(); }
  ...
  friend bool operator==(value_type const& lhs, hip_gpu_reference const& rhs) { return lhs == rhs.val(); }
  friend bool operator!=(value_type const& lhs, hip_gpu_reference const& rhs) { return lhs != rhs.val(); }
  friend bool operator>(value_type const& lhs, hip_gpu_reference const& rhs) { return lhs > rhs.val(); }
  friend bool operator<(value_type const& lhs, hip_gpu_reference const& rhs) { return lhs < rhs.val(); }
  friend bool operator>=(value_type const& lhs, hip_gpu_reference const& rhs) { return lhs >= rhs.val(); }
...
struct hip_gpu_allocator
{
  ...
  void deallocate(device_pointer<T> ptr, size_type) { hipFree(ptr.impl_); }
  bool operator==(hip_gpu_allocator const& other) const { return true; }
  bool operator!=(hip_gpu_allocator const& other) const { return false; }
...
template<class T, class Q1, class Q2>
multi::array_iterator<T, 1, T*> copy(multi::array_iterator<Q1, 1, qmc_hip::device_pointer<Q2>> first,
                                     multi::array_iterator<Q1, 1, qmc_hip::device_pointer<Q2>> last,
...
template<class T, class Q1, class Q2, typename Size>
multi::array_iterator<T, 1, T*> copy_n(multi::array_iterator<Q1, 1, qmc_hip::device_pointer<Q2>> first,
                                       Size N,
                                       multi::array_iterator<T, 1, T*> dest)
{
  static_assert(std::is_same<typename std::decay<Q1>::type, T>::value, "Wrong dispatch.\n");
