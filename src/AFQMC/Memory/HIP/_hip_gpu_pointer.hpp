...
#ifndef AFQMC_HIP_GPU_POINTERS_HPP
#define AFQMC_HIP_GPU_POINTERS_HPP
...
template<class T> struct hip_gpu_allocator;
template<class T> struct device_pointer;

// no const for now
template<class T>
struct hip_gpu_reference {
  ...
  hip_gpu_reference(pointer const& gpu_ptr) : impl_(to_address(gpu_ptr)) {}
  hip_gpu_reference(hip_gpu_reference<T> const& gpu_ref) = default;

  // assignment
  hip_gpu_reference& operator=(hip_gpu_reference const& x) {
    if(hipSuccess != hipMemcpy(impl_,x.impl_,sizeof(T),hipMemcpyDefault))
     throw std::runtime_error("Error: hipMemcpy returned error code.");
    return *this;
  }

  hip_gpu_reference& operator=(value_type const& x) {
    if(hipSuccess != hipMemcpy(impl_,std::addressof(x),sizeof(T),hipMemcpyDefault))
     throw std::runtime_error("Error: hipMemcpy returned error code.");
    return *this;
  }
  ...
  operator value_type&()
  {
    if(hipSuccess != hipMemcpy(std::addressof(host_impl_),impl_,sizeof(T),hipMemcpyDefault))
     throw std::runtime_error("Error: hipMemcpy returned error code.");
    return host_impl_;
  }
  ...
  friend auto  addressof(hip_gpu_reference const& rhs) { return rhs.addressof(); }
  friend auto addressof(hip_gpu_reference && rhs) { return rhs.addressof(); }
  friend auto addressof(hip_gpu_reference & rhs) { return rhs.addressof(); }

  void swap(hip_gpu_reference& other) { std::swap(impl_,other.impl_); }

  hip_gpu_reference&  operator++ (void) { kernels::op_plus(impl_,T(1)); return *this; }
  hip_gpu_reference&  operator-- (void) { kernels::op_minus(impl_,T(1)); return *this; }

  value_type  operator++ (int) {
    value_type res;
    if(hipSuccess != hipMemcpy(std::addressof(res),impl_,sizeof(T),hipMemcpyDefault))
     throw std::runtime_error("Error: hipMemcpy returned error code.");
    return res++;
  }
  value_type  operator-- (int) {
  ...
  hip_gpu_reference&  operator+= (const value_type &rhs)
  ...
  hip_gpu_reference&  operator+= (hip_gpu_reference const& rhs)
  ...
  friend value_type& operator+=(value_type& lhs, hip_gpu_reference const& rhs)
  ...
  value_type  operator+ (hip_gpu_reference const& rhs) const
  ...
  bool operator==(hip_gpu_reference const& rhs) const { return this->val()==rhs.val(); }
  bool operator!=(hip_gpu_reference const& rhs) const { return this->val()!=rhs.val(); }
  bool operator>(hip_gpu_reference const& rhs) const { return this->val()>rhs.val(); }
  bool operator<(hip_gpu_reference const& rhs) const { return this->val()<rhs.val(); }
  bool operator>=(hip_gpu_reference const& rhs) const { return this->val()>=rhs.val(); }
  ...
  friend bool operator==(value_type const& lhs, hip_gpu_reference const& rhs)
    { return lhs==rhs.val(); }
  friend bool operator!=(value_type const& lhs, hip_gpu_reference const& rhs)
    { return lhs!=rhs.val(); }
  friend bool operator>(value_type const& lhs, hip_gpu_reference const& rhs)
  ...
  friend std::ostream& operator<<(std::ostream& os, hip_gpu_reference const& obj)
  {
    os<<obj.val();
    return os;
  }

  friend std::istream& operator<<(std::istream& is, hip_gpu_reference & obj)
  {
    value_type val;
    is>>val;
    obj = val;
...
/*
  hip_gpu_reference&  operator<<= (const T &rhs)
...
*/
  ...
  value_type val() const {
    ...
    if(hipSuccess != hipMemcpy(std::addressof(res),impl_,sizeof(value_type),hipMemcpyDefault))
     throw std::runtime_error("Error: hipMemcpy returned error code.");
...
// this is wrong!!! but no synthetic references yet!!!
  using reference = hip_gpu_reference<T>;
  using const_reference = hip_gpu_reference<T> const;
  using iterator_category = std::random_access_iterator_tag;
  static const int memory_type = GPU_MEMORY_POINTER_TYPE;
  using default_allocator_type = hip_gpu_allocator<T>;
  friend class hip_gpu_allocator<T>;
  friend class device_pointer<typename std::decay<T>::type>;
  default_allocator_type default_allocator() const{ return hip_gpu_allocator<T>{}; };
...
//  operator device_pointer<T const>() const{return device_pointer<T const>{impl_}; }
  ...
  device_pointer& operator++() {++impl_; return *this;}
  device_pointer& operator--() {--impl_; return *this;}
  device_pointer& operator+=(std::ptrdiff_t d){impl_ += d; return *this;}
  device_pointer& operator-=(std::ptrdiff_t d){impl_ -= d; return *this;}
 ...
 */
template<class T> struct hip_gpu_allocator{
  template<class U> struct rebind{typedef hip_gpu_allocator<U> other;};
  using element_type = T;
  using value_type = T;
  using const_value_type = T const;
  using pointer = device_pointer<T>;
  ...
  hip_gpu_allocator() = default;
  ~hip_gpu_allocator() = default;
  hip_gpu_allocator(hip_gpu_allocator const& other) = default;
  template<class U>
  hip_gpu_allocator(hip_gpu_allocator<U> const& other) {}

  device_pointer<T> allocate(size_type n, const void* hint = 0){
    if(n == 0) return device_pointer<T>{};
    T* p;
    if(hipSuccess != hipMalloc ((void**)&p,n*sizeof(T))) {
...
//    return device_pointer<T>{p};
  ...
  void deallocate(device_pointer<T> ptr, size_type){
    hipFree(ptr.impl_);
  }
  bool operator==(hip_gpu_allocator const& other) const{
    return true;
  }
  bool operator!=(hip_gpu_allocator const& other) const{
    return false;
 ...
 * hip code upstream due to the template needed to pass the UnaryOperator
...
device_pointer<T> copy_n(device_pointer<T> const A, Size n, device_pointer<T> B) {
  if(hipSuccess != hipMemcpy(to_address(B),to_address(A),n*sizeof(T),hipMemcpyDefault))
   throw std::runtime_error("Error: hipMemcpy returned error code.");
  return B+n;
}
...
device_pointer<T> copy_n(T* const A, Size n, device_pointer<T> B) {
  if(hipSuccess != hipMemcpy(to_address(B),A,n*sizeof(T),hipMemcpyDefault))
   throw std::runtime_error("Error: hipMemcpy returned error code.");
  return B+n;
}
...
T* copy_n(device_pointer<T> const A, Size n, T* B) {
  if(hipSuccess != hipMemcpy(B,to_address(A),n*sizeof(T),hipMemcpyDefault))
   throw std::runtime_error("Error: hipMemcpy returned error code.");
  return B+n;
}
...
template<class Alloc, typename T>
device_pointer<T> uninitialized_copy(Alloc &a, device_pointer<T> const Abeg, device_pointer<T> const Aend, device_pointer<T> B) {
  return copy_n(Abeg,std::distance(Abeg,Aend),B);
}
...
template<typename T>
void fill2D(int n, int m, qmc_hip::device_pointer<T> first, int lda, T const& val) {
  assert(lda >= m);
...
/**************** copy *****************/
// Can always call hipMemcopy2D like you do in the blas backend

template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_fill_n(
                    Alloc &a,
                    multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> first,
                    Size n, T const& val){
  if(n == 0) return first;
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_fill(
                    Alloc &a,
                    multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> first,
                    multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> last, T const& val){
...
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> copy(
           multi::array_iterator<Q1, 1, qmc_hip::device_pointer<Q2>> first,
           multi::array_iterator<Q1, 1, qmc_hip::device_pointer<Q2>> last,
...
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> copy_n(
             multi::array_iterator<Q1, 1, qmc_hip::device_pointer<Q2>> first,
             Size N,
             multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> dest ){
  static_assert(std::is_same<typename std::decay<Q1>::type,T>::value,"Wrong dispatch.\n");
...
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_copy(
                         Alloc &a,
                         ForwardIt first,
                         ForwardIt last,
                         multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> dest ){
...
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_copy(
                         Alloc &a,
                         multi::array_iterator<Q, 1, qmc_hip::device_pointer<Q>> first,
...
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_copy_n(
                           Alloc &a,
                           multi::array_iterator<Q, 1, qmc_hip::device_pointer<Q>> first,
                           Size N,
                           multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> dest ){
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_default_construct_n(
                        Alloc &a,
                        multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> first, Size n){
  return uninitialized_fill_n(first,n,T());
}

template<class Alloc, typename T>
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_default_construct(
                        Alloc &a,
                        multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> first,
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_value_construct_n(
                        Alloc &a,
                        multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> first, Size n){
  return uninitialized_fill_n(first,n,T());
}
...
template<class Alloc, typename T>
multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> uninitialized_value_construct(
                        Alloc &a,
                        multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> first,
                        multi::array_iterator<T, 1, qmc_hip::device_pointer<T>> last){
  return uninitialized_fill_n(a,first,std::distance(first,last),T());
