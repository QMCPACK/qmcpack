...
struct device_reference {
  ...
  device_reference&  operator++ (void) { kernels::op_plus(impl_,T(1)); return *this; }
  device_reference&  operator-- (void) { kernels::op_minus(impl_,T(1)); return *this; }
  ...
  value_type  operator+ (device_reference const& rhs) const 
  ...
  friend bool operator==(value_type const& lhs, device_reference const& rhs)
    { return lhs==rhs.val(); }
  friend bool operator!=(value_type const& lhs, device_reference const& rhs)
    { return lhs!=rhs.val(); }
  friend bool operator>(value_type const& lhs, device_reference const& rhs)
...
//  static const int memory_type = GPU_MEMORY_POINTER_TYPE; 
  using default_allocator_type = device_allocator<T>;
  friend class device_allocator<T>;
  friend class device_pointer<typename std::decay<T>::type>;
  default_allocator_type default_allocator() const{ return device_allocator<T>{}; };
...
template<class T> struct device_allocator{
  ...
  bool operator==(device_allocator const& other) const{
    return true; 
  }
  bool operator!=(device_allocator const& other) const{
    return false; 
...
multi::array_iterator<T, 1, device::device_pointer<T>> copy( 
  ...
  arch::memcopy2D(to_address(base(dest)),sizeof(T)*stride(dest),
...
multi::array_iterator<T, 1, device::device_pointer<T>> uninitialized_default_construct_n(
                        Alloc &a, 
                        multi::array_iterator<T, 1, device::device_pointer<T>> first, Size n){
  return uninitialized_fill_n(first,n,T());
}
