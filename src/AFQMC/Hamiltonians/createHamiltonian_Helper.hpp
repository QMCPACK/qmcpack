...
namespace afqmc
{
  ...
  inline void count_allowed_terms(int NMO, std::vector<std::size_t>& sz, std::vector<s4D<ValueType> >& vs4D, std::map<IndexType,std::pair<bool,IndexType>>&  occ_a)
  {
    for(auto& v:vs4D)
  ...
  inline void count_allowed_terms(std::vector<std::size_t>& sz, std::vector<s4D<ValueType> >& vs4D, std::map<IndexType,std::pair<bool,IndexType>>&  occ_a, std::map<IndexType,std::pair<bool,IndexType>>& occ_b)
  {
  }

  template<class Container>
  inline void add_allowed_terms(int NMO,std::vector<s4D<ValueType> >& vs4D, std::map<IndexType,std::pair<bool,IndexType>>&  occ_a, Container& V)
  {
    for(auto& vi:vs4D)
      if( occ_a[std::get<0>(vi)].first && occ_a[std::get<1>(vi)].first ) 
        V.emplace( occ_a[std::get<0>(vi)].second*NMO + std::get<2>(vi),
                   occ_a[std::get<1>(vi)].second*NMO + std::get<3>(vi), 
  ...
  template<class Container>
  inline void add_allowed_terms(int NMO,std::vector<s4D<ValueType> >& vs4D, std::map<IndexType,std::pair<bool,IndexType>>&  occ_a, std::map<IndexType,std::pair<bool,IndexType>>& occ_b, Container& V , bool needs_locks=false, bool GHF=false)
  {
  }

  inline void push_ijkl(int NMO, OrbitalType i, OrbitalType j, OrbitalType k, OrbitalType l, ValueType V, std::vector<s4D<ValueType> >& v, bool GHF=false) {
    long ik,jl;
    if(GHF) {
      ik = i*2*NMO+k;
      jl = j*2*NMO+l;
  ...
  //  J2a = <ik|lj>
  //  J3a = <il|jk> 
  inline void find_all_contributions_to_hamiltonian_collinear(int NMO, OrbitalType i, OrbitalType j, OrbitalType k, OrbitalType l, ValueType J1, ValueType J2, ValueType J3, ValueType J1a, ValueType J2a, ValueType J3a, double cut, std::vector<s4D<ValueType> >& v) 
  { 
    // simple algorithm for now
    // 1. add all contributions blindly
    // 2. check for repeated and remove
  ...
  //  J2a = <ik|lj>
  //  J3a = <il|jk> 
  inline void find_all_contributions_to_hamiltonian_ghf(int NMO, OrbitalType i, OrbitalType j, OrbitalType k, OrbitalType l, ValueType J1, ValueType J2, ValueType J3, ValueType J1a, ValueType J2a, ValueType J3a, double cut, std::vector<s4D<ValueType> >& v) 
  { 
    // simple algorithm for now
    // 1. add all contributions blindly
    // 2. check for repeated and remove
  ...
  //  J2a = <ik|lj>
  //  J3a = <il|jk> 
  inline void find_all_contributions_to_hamiltonian_general(int NMO, OrbitalType i, OrbitalType j, OrbitalType k, OrbitalType l, ValueType J1, ValueType J2, ValueType J3, ValueType J1a, ValueType J2a, ValueType J3a, double cut, std::vector<s4D<ValueType> >& v) 
  {
