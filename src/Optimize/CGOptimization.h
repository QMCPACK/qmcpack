...
template<class T>
class CGOptimization: public OhmmsElementBase, private NRCOptimization<T> {
  public:

  typedef typename NRCOptimization<T>::Return_t Return_t;
...
CGOptimization<T>::CGOptimization(ObjectFuncType* atarget):
  ...
  NumSteps(100),CostTol(1.e-6),GradTol(1.e-6),GammaTol(1.e-4),msg_stream(0) {
...
bool CGOptimization<T>::optimize() {
  ...
  do {
    ...
    if(this->lineoptimization()) { //successful lineminimization
