...
//////////////////////////////////////////////////////////////////
// -*- C++ -*-
/** @file BFGSOptimization.h
 * @brief Declaration and definition of optimization class based on MacOptII
 */
#ifndef QMCPLUSPLUS_BFGSOptimization_H
#define QMCPLUSPLUS_BFGSOptimization_H

#include "Optimize/OptimizeBase.h"
#include "OhmmsData/ParameterSet.h"

template<class T>
class BFGSOptimization: public MinimizerBase<T> {
public:
...
vector<Return_t> PastGradient, NewGradient;
Return_t PastCost, NewCost;
vector< vector<Return_t> > Hessian;


BFGSOptimization(ObjectFuncType* atarget=0):  TargetFunc(atarget), 
...
a_linmin_g3(0.5), a_restart(0), xycleanup(4), Gfactor(3), deltaG(1e-4),
maxCG(28), Hevals(0)
{
...
PastGradient.resize(newSize,0.0); NewGradient.resize(newSize,0.0);
vector< vector<Return_t> > TH(newSize,vector<Return_t> (newSize,0.0));
for (int i=0;i<newSize;i++) TH[i][i]=1.0;
Hessian = TH;
if (a_verbose>5)
{
...
void reset()
{
	Past_a_xi.clear(); Past_Parms.clear(); Past_a_h.clear();
...
void dfuncOnly(vector<Return_t> RT, vector<Return_t> &FG)
{
...
//We will use this to build the hessian
NewCost = TargetFunc->Cost();
PastCost= NewCost;
PastGradient=NewGradient=FG;
}
...
void dfuncandHessian(vector<Return_t> RT, vector<Return_t> &FG)
{
	//For Hessian
	PastCost=NewCost;
	PastGradient=NewGradient;
	vector<Return_t> tmpPs(NumParams,0.0);
	for(int k=0; k<NumParams; k++) tmpPs[k] = TargetFunc->Params(k) ;
	Return_t dh=1.0/(2.0*Displacement);
	for(int i=0; i<NumParams ; i++) {
		...
		TargetFunc->Params(i) = RT[i]- Displacement;
		Return_t CostMinus = TargetFunc->Cost(); 
		NewGradient[i] = FG[i] = (CostPlus-CostMinus)*dh;
	}
	for(int k=0; k<TargetFunc->NumParams(); k++) TargetFunc->Params(k)=tmpPs[k];
	//We will use this to build the hessian
	NewCost=TargetFunc->Cost();
...
void updateHessian()
{
	if ( a_verbose > 3 ) cout<<" UPDATING HESSIAN INVERSE"<<endl;
	++Hevals;
	int CurLineDir = Past_a_h.size()-1;
	vector<Return_t> DG(NewGradient);
	vector<Return_t> HDG(NumParams,0.0);
	for(int i=0; i< NumParams; i++) DG[i] -= PastGradient[i];
	for(int i=0; i< NumParams; i++)
	{
		for(int j=0; j< NumParams; j++) HDG[i] += Hessian[i][j]*DG[j];
	}
	Return_t c0(0.0) ,c1(0.0) ,c2(0.0);
	Return_t sumDG(0), sumxi(0);
	for(int i=0; i< NumParams; i++)
	{
		c0 += DG[i] * a_h[i];
		c1 += DG[i] * HDG[i];
	...
	if ( a_verbose > 3 ) cout<<sumDG<<"  "<< sumxi<<"  "<< c0 <<"  "<<c1<<endl;
	if (c0*c0 > 1e-4*sumDG*sumxi)
	{
		c0 = 1.0/c0;
		c2 = 1.0/c1;
		for(int i=0; i< NumParams; i++) DG[i] = c0*a_h[i] - c2*HDG[i];
		for(int i=0; i< NumParams; i++)
		{
			for(int j=0; j< NumParams; j++) 
			{
				Hessian[i][j] += c0*a_h[i]*a_h[j] - c2*HDG[i]*HDG[j] + c1*DG[i]*DG[j];
			}
		}
		if ( a_verbose > 3 ) cout<<" UPDATED HESSIAN INVERSE"<<endl;
	}
	if (a_verbose>4)
  {
		for (int i=0;i<NumParams;i++)
...
else 
{
	...
	if ((deltaG > std::fabs(gg-GGnew)) & (CG_ortho>0) ) 
	{
		...
		if (a_verbose>3)
		{
			cout<<"Temporarily switching to Steepest Descent"<<endl;
		}
	}
	Past_Parms.push_back(Parms);
	Failed_Last=false;
...
{
	if ((Hevals==maxCG) & (a_verbose>0)) cout<<"Switching to Hessian for Search"<<endl;
	//Setting new line direction using Hessian
	vector<Return_t> tmpA_H(NumParams,0.0);
	for (int i = 0 ; i < NumParams ; i ++ ) 
	{
		for (int j = 0 ; j < NumParams ; j ++ ) tmpA_H[i] -= Hessian[i][j]*a_xi[j];
	}
// rescale this line direction.
	Return_t a_htyp(0);
	for (int i = 0 ; i < NumParams ; i ++ ) a_htyp+=tmpA_H[i]*tmpA_H[i];
	a_htyp = std::pow(a_htyp,-0.5);
	for (int i = 0 ; i < NumParams ; i ++ ) tmpA_H[i] *= a_htyp;
...
if ( a_verbose > 1 )  printf (" s = %6.3g: x = %6.3g \n",s,  x  );
if (s!=s)
{
	if ( a_verbose > 1 )  printf ("Bombed out of line minimization");
  return -1;
}
...
if ( a_rich || a_restart ) dfuncandHessian(Parms , a_xi);
else updateHessian();
Past_a_xi.push_back(a_xi);
