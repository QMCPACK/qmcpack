...
void uniform_fail(mpi3::communicator const& comm) {  // cppcheck-suppress [unusedFunction,unmatchedSuppression]
	...
	std::this_thread::sleep_for(comm.rank() * 1s);  // NOLINT(misc-include-cleaner) bug in clang-tidy 18.1

	std::cout << "uniform_fail in n = " << comm.rank() << " is about to fail\n";
...
//  if(comm.rank() > 1) {
//      std::cout << "nonuniform_fail in n = " << comm.rank() << " is about to fail\n";
...
//  std::cout << "not essential message: aborting from rank " << comm.rank() << '\n';
//  comm.abort();
// }
...
auto main(int argc, char** argv) -> int try {
	...
	std::set_terminate([] {
		using namespace std::chrono_literals;
		mpi3_timed_terminate(20s);  // NOLINT(misc-include-cleaner) bug in clang-tidy 18.1.3
	});
