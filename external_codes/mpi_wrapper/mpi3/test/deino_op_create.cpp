...
int mpi3::main(int /*argc*/, char** /*argv*/, mpi3::environment& mpie) {  // NOLINT(bugprone-exception-escape)
...

	{
		...
		for(int i = 0; i != world.size(); ++i) {  // NOLINT(altera-id-dependent-backward-branch,altera-unroll-loops)
			correct_result = correct_result + singleton{i};
		}  // NOLINT(altera-unroll-loops,altera-id-dependent-backward-branch)
		assert(result == correct_result);
	}
	{
		auto data = std::vector<singleton>{
			{{world.rank()}, {world.rank()}, {world.rank()}}
		};
