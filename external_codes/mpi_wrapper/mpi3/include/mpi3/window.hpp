...
class window<void> {
 ...
 public:
	...
	window(communicator const& c, T* b, Size n = 0) {
		...
		(b, n * sizeof(T), alignof(T), MPI_INFO_NULL, c.get(), &impl_);
	...
	// }
	friend void swap(window& a, window& b) noexcept { std::swap(a.impl_, b.impl_); }
	~window() { clear(); }  // NOLINT(bugprone-exception-escape)
	...
	void accumulate_n(It1 first, Size count, int target_rank, int target_disp = 0) {
		...
		MPI_(Accumulate)
		(data(first), count, detail::basic_datatype<V>{}, target_rank, target_disp, target_count, detail::basic_datatype<V>{}, MPI_SUM, impl_);
	}
	...
	auto operator&() const { return impl_; }  // NOLINT(google-runtime-operator)
	//  void free_keyval(...);
	...
	void flush_local() { flush_local_all(); }
	...
	mpi3::size_t const& size() const {
		...
		int       flag;    // NOLINT(cppcoreguidelines-init-variables) delayed init
		MPI_(Win_get_attr)
		(impl_, MPI_WIN_SIZE, &size_p, &flag);
		assert(flag);
		return *size_p;
	}
	int const& disp_unit() const {
		...
		int  flag;         // NOLINT(cppcoreguidelines-init-variables) delayed init
		MPI_(Win_get_attr)
		(impl_, MPI_WIN_DISP_UNIT, &disp_unit_p, &flag);
		assert(flag);
		return *disp_unit_p;
	}
	...
	void fetch_sum_value(T const& origin, T& target, int target_rank, int target_disp = 0) const {
		MPI3_CALL(MPI_Fetch_and_op)
		(&origin, &target, detail::basic_datatype<T>{}, target_rank, target_disp, MPI_SUM, impl_);
	}
	template<class T>
	void fetch_prod_value(T const& origin, T& target, int target_rank, int target_disp = 0) const {
		MPI3_CALL(MPI_Fetch_and_op)
		(&origin, &target, detail::basic_datatype<T>{}, target_rank, target_disp, MPI_PROD, impl_);
	}
	template<class T>
	void fetch_replace_value(T const& origin, T& target, int target_rank, int target_disp = 0) const {
		MPI3_CALL(MPI_Fetch_and_op)
		(&origin, &target, detail::basic_datatype<T>{}, target_rank, target_disp, MPI_REPLACE, impl_);
	}
	template<
		class CI1, class CI2,
	...
	void fetch_replace(CI1 it1, CI2 it2, int target_rank, int target_disp = 0) const {
		MPI3_CALL(MPI_Fetch_and_op)
		(std::addressof(*it1), std::addressof(*it2), DatatypeT{}, target_rank, target_disp, MPI_REPLACE, impl_);
	}
	...
	void put_n(ContiguousIterator it, std::size_t n, int target_rank, int target_disp = 0) const {
		...
		(
			...
			static_cast<int>(n),        /*int origin_count = 1 */
			detail::basic_datatype<typename std::iterator_traits<ContiguousIterator>::value_type>{},
			target_rank, /*int target_rank = 1*/
			static_cast<MPI_Aint>(target_disp), /*int target_disp = i*/
			static_cast<int>(n),           /*int target_count = 1*/
			detail::basic_datatype<typename std::iterator_traits<ContiguousIterator>::value_type>{},
			impl_
		);
	}
...
class window : public window<void> {
 ...
 public:
	...
	mpi3::size_t size() const { return window<void>::size() / sizeof(T); }  // cppcheck-suppress duplInheritedMember ;
};
...
struct shm_pointer : window<> {
	...
	T* local_ptr(int rank) const {
		...
		MPI_(Win_shared_query)
		(window::impl_, rank, &size, &disp_unit, &baseptr);
	...
	mpi3::size_t local_size(int rank) const {
		...
		void*        baseptr;    // NOLINT(cppcoreguidelines-init-variables) delayed init
		MPI_(Win_shared_query)
		(window::impl_, rank, &ret, &disp_unit, &baseptr);
...
class epoch {
 ...
 public:
	explicit epoch(Window& win) : win_{win} { win_.fence(); }

	epoch(epoch const&) = delete;
	epoch(epoch&& other) = delete;  // { win_.fence(); }

	// auto fence() && -> epoch { engaded_ = false; return epoch(*Pwin_); }  // win_.fence(); return *this; }

	~epoch() { win_.fence(); }

	auto operator=(epoch const&) = delete;
	auto operator=(epoch&&) = delete;

	template<class... Args>
	auto put_n(Args&&... args) { win_.put_n(std::forward<Args>(args)...); }

	template<class... Args>
	auto get_n(Args&&... args) { win_.get_n(std::forward<Args>(args)...); }
};
