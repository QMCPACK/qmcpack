...
The fundamental goal of the library is that the arrays and iterators can be used out-of-the-box with pass:[C++] Standard Library, in particular the Standard Template Library (STL) and the algorithms it provides.
The clearest example of this is that `std::sort` works with array as it is shown in a previous section.

Along with STL itself, the library tries to interact with other existing quality pass:[C++] libraries listed below.
...
----
	...
	// for(auto i : R.extent()) {R[i] = A[0][i] + V[i];}  // ...and then mutated here
----
...
In this example, the expression will yield the maximum value of the row sums:
...
There are many range views to enumerate here, including link:https://godbolt.org/z/Pjc9bjoe6[`std::reverse`] (reverse the elements of the leading dimension).

NOTE: clang-15, when paired with GCC's standard library, has bugs that breaks some range types (not particular to this library);
range-adaptor pipes (e.g., `std::views::reverse`, `std::views::transform`) may fail to compile under that specific compiler/standard-library combination.
Later clang releases are unaffected.

=== Polymorphic Memory Resources

In addition to supporting classic allocators (`std::allocator` by default), the library is compatible with pass:[C++17]'s link:https://en.cppreference.com/w/cpp/header/memory_resource[polymorphic memory resources (PMR)], which allows using advanced allocation strategies, including preallocated buffers.
...
int main() {
	...
	assert( buffer != std::string{"XXXXXXXXXXXX"} );  // overwritten w/elements, implementation-dependent (libstdc\++ consumes from left, and libc\++, from the right)
}
----
...
The one-dimensional case `multi::array<T, 1>` is special and partially overlaps the functionality with other dynamic array implementations, such as `std::vector`.
The analogies are obvious, both are contiguous memory, provide random access, use allocated dynamic memory, and sizes can be selected at runtime, 
so it is to be expected to be equivalent to a certain degree.
...
At the same time, we should know about the differences in the design goals of `std::vector` and `multi::array`, to understand their diverging interface, rather than forcing an equivalance.
`multi::array<T, 1>` is the one-dimensional special case of general multidimensional arrays oriented toward high performance data access and manipulation;
`std::vector` was conceived specifically as a _growable_ allocated array, with optimizations tailored to that role, such as amortized-cost push-backs.
While `std::vector` can be used in high-performance contexts, some points of friction stem from this original design.
These notoriously include link:https://lemire.me/blog/2012/06/20/do-not-waste-time-with-stl-vectors/[enforced initialization], and awkward fine-grained control over excess memory capacity (`shrink_to_fit` or the "swap trick" idiom in Item 14 of the link:https://www.informit.com/store/effective-stl-50-specific-ways-to-improve-your-use-9780201749625[Effective STL book]).
...
In the same way, the `clear()` operation is replaced by an operation called `reset()`;
the reason is that for `std::vector`, `clear()` doesn't frees the memory but sets the size to zero.
Regarding memory fine control, a second overload `reset(new_size)` reallocates if neededed without necessary preserving the elements.

In a departure from standard containers, elements are left uninitialized if they have a trivial constructor.
...
Another difference is in scope, with the appropriate specification of the memory allocator, `multi::array<T, 1, Alloc>` can refer to special memory not supported by `std::vector`, such as GPU memory or other fancy memory (mapped memory, see example.)
...
Despite these divergences, an immutable `std::vector<T>` can be completely replaced by `multi::array<T, 1>`.

A closedly related type is `std::span`.
The `std::span` (pass:[C++20]) does not have a well-defined reference- or pointer-semantics; it doesn't respect `const` correctness in generic code.
...
Multidimensional arrays can interoperate with pass:[C++23]'s non-owning link:https://en.cppreference.com/w/cpp/container/mdspan.html[`mdspan`].
...
Some member functions internally perform algorithms that can benefit from execution policies;
...
Execution policies and ranges can be mixed (`x` and `y` can be 1-dimensional arrays, of any arithmetic element type)
...
Unfortunately, the pass:[C++] language does not provide facilities for serialization, and the standard library doesn't either.
...
Here is a small implementation of save and load functions for an array to JSON format with the Cereal library.
...
Cryptic errors and crashes can occur if serialization libraries, file formats, or pass:[C++] types are mixed between writes and reads.
...
Here is a comparison of speeds when serializing and deserializing a 134 MB 4-dimensional array with random `double` elements.
...
Lambdas which are expected to be executed in the GPU need to be declared `pass:c[__host__ __device__]`.
Take into account that lambdas executed in the device cannot take captures by reference.
Library's restrictions do not work on `pass:c[__device__]`-only functions due to limitations of the CUDA language.
In addition, `pass:c[__host__ __device__]` cannot take initializing captures (e.g. `[var = ...]`),
or `auto` arguments.
As a partial workaround, the library provides a macro `BOOST_MULTI_HD_LAMBDA` (note the parentheses).
...
(Alternatively the functions can be defined as separate class with a `pass:c[__host__ __device__ operator()]`).
...
As a quick way to improve performance in many cases, here is a recipe for a `caching_allocator` which uses a global (one per thread) memory pool that can replace the default Thrust allocator.
...
link:https://developer.nvidia.com/cuda/toolkit[CUDA] is a dialect of pass:[C++] that allows writing pieces of code for GPU execution, known as "CUDA kernels".
CUDA code is generally "low level" (less abstracted), but it can be used in combination with CUDA Thrust or the CUDA runtime library, especially to implement custom algorithms.
Although code inside kernels has certain restrictions, most Multi features can be used. 
(Most functions in Multi, except those involving memory allocations, are marked `pass:c[__device__]` to allow this.)
...
Here is an example implementation for matrix multiplication, in combination with Thrust and Multi:
...
HPX (High Performance ParallelX) is a library for parallelism and concurrency that implements the pass:[C++] parallel algorithms interface while extending it to distributed and asynchronous execution across multi-core and multi-node systems.

Intranode parallelism works out of the box with this library.
These are a few examples, using reduction and for-each in 1D and 2D.
...
The library doesn't have a "pretty" printing facility to display arrays, beyond basic debug-type output.
Although it is not ideal, for more control, arrays can be printed and formatted by looping over elements and dimension, as shown in other examples (using standard streams).
...
(A similar effect can be achieved with link:https://godbolt.org/z/4ehd4s5vf[experimental pass:[C++23] `std::print` in pass:[libc++]].)
...
link:https://godbolt.org/z/7dcKxa8of[(open with Compiler Explorer)]
...
The library works out-of-the-box via the automatic `cppyy` bindings, providing a syntax similar to that of pass:[C++].
Additionally, the memory layouts are compatible with NumPy which makes it possible to interoperate pass:[C++] and Python code without memory copies.
The examples in this section were verified with cppyy 3.5.0 and NumPy 2.5.
...
[source,python]
----
# We can also create a 2x2 array from its extensions, and assign its elements:
a2d = multi.array['double', 2](multi.extensions_t[2](2, 2), 0.0)
a2d[0][0] = 1.0; a2d[0][1] = 2.0
a2d[1][0] = 3.0; a2d[1][1] = 4.0

# We can retrieve information from an individual element
print(a2d[0][1])
----
> ```
> 2.0
...
NOTE: As of cppyy 3.5.0, converting a nested Python list directly into a bidimensional array (e.g. `multi.array['double', 2]([[1.0, 2.0], [3.0, 4.0]])`) crashes the interpreter; construct from extensions and assign the elements instead.
...
Take into account that assignment in Python differs semantically from that of pass:[C++].
...
To replace the contents without rebinding the name, use the pass:[C++] assignment operator, which cppyy exposes as `__assign__`:

[source,python]
----
a2d = multi.array['double', 2]()  # empty 2D array again
a2d.__assign__(multi.array['double', 2](multi.extensions_t[2](2, 2), 1.0))  # a2d keeps its C++ type
----
...
npa2d = np.frombuffer(a2d.data_elements(), dtype=np.float64, count=a2d.num_elements()).reshape(a2d.shape().to_array())
print(npa2d)
----
> ```
> [[1. 2.]
...
The other way around, a Multi reference can be constructed over the NumPy array's memory; modifications through either object are visible in the other:
...
----
such a function can be called from pass:[C++] on Multi array (`arr`), by extracting the size and layout information,
