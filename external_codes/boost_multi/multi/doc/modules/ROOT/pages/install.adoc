...
`sudo apt install cmake git g{plus}{plus} libboost-dev make`
...
Once installed, other CMake projects (or targets) can depend on Multi by adding a single line: `add_subdirectory(my_multi_path)`, or by adding `find_package`:
