...
// ... by analogy, implict conversion is also allowed from array of reals to arrays of complex
multi::array<double, 2> D({10, 10}, 5.0); multi::array<std::complex<double>, 2> Z = D;
// implicit (or explicit) conversions from the other way (real to complex) are disallowed
// multi::array<double, 2> D = Z;  // not allowed (compilation error)
----
