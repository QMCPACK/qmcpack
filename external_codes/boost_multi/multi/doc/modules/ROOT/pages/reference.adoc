...
= Reference of Types and Concepts

:idprefix: reference_

The library interface presents several closely related types (pass:[C++] classes) representing arrays.
...
This section is a conceptual overview of the system of types; if you want to read the API interface as coded, please visit the xref:reference:index.adoc[API documentation page].
...
|===
| Type                              | Dimensionality | Sizes                          | Memory                              | Ownership                            | Common names                                    | Uses

| `multi::array<T, D, ...>`         | Static         | Dynamic/Mutable                | Heap (allocates)                    | Yes, transferable (movable)          | "array", "array-value"                          | General purpose (value type)
| `multi::array_ref<T, D>`          | Static         | Dynamic/Immutable              | Any  (doesn't allocate)             | No, (pinned)                         | "array-reference", "array-view"                               | Use existing buffers (e.g. from other libs)
| `multi::inplace_array<T, D, ...>` | Static         | Dynamic/Mutable (bounded)      | Stack (doesn't heap-allocate)       | Yes, non-transferable (pinned)       | "inplace array", "stack array", "static array"  | Fast, stack constrained, lowest level of divide and conquer
| `multi::dynamic_array<T, D, ...>` | Static         | Dynamic/Immutable              | Heap (allocates, never reallocates) | Yes, non-transferable (pinned)       | "dynamic array", "fixed array", "pinned array"  | Multithreading, prevent invalidation
| `multi::unique_array<T, D, ...>`  | Static         | Dynamic/Mutable (by move from) | Heap (allocates, never reallocates) | Yes (unique), transferable (movable) | "unique array", "move-only array"               | Prevent secondary allocations
| `multi::subarray<T, D>`           | Static         | Dynamic/Immutable              | Any  (doesn't allocate)             | No, (pinned)                         | "subarray", "subarray-reference", "subarray-view"              | As a reference, implicitly used through `auto` or template, prevent code bloat across compilation boundaries
| `multi::restriction<D, F>`        | Static         | Dynamic/Immutable              | None (doesn't allocate)             | None                                 | "array restriction", "lazy array", "array-expression" | Use once, build expressions, pass functions as if they were array refences to generic algorithms 
|===
...
however, it is convenient for documentation to present the classes in a different order since the classes `multi::array`, `multi::dynamic_array`, `multi::array_ref`, and `subarray` have an *is-a* relationship (from left to right), through pass:[C++] public inheritance.
...
A subarray-reference is part (or a whole) of another larger array, and they are represented by xref:reference:boost/multi/subarray.adoc[`multi::subarray`] in the library.
...
The interface is almost the same as for `subarray`, which allows generic programming for both `subarray` and `array_ref`.
...
This class owns the elements it contains and never reliquishes the memory it own before destruction;
it has _restricted_ value semantics because assignments are restricted to sources with equal sizes.
There is no `resize` or `reextents` operation.
...
The pass:[C++] core guidelines proposed a similar (albeit one-dimensional) class, called link:http://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#gslowner-ownership-pointers[`gsl::dyn_array`].
The reason it is called "dynamic" is because it uses dynamic memory (heap memory).
...
A "unique" array is a _D_-dimensional array that manages an internal memory buffer, and it is represented by `multi::unique_array<T, D, Alloc = std::allocator<T>>`.
It is similar to "dynamic" array, except that this class owns the elements it contains but can transfer the memory to another `unique_array`.

Use `unique_array` only when fine grained restrictions over allocations needs to be enforced at compile time.
For most uses, a `multi::array` should be preferred instead.

It is a "move-only" object;
it semantics and use cases is similar to `std::unique_ptr<T[]>`, but in the multidimensional context.

Iterators, subarrays, and pointers are transfered with move.

The destructor deallocates memory and destroys the elements
`operator=` assigns the elements from the source (sizes must match);
but it also supports move semantic in other cases.
...
Finally `multi::inplace_array` is a special kind of `multi::array` that uses an internal stack-based allocator; this guarantees total interface compatibility with `multi::array` (it owns the elements, it is resizable, has iterators, etc.)
...
Because it is stack allocated it doesn't have O(1) moves (move falls back to copy), although copies are fast because memory is in the stack.
It is very efficient for small arrays of known maximum size.

[#ref_restriction]
== Restrictions

A restriction is an array that generates its elements lazily on the fly.
It is defined by combining a D-dimensional function with a D-dimensional extents object.
...
This includes the iteration and element-access interface.
Pointer access and layout access are not part of the interface, since there is no actual memory representation as its elements are not stored.
...
// NOTE: Do not place references in lambda captures and put away in the restriction, because then it would turn into a powerful multidimensional range."
...
----
// Alternatively, the call operator accepts all indices at once:
// [source,cpp]
// ----
// auto& elem = c(i, j, k);
// ----
// `operator+=` with a tuple of indices advances the cursor's base pointer by the dot product of the indices and the strides, allowing efficient pointer arithmetic:
// [source,cpp]
// ----
// c += {i, j, k};  // moves the cursor to position (i, j, k)
// auto& elem = *c;
// ----
// `operator*` and `operator->` dereference the cursor at its current position, and `.base()` returns the underlying element pointer.

Cursor objects are cheap (and trivial) to copy.
Cursors are particularly useful in GPU and SIMD contexts where direct pointer arithmetic over strided memory is more natural than iterator-based traversal.

In some loose sense, cursors are like array-references that "forgot" their sizes, they are a way to pass arrays to interfaces that are restricted to take objects by value (such as CUDA kernels or C functions)
...
Associated fancy pointers and fancy references (if any) are deduced from the allocator types.
