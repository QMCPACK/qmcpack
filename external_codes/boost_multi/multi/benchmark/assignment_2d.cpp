...
static void VectorAssignment(benchmark::State& state){
	std::vector<T> a(N*N); std::iota(begin(a), end(a), 1.11);
	std::vector<T> b(N*N); std::iota(begin(b), end(b), 0.);
	for(auto _ : state){
		b = a;
...
static void MultiAssignment(benchmark::State& state){
	multi::array<T, 2> a({N, N}); std::iota(a.elements().begin(), a.elements().end(), 1.11);
	multi::array<T, 2> b({N, N}); std::iota(b.elements().begin(), b.elements().end(), 0.0);
	for(auto _ : state){
		b = a;
		benchmark::DoNotOptimize(b.data_elements());
		benchmark::ClobberMemory();
	}
}
BENCHMARK(MultiAssignment)->Unit(benchmark::kMillisecond);

static void MultiForLoopAssignment(benchmark::State& state){
	multi::array<T, 2> a({N, N}); std::iota(a.elements().begin(), a.elements().end(), 1.11);
	multi::array<T, 2> b({N, N}); std::iota(b.elements().begin(), b.elements().end(), 0.0);
	for(auto _ : state){
		for(auto i = N*0; i != N; ++i)
			for(auto j = N*0; j != N; ++j)
				b[i][j] = a[i][j];
...
BENCHMARK(MultiForLoopAssignment)->Unit(benchmark::kMillisecond);

static void MultiRangeAssignment(benchmark::State& state){
	multi::array<T, 2> a({N, N}); std::iota(a.elements().begin(), a.elements().end(), 1.11);
	multi::array<T, 2> b({N, N}); std::iota(b.elements().begin(), b.elements().end(), 0.0);
	for(auto _ : state){
		b() = a();
...
BENCHMARK(MultiRangeAssignment)->Unit(benchmark::kMillisecond);

static void MultiElementsAssignment(benchmark::State& state){
	multi::array<T, 2> a({N, N}); std::iota(a.elements().begin(), a.elements().end(), 1.11);
	multi::array<T, 2> b({N, N}); std::iota(b.elements().begin(), b.elements().end(), 0.0);
	for(auto _ : state){
...
BENCHMARK(MultiElementsAssignment)->Unit(benchmark::kMillisecond);
