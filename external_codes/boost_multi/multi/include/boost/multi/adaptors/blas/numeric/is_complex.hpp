...
template<class T> struct is_complex : std::integral_constant<bool, 
	(has_real_v<T> || has_real_fun_v<T>) && (has_imag_v<T> || has_imag_fun_v<T>)
>{};
...
template<class T, class V> struct is_complex_of : std::integral_constant<bool, real_is<T, V>::value && imag_is<T, V>::value>{};
