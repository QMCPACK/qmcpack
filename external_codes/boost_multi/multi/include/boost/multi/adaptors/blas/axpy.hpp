...
->decltype(axpy(n, &alpha, first.base(), first.stride(), d_first.base(), d_first.stride()), d_first + n) {
	return axpy(n, &alpha, first.base(), first.stride(), d_first.base(), d_first.stride()), d_first + n; }
...
class axpy_range {
 ...
 public:
	...
	auto extensions() const { return extents_t<1>{ {0, size()} }; }
	[[nodiscard]] constexpr auto extents() const { return extents_t<1>{ {0, size()} }; }
