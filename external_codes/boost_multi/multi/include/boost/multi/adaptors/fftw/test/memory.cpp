...
auto main() -> int {  // NOLINT(bugprone-exception-escape,readability-function-cognitive-complexity)
	...
	// BOOST_AUTO_TEST_CASE(fftw_2D)
	{
		...
		BOOST_TEST( std::abs(fwd[3][1].real() - (-19.0455)) < 0.0001 );  // Fourier[in, FourierParameters -> {1, -1}][[4]][[2]]
		BOOST_TEST( std::abs(fwd[3][1].imag() - (-2.22717)) < 0.0001 );

		multi::array<complex, 1> const in0 = {1. + 2. * I, 9. - 1. * I, 2. + 4. * I};

		BOOST_TEST( fftw::dft_forward(in[0]) == fftw::dft_forward(in0) );
	}

	// BOOST_AUTO_TEST_CASE(fftw_2D_rotated)
	{
		using complex                 = std::complex<double>;
		[[maybe_unused]] auto const I = complex{0.0, 1.0};  // NOLINT(readability-identifier-length) imag unit
		...
		BOOST_TEST( dft_forward(rotated(in)) == rotated(fwd) );
	}
	...
	// BOOST_AUTO_TEST_CASE(fftw_2D_many)
	{
		...
		BOOST_TEST( dft_forward(in[0]) == out[0] );

		multi::fftw::dft({false, true}, rotated(in), rotated(out), fftw::forward);
		BOOST_TEST( dft_forward(rotated(in)[0]) == rotated(out)[0] );
	...
	// BOOST_AUTO_TEST_CASE(cufft_many_2D)
	{
		...
		auto const in = [] {
			...
			std::generate(ret.data_elements(), ret.data_elements() + ret.num_elements(), [eng = std::default_random_engine{std::random_device{}()}, uniform_01 = std::uniform_real_distribution<>{}]() mutable {
	...
	// BOOST_AUTO_TEST_CASE(fftw_5D)
	{
		...
		BOOST_TEST( size(in) == 4 );

		in[2][3][4][5][6]  = 99.0;
		auto const out_fwd = multi::fftw::dft(in, fftw::forward);

		BOOST_TEST(in[2][3][4][5][6] == 99.0);
		BOOST_TEST( power(in) - power(out_fwd)/num_elements(out_fwd) < 1e-5 );
	}
	...
	// BOOST_AUTO_TEST_CASE(fftw_3D_power)
	{
		...
		BOOST_TEST( std::abs(power(in) - power(out)/num_elements(out)) < 1e-10 );
	}
	...
	// BOOST_AUTO_TEST_CASE(fftw_3D_power_out_of_place_over_ref)
	{
		...
		BOOST_TEST( power(in) - power(out)/num_elements(out) < 1e-10 );
	}
	...
	// BOOST_AUTO_TEST_CASE(fftw_3D_power_out_of_place_over_temporary)
	{
		...
		BOOST_TEST( std::abs(powerin - power(out)/num_elements(out)) < 1e-10 );
	}
	...
	// BOOST_AUTO_TEST_CASE(fftw_1D_power_c_interface)
	{
		...
		BOOST_TEST( size(in) == 16 );

		std::iota(begin(in), end(in), 1.0);
		BOOST_TEST( power(in) == 1496.0 );
		...
		BOOST_TEST( std::abs(power(in) - power(out)/num_elements(out)) < 1e-15 );
	}
...
// 		BOOST_TEST( size(fwd) == size(in) );
// 		BOOST_TEST( data == fwd.data_elements() );

// 		BOOST_TEST( power(  in    ) - power(  fwd    )/size(fwd) < 1e-8 );
// 		BOOST_TEST( power((~in)[0]) - power((~fwd)[0])/size(fwd) < 1e-8 );
// 		BOOST_TEST( power((~in)[2]) - power((~fwd)[2])/size(fwd) < 1e-8 );
// 	}
// 	{
...
// 		BOOST_TEST( fwd.extensions() == in.extensions() );
...
// }
	...
	// BOOST_AUTO_TEST_CASE(fftw_2D_const_range_ref)
	{
		...
		}
		{
			...
			BOOST_TEST(   fwd .size() == 3 );
			BOOST_TEST( (~fwd).size() == 5 );

			BOOST_TEST( fwd[0][0] == in[0][0] );
			BOOST_TEST( fwd[1][0] == in[0][1] );
			BOOST_TEST( fwd[2][0] == in[0][2] );
		}
		{
	...
	// BOOST_AUTO_TEST_CASE(fftw_2D_const_range_ref_part2)
	{
		...
		}
		{
			...
			BOOST_TEST( in2 == in.transposed() );
		}
		{
			auto in2 = +multi::fftw::ref(in.rotated());
			BOOST_TEST( in2 == in.rotated() );
		}
		{
			...
			BOOST_TEST( tt   == in.transposed() );
			BOOST_TEST( in2t == in.transposed() );
		}
		{
			auto in2 = +multi::fftw::ref(in).transposed();
			BOOST_TEST( in2 == in.transposed() );
