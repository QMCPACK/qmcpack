// Copyright 2022-2026 Alfredo A. Correa
// Distributed under the Boost Software License, Version 1.0.
// https://www.boost.org/LICENSE_1_0.txt

#ifndef BOOST_MULTI_CONFIG_VERSION_HPP
#define BOOST_MULTI_CONFIG_VERSION_HPP

// NOLINTBEGIN(cppcoreguidelines-macro-usage,modernize-macro-to-enum) indicate library version
#define BOOST_MULTI_VERSION_MAJOR  0
#define BOOST_MULTI_VERSION_MINOR 91
#define BOOST_MULTI_VERSION_MICRO  1

#define BOOST_MULTI_VERSION (BOOST_MULTI_VERSION_MAJOR * 10000 + BOOST_MULTI_VERSION_MINOR * 100 + BOOST_MULTI_VERSION_MICRO)

// NOLINTEND(cppcoreguidelines-macro-usage,modernize-macro-to-enum)

#endif  // BOOST_MULTI_CONFIG_VERSION_HPP_
