...
Ity gemv_n(Trans IN, T a, ItA A_first, Size n, Itx x_first, T beta, Ity y_first){
	...
	gemv(IN, A_first->size(), n, a, base(A_first), stride(A_first), base(x_first), stride(x_first), beta, base(y_first), stride(y_first));
	switch(IN){
		case 'N' : return y_first + A_first->size();
...
Y1D&& gemv(Trans IN, T a, A2D const& A, X1D const& x, T beta, Y1D&& y){
	...
	auto e = gemv(IN, a, begin(A), end(A), begin(x), beta, begin(y));
	assert( e == end(y) );
...
Y1D&& gemv(A2D const& A, X1D const& x, Y1D&& y){return gemv(1., A, x, 0., y);}
...
int main(){
	{
		...
		double a = 1.1, b = 1.2;
		multi::blas::gemv('T', a, M, X, b, Y); // y = a*M*x + b*y
	...
	}
	{
		...
		double a = 1.8, b = 1.6;
		multi::blas::gemv('N', a, M, X, b, Y); // y = a*(M^T)*x + b*y, y^T = a*(x^T)*M + b*y^T
		...
		cout << Y[2] - Y3[2] << std::endl;
		assert( std::abs(Y[2] - Y3[2]) < 1e-13 );
	}
	{
		multi::array<double, 2> const M = {
			{ 9., 24., 30., 9.},
		...
		multi::array<double, 1> Y = {4.,5.,6.};
		double a = 1.1, b = 1.2;
		multi::blas::gemv(a, M, X, b, Y); // y = a*M*x + b*y
	...
	}
	{
		...
		multi::array<double, 1> Y = {4.,5.,6.};
		double a = 1.1, b = 1.2;
		multi::blas::gemv(a, rotated(M), X, b, Y); // y = a*M*x + b*y
