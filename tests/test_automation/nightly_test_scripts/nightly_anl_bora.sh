...
if [[ $sys == *"ClangDev"* ]]; then
  ...
  if [[ $sys == *"Offload-CUDA2HIP"* ]]; then
    CTEST_FLAGS="$CTEST_FLAGS -DQMC_OPTIONS='-DHIP_ARCH=gfx1030;-DENABLE_OFFLOAD=ON;-DQMC_PERFORMANCE_NIO_MAX_ATOMS=32'"
    CTEST_FLAGS="$CTEST_FLAGS -DENABLE_CUDA=ON -DQMC_CUDA2HIP=ON"
  elif [[ $sys == *"CUDA2HIP"* ]]; then
    CTEST_FLAGS="$CTEST_FLAGS -DQMC_OPTIONS='-DHIP_ARCH=gfx1030;-DQMC_PERFORMANCE_NIO_MAX_ATOMS=32'"
    CTEST_FLAGS="$CTEST_FLAGS -DENABLE_CUDA=ON -DQMC_CUDA2HIP=ON"
  elif [[ $sys == *"Offload"* ]]; then
