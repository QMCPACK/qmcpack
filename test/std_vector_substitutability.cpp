	...
	BOOST_AUTO_TEST_CASE(construct_from_vector_2D) {
		{
			...
			std::vector<multi::array<int, 1>> const aa(AA.begin(), AA.end());  // NOLINT(fuchsia-default-arguments-calls)

			BOOST_TEST(( aa[0] == multi::array<double, 1>{10, 20} ));
		}
		{
			multi::array<int, 2> const AA = {
				{10, 20},
			...
			auto const aa(AA().operator std::vector<std::vector<int>>());
			BOOST_TEST( aa[1][1] == AA[1][1] );
		}
		{
			multi::array<int, 2> const AA = {
				{10, 20},
		...
		}
		{
			...
			auto const aa = static_cast<std::vector<std::vector<int>>>(AA);
			BOOST_TEST( aa[1][1] == AA[1][1] );
		}
		{
