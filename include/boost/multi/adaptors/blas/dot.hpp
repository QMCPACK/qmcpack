...
auto dot_n(Context&& ctxt, XIt x_first, Size count, YIt y_first, RPtr rp) {
	if constexpr(!is_complex<typename XIt::value_type>{}) {
		...
		if constexpr(!is_conjugated<XIt>{} && !is_conjugated<YIt>{}) {
			std::forward<Context>(ctxt)->dotu(count, x_first.base(), stride(x_first), y_first.base(), stride(y_first), rp);
		} else if constexpr(!is_conjugated<XIt>{} && is_conjugated<YIt>{}) {
			std::forward<Context>(ctxt)->dotc(count, underlying(y_first.base()), stride(y_first), x_first.base(), stride(x_first), rp);
		} else if constexpr(is_conjugated<XIt>{} && !is_conjugated<YIt>{}) {
			std::forward<Context>(ctxt)->dotc(count, underlying(x_first.base()), stride(x_first), y_first.base(), stride(y_first), rp);
		} else if constexpr(is_conjugated<XIt>{} && is_conjugated<YIt>{}) {
			static_assert(!sizeof(XIt*), "not implemented in blas");
		}
	}
	...
	struct {
		XIt x_last;
		YIt y_last;
	} ret{x_first + count, y_first + count};
	return ret;
}
...
template<class Context, class X1D, class Y1D, class R, std::enable_if_t<!multi::has_base<std::decay_t<R>>::value, int> = 0>  // NOLINT(modernize-use-constraints) TODO(correaa) for C++20
...
struct dot_ref : private Ptr {
	...
	template<class Other, class = std::enable_if_t<!std::is_same_v<dot_ref, Other>>>  // NOLINT(modernize-use-constraints) TODO(correaa) for C++20
...
auto operator,
	(X1D const& x, Y1D const& y)  // NOLINT(readability-identifier-length) BLAS naming
	-> decltype(dot(x, y)) {
	return dot(x, y);
}
