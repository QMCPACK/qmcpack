...
int main() {
	...
	// BOOST_AUTO_TEST_CASE(cublas_one_gemv_complex_conj_zero)
	{
		...
		blas::gemv(1.0, blas::T(A), x, 0.0, y);
		{
			...
			BOOST_TEST( std::abs( static_cast<complex>(y[0]).real() - static_cast<complex>(yy[0]).real()) < 1e-7);
			BOOST_TEST( static_cast<complex>(y[1]) == static_cast<complex>(yy[1]) );
			BOOST_TEST( static_cast<complex>(y[2]) == static_cast<complex>(yy[2]) );
		}
		{
...
// TODO(correaa) MKL gives an error here
	...
	// BOOST_AUTO_TEST_CASE(cublas_one_gemm_complex_trans_first)
	{
		...
		};
		{
			...
			std::transform(begin(transposed(B)), end(transposed(B)), begin(transposed(C_copy)), begin(transposed(C_copy)), [&A, aa = 1.0, bb = 0.0](auto const& Bc, auto&& Cc) { return blas::gemv(aa, blas::T(A), Bc, bb, std::move(Cc)); });
		...
		}
		{
			...
			std::transform(begin(transposed(B)), end(transposed(B)), begin(transposed(C_copy)), begin(transposed(C_copy)), [&A, aa = 1.0, bb = 1.0](auto const& Bc, auto&& Cc) { return blas::gemv(aa, blas::T(A), Bc, bb, std::move(Cc)); });
	...
	// BOOST_AUTO_TEST_CASE(cublas_one_gemm_complex_trans_both)
	{
		...
		};
		{
			...
			std::transform(B.begin(), B.end(), C_copy.transposed().begin(), C_copy.transposed().begin(), [&A, aa = 1.0, bb = 0.0](auto const& Br, auto&& Cc) { return blas::gemv(aa, blas::T(A), Br, bb, std::move(Cc)); });
		...
		}
		{
			...
			std::transform(begin(B), end(B), C_copy.transposed().begin(), C_copy.transposed().begin(), [&A, aa = 1.0, bb = 1.0](auto const& Br, auto&& Cc) { return blas::gemv(aa, blas::T(A), Br, bb, std::move(Cc)); });
...
// unknown location(0): fatal error: in "cublas_one_gemv_complex_conjtrans_zero": memory access violation at address: 0x00000007: no mapping at fault address
	...
	// BOOST_AUTO_TEST_CASE(UTA_blas_trsm_complex_nonsquare_default_diagonal_hermitized_gemm_check_no_const)
	{
		...
		BOOST_TEST( std::abs( static_cast<complex>(B[1][2]).imag() - -0.147059) < 0.001);
	}
	...
	// BOOST_AUTO_TEST_CASE(UTA_blas_trsm_complex_nonsquare_default_diagonal_hermitized_gemm_check_no_const_UTH)
	{
		...
		BOOST_TEST( std::abs( static_cast<complex>(B[1][1]).imag() - -0.0811359 ) < 0.001 );
		BOOST_TEST( std::abs( static_cast<complex>(B[2][1]).imag() - -0.147059  ) < 0.001 );
	}
	...
	// BOOST_AUTO_TEST_CASE(UTA_blas_trsm_complex_nonsquare_default_diagonal_gemm_check_no_const_right)
	{
		...
		BOOST_TEST( std::abs(static_cast<complex>(B[1][2]).imag() - 1.60142) < 0.001 );
	}
	...
	// BOOST_AUTO_TEST_CASE(UTA_blas_trsm_complex_nonsquare_default_diagonal_gemm_check_no_const_right_LT)
	{
		...
		BOOST_TEST( std::abs( static_cast<complex>(B[1][2]).imag() - -0.0882353) < 0.001 );
	}
