...
stored as separate C++ variables.
...
There are several libraries for multidimensional arrays in C++ (see the section on comparison with other libraries), and all address this abstraction problem at different levels of generality.
...
First, we extract the extensions (the 2-dimensional range of valid indexes) of the array,
...
An array can be assigned at any moment, independently of the previous state or shape (extensions).
...
Because of their semantics they are called subarray-references and they are documented in Section <<reference_>>.
...
As we described earlier, it can be regarded as a product type of one-dimensional extensions,
...
therefore and as an example `multi::array<std::string, 2> A2({2, 3}, multi::uninitialized_elements)` will not compile because `std::string` does not have a trivial constructor.
...
In other C++ libraries, these are known as link:https://en.wikipedia.org/wiki/Expression_templates[Expression Templates] because they leverage heavy templated types.
