...
We can create a static C-array of ``double``s, and refer to it via a two-dimensional (2D) array `multi::array_ref<double, 2>`.
...
Next, just as an illustration, we print the elements in a way that corresponds to the logical arrangement.
...
Of course many other standard algorithms work with arrays at the top level dimension, or at the element level dimension.
...
For this type of usage, the valid range of indices in all the dimensions of an array is extracted with `.extents()`, and in the 2D case, `.extents()` can be conveniently decomposed into two ranges, one for each dimension.
...
The resulting object are of type `multi::subarray`, which is a type name that is rarely used explicity in code, thanks to the use of `auto`. 
...
In fact, using the `.elements()` method, one can achieve an effective way to fuse nested loops.
...
For example, the results of `A.extension()` is itself iterable and indexable; returning a sequence with valid indices for `A`.

[source,cpp]
----
	for(auto idx : A.extension()) {
		A[idx][0] = 5;
	}
----

`A.extension()` not only has a `.begin()` and `.end()`, but it also can be indexed, `A.extension()[1] == 1`.
`A.extension()` is conceptually the sequence {0, 1}

`A.extents()` deserves special attention because manipulation of this object enables powerful constructs.
As we described earlier, it can regarded as a product type of one-dimensional extensions, 
...
----
link:https://godbolt.org/z/MojPrb5eP[(open on Compiler Explorer)]
...
These are called "restrictions arrays" here, also commonly known "lazy arrays", and they enable delayed evaluation.
...
The main constrain of the function is that it needs to return values independently of the order of evaluation in which elements are probed; more formally it has to be link:https://eel.is/c++draft/res.on.requirements#3[equality preserving] in the context in which it is used.
...
Note that lazy expression are themselves multidimensional arrays, and share their generic interface (`.extents()`, `.size()`, `.begin/end()`, etc) therefore they can be combined with algorithms (or used as part of other expressions)
