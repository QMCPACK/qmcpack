...
Opium is a pseudopotential generation code available from the website \url{http://opium.sourceforge.net/}.  Opium can generate pseudopotentials with either Hartree-Fock or DFT methods.  Once you have a useable pseudopotential param file (for example, \ishell{Li.param}), generate pseudopotentials for use in Quantum ESPRESSO with the \ishell{upf} format as follows:
\begin{lstlisting}[style=SHELL,caption=Generate UPF-formatted pseudopotential with Opium]
opium Li.param Li.log all upf
\end{lstlisting}
This generates a UPF-formatted pseudopotential (\ishell{Li.upf}, in this case) for use in Quantum ESPRESSO.  The pseudopotential conversion tool \ishell{ppconvert} can then convert UPF to FSAtom xml format for use in QMCPACK:
...
\begin{lstlisting}[style=GAMESS,caption=BFD Li pseudopotential in GAMESS format]
Li-QMC GEN 2 1
3
...
\end{lstlisting}
Save these lines to a file (here, named \ishell{Li.BFD.gamess}; the exact name may be anything as long as it is passed to \ishell{ppconvert} after --gamess\_pot).  Then, convert using \ishell{ppconvert} with the following:
...
\subsection{CASINO}
\label{subsec:CASINO}
The QMC code CASINO also makes available its pseudopotentials available at the website \url{https://vallico.net/casinoqmc/pplib/}. To use one in QMCPACK, select a pseudopotential and download its summary file (\ishell{summary.txt}), its tabulated form (\ishell{pp.data}), and (for ppconvert to construct the projectors to convert to Quantum ESPRESSO's UPF format) a CASINO atomic wavefunction for each angular momentum channel (\ishell{awfn.data\_*}).  Then, to convert using ppconvert, issue the following command:
...
\end{lstlisting}
QMCPACK can directly read in the CASINO-formated pseudopotential (\ishell{pp.data}), but four parameters found in the pseudopotential summary file must be specified in the pseudo element (\ishell{l-local}, \ishell{lmax}, \ishell{nrule}, \ishell{cutoff})[see Section~\ref{sec:nlpp} for details]:
