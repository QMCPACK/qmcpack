...
\begin{itemize}
  ...
  \item{Generating multi-determinant trial wavefunctions with GAMESS (CISD, CASSCF, CASCI, SOCI)}
...
│   │   ├── h2o.ptcl.xml        - converted particle positions
│   │   └── h2o.wfs.xml         - converted wave function 
│   └── thres0.075           - VMC optimization with more determinants
│       ├── optm.xml            - QMCPACK VMC optimization input
│       ├── h2o.ptcl.xml        - converted particle positions
│       └── h2o.wfs.xml         - converted wave function 
...
jobrun_vesta rungms h2o.hf.inp > h2o.hf.out
cd ../convert
ln -s ../gms/h2o.hf.out
jobrun_vesta convert4qmc -gamessAscii h2o.hf.out -add3BodyJ
...
and h2o.wfs.xml. Study both files and submit when you are ready. Notice that the
...
\end{verbatim}
\end{shaded}
The resulting energy as a function of optimization step should look qualitatively similar to figure \ref{fig:lam_opt_conv} below.
...
slightly different in this case), copy the input file optm.xml %and the sample submission file
...
coefficient cutoffs with the energies obtained with DFT orbitals. Due to the time limitations
...
wave-function optimization. Go to “ex3\_multi-slater-jastrow” and make folders for the remaining
wave-function types: CASCI and SOCI. The exercise has already been performed with a CISD wave-function in exercise \#2.
