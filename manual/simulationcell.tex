...
the box used for Ewald summation increases along the axis labeled \texttt{n} by a factor of \texttt{vacuum}.
The default value is 1, \textit{i.e.} no change to the specified axis.
All the particles remain in the original box without altering their positions.
...
This option helps reduce the computational cost of DFT and memory requirement in QMC.
...
a lot of memory is needed to store the orbitals in B-Spline representation.
With the `vacuum' option, the DFT calculation can use a smaller box (but still large enough for accuracy)
and the QMC calculation can rescale the open directions of the box to
have the long-range parts of Coulomb interations calculated in the large box.
