...
\subsection{Installing on ORNL SummitDev}
SummitDev is the development cluster for the next GPU accelerated
supercomputer at Oak Ridge National Laboratory's Oak Ridge
Leadership Computing Facility  (ORNL OLCF).
...
./configure --prefix=${MYSOFTWARE_PATH} --with-python-install-dir=${MYSOFTWARE_PATH}/lib/python
make -j 8
make install
\end{verbatim}
...
PKG_CONFIG_PATH and LD_LIBRARY_PATH to be set for libxml2 properly
set at compile and runtime the following three lines should also be
...
cd build_summitdev
cmake -DCMAKE_C_COMPILER="mpicc" -DCMAKE_C_FLAGS="-std=gnu11 -qsmp -O3" -DCMAKE_CXX_COMPILER="mpicxx" -DCMAKE_CXX_FLAGS="-std=gnu++11 -qnoxlcompatmacros -qsmp -O3" -DQMC_CUDA=1 -DCUDA_NVCC_FLAGS="-arch=sm_60;-Drestrict=__restrict__;-DNO_CUDA_MAIN;-O3" ..
make -j 8
ls -l bin/qmcpack 
\end{verbatim}
