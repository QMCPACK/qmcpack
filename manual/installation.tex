...
For large problem sizes, the initialization of spline orbitals may take quite a large portion of runtime.
Consider running qmcpack once with \texttt{--save\_wfs=1} to save the spline coefficients to the disk and load it for later runs in the same folder..

The delayed update algorithm in section~\ref{sec:singledeterminant} significantly change the performance characteristics of QMCPACK.
A parameter scan of the maximal number of delays is required for achieving the best performance of every architecture and problem size.
