...
\subsubsection{Serial or multi-threaded library}
\label{sec:threadedlibrary}
Vendors may provide both serial and multi-threaded versions of their libraries.
...
When being called from outside an OpenMP parallel region, the multi-threaded version is preferred for the possibility of using all the avaiable cores.
...
Fortunately nowadays the multi-threaded versions of many vendor libraries (MKL, ESSL) are OpenMP aware.
...
If the multi-threaded version doesn't provide this feature of dynamically adjusting the number of threads,
the serial version is preferred. In addition, thread-safety is required no matter whichever version is in use.
...
make -j 56
\end{shade}
Note that armclang is recognized as an `unknown' compiler by cmake v3.13.* and below. In this case, we need to force it as clang (use (\ishell{-DCMAKE_\{C,CXX\}_COMPILER_ID}) to apply necessary flags.
...
e.g. \ishell{short-LiH_dimer_ae_vmc_hf_noj_16-1} indicates a short VMC test
for the LiH dime. 
...
Currently under development is a wider range of deterministic integration
tests. The goal is to test much more of QMCPACK than the unit tests
currently do, and to do so in a manner that is completely reproducible
across platforms. All of these should eventually pass 100\% reliably
and quickly. Currently some fail on some platforms and for certain
build types.
