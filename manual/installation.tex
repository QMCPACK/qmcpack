...
\item Untar the archive, e.g., \ishell{tar xvf qmcpack\_v1.3.tar.gz}
\end{itemize}
...
If the install step is desired, run \ishell{make install} command to install the QMCPACK executable, the converter,
...
Specify the \ishell{CMAKE\_INSTALL\_PREFIX} CMake variable during configuration to set the install location.
...
\ishell{\~/.spack/packages.yaml}. For example, this works on Mac OS X for the Intel MKL package.
...
e.g. \ishell{short-LiH\_dimer\_ae\_vmc\_hf\_noj\_16-1} indicates a short VMC test
for the LiH dime. 
...
The system-level test directories are grouped into \ishell{heg}, \ishell{molecules}, and \ishell{solids}, with particular physical systems under each (for example \ishell{molecules/H4\_ae}
...
we suggest checking some of the test console output in \ishell{build/Testing/Temporary/LastTest.log}
or the output files under \ishell{build/tests/}.
%The runs occur in \ishell{build/tests/test\_dir/test\_name}.
...
Consider running QMCPACK once with \ishell{--save\_wfs=1} to save the
...
\ishell{-DBUILD\_PPCONVERT=1} to cmake and then typing \ishell{make ppconvert}.
...
\end{shade}
   ...
   but use the old non-HDF5 I/O within QE, replace \ishell{-D\_\_HDF5} with \ishell{-D\_\_HDF5\_C} in make.inc.
\item if your system has HDF5 with C only, manually edit make.inc by adding \ishell{-D\_\_HDF5\_C} and \ishell{-DH5\_USE\_16\_API}
   in \ishell{DFLAGS} and provide include and library path in \ishell{IFLAGS} and \ishell{HDF5\_LIB}
\end{itemize}
...
By adding \ishell{-D QE\_BIN=your\_QE\_binary\_path} in the cmake command line when building your QMCPACK,
tests named with ``qe-'' prefix will be included in the test set of your build.
You can test the whole pw$\to$pw2qmcpack$\to$qmcpack workflow by
...
\item Verify that the compilers and libraries that you expect are
  ...
  \begin{shade}
  cmake -DCMAKE\_C\_COMPILER=/full/path/to/mpicc -DCMAKE\_CXX\_COMPILER=/full/path/to/mpicxx ..
  \end{shade}
