...
\section{Spack Package (Beta)}
Think of Spack as MacPorts for scientific software. One of goals of
Spack is to reduce the barrier for user to installing scientific
...
automatically build for you and that different versions of QMCPACK can
...
something sensible on multi-core architectures. This can be
...
Next, make sure your compilers are properly detected. For many
...
automatically detected. You you will need to manually add them to your
\texttt{~/.spack/packages.yaml}. For example, this works on a Mac
laptop for the Intel MKL package.
\begin{verbatim}
...
The QMCPACK Spack package has a number of variants (combination of
compile-time options). A full list can be display by typing:
\begin{verbatim}
...
For example, to install QMCPACK in mixed-precision with support for
k-points simply type:
\begin{verbatim}
...
Spack does not set-up an environment to find your own packages
...
Then modify our\texttt{ ~/.cshrc} file with something like
\begin{verbatim}
...
section once we have a recipe that works.
