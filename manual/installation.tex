...
QMCPACK makes calls from both inside and outside the threaded region.
When being called from outside OpenMP parallel region, multi-threaded version is preferred for possibly using all the avaiable cores.
When being called from by every thread inside OpenMP parallel region, serial version is preferred for not oversubscribing the cores.
Fortunately nowadays the multi-threaded versions of many vendor libraries (MKL, ESSL) are OpenMP aware.
They use only one thread when being called inside the OpenMP parallel region.
...
the serial version is preferred. In addtion, thread-safety is required no matter whichever version in use.
