...
For small to medium problem sizes, using one thread on each core working on multiple walkers is the most efficient way because of minimal thread synchronization cost.
...
Setting the right thread affinity is critical to the performance. However, how to set the affinity is machine dependent.
...
although the knowledge of how OS logical CPU cores (/prco/cpuinfo) are bound to the hardware is needed.
