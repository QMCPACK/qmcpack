...
\item[\texttt{-{}-enable-timers=none|coarse|medium|fine}]{ Control the timer granularity
  when the build option \ishell{ENABLE_TIMER} is enabled. }
...
Most multi-threaded BLAS/LAPACK libraries do not spawn threads by default
when being called from an OpenMP parallel region. See explanation in \ref{sec:threadedlibrary}.
...
However, this forces all the calls to the library using the same amount of threads.
...
Instead, QMCPACK use the library APIs to turn on the nested threading only at selected performance critical calls.
In the case of using serial library, QMCPACK implements nested threading to distribute the workload wherever necessary.
