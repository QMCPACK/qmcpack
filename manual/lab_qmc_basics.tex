...
\label{sec:lqb_dft}
With the pseudopotential in hand, the next step toward a QMC calculation is to obtain the Fermionic part of the wavefunction, in this case a single Slater determinant constructed from DFT-LDA orbitals for a neutral oxygen atom.

Enter the \texttt{oxygen\_atom} directory and copy over the oxygen pseudopotential files (\texttt{O.BFD.upf} and \texttt{O.BFD.xml}) you made from section \ref{sec:lqb_pseudo}.  Alternatively, the already converted pseudopotential files are located in the \texttt{oxygen\_atom/reference} directory.  
...
\begin{lstlisting}[caption={Quantum Espresso input file for the neutral oxygen atom (O.q0.dft.in)\label{lst:O_q0_dft}}]
&CONTROL
   calculation       = 'scf'
...
Run Quantum Espresso by typing the following if you are on a workstation or simple cluster (it is assumed below that the PWSCF executable, \texttt{pw.x}, is in your \texttt{PATH})
\begin{shaded}
\begin{verbatim}
mpirun -np 4 pw.x -input O.q0.dft.in >&O.q0.dft.out&
\end{verbatim}
\end{shaded}
or if you are completing the lab on the VESTA supercomputer at ALCF, instead type
\begin{shaded}
\begin{verbatim}
...
The DFT run should take a few minutes to complete.  Once finished, you should check the LDA total energy in \texttt{O.q0.dft.out} by typing ``\texttt{grep '!  ' O.q0.dft.out}''.  The result should be close to
\begin{shaded}
\begin{verbatim}
...
\begin{lstlisting}[caption={\texttt{pw2qmcpack.x} input file for orbital conversion (O.q0.p2q.in)\label{lst:O_q0_p2q}}]
&inputpp
  prefix     = 'O_q0'
  outdir     = 'O_q0_dft_output'
  write_psir = .false.
...
Perform the orbital conversion now by typing the following if you are on a workstation or simple cluster (again it is assumed below that \texttt{pw2qmcpack.x} is in your PATH)
\begin{shaded}
\begin{verbatim}
...
\end{align}
Here $\sigma$ is a spin variable while $r_i$ and $r_I$ represent electron and ion coordinates, respectively.  The introduction of $J$ into the wavefunction is similar to F12 methods in quantum chemistry, though it has been present in essentially all QMC studies since the origins of the method in 1965.

How are the functions $u_2^{\sigma\sigma'}$ and $u_1^{\sigma}$ obtained?  Generally, they are approximated by analytical functions with several unknown parameters that are determined by minimizing the energy or variance directly within VMC since the energy and variance reach a global minimum only for the true ground state wavefunction ($\textrm{Energy}=E\equiv\expval{\Psi}{\hat{H}}{\Psi}$, $\textrm{Variance}=V\equiv\expval{\Psi}{(\hat{H}-E)^2}{\Psi}$).
