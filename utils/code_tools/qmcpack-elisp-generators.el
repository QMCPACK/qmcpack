...
(defun qmcp-add-enum-string-set()
  "Select all input enum classes into a single region to create string to value lookup
   to use with assignAnyEnum unordered_map is written on starting on the line after the REGION"
  (interactive)
  (let ((leading-indent (qmcp-leading-indent)))
    (save-excursion
      (save-restriction
