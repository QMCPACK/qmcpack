...
echo ${run_file}
echo ${ref_file}
${QMCA} --fp=24.12f $run_file 2>/dev/null | gawk -e '/\s\s[a-zA-Z]+\s+=/ {print $1 " " $3 " " $5}' > run_vals
${QMCA} --fp=24.12f $ref_file 2>/dev/null | gawk -e '/\s\s[a-zA-Z]+\s+=/ {print $1 " " $3 " " $5}' > ref_vals
...
for ref, short in zip(rlines, slines):
    combined = math.sqrt(float(ref[2])**2 + float(short[2])**2)
    if ref[0] in why_why_why:
        ref_data.append([why_why_why[ref[0]], float(ref[1]), float(combined)])
print(ref_data)
for rd in ref_data:
