...
``*.scalar.dat``, ``*.dmc.dat``, and ``*.vmc.dat`` (see :ref:`output-overview` for a detailed description of these files).
...
::
  ...
  >qmca -t -q e -e 100 qmc.s000.vmc.dat

For a ``vmc.dat`` file, the equilibration count supplied with ``-e`` is in
VMC steps (file rows), rather than blocks.
